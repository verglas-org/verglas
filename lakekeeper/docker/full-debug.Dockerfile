...
RUN $NO_CHEF || cargo chef cook --all-features --locked --recipe-path recipe.json
# Build application
COPY . .
...
COPY --chmod=555 --from=builder /app/target/debug/lakekeeper /home/nonroot/lakekeeper
