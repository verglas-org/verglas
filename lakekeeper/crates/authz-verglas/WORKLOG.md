...
- Added `verglas` backend selection to the Lakekeeper server and OpenAPI command.
- Added contract tests and operator documentation.
- Added fail-closed Lakekeeper lifecycle synchronization for catalog resources.
...
- Removed caller-selected principals and Lakekeeper principal synchronization from the Verglas path.
...
- #135: Marked the Verglas-authored authorization adapter as FSL-1.1-ALv2.
  The catalog licensing document now distinguishes it from upstream Lakekeeper.
- #0: Replaced the tenant-local authorization client with Cloudflare JWKS
  verification in Lakekeeper. Short-lived Worker credentials now carry the
