...
- #135: Moved the CRaft-backed Lakekeeper adapter into `verglas-cloud` while
...
- #137: Replaced the pinned git dependencies on `verglas-catalog` and
  `verglas-consensus` with path dependencies, as part of moving the Lakekeeper
  fork into the `verglas` repository under `lakekeeper/`. Engine changes no
  longer need a re-pin commit here; they are picked up from the checkout, and
  `.github/workflows/lakekeeper.yml` builds this crate when those engine crates
