...
impl<A: Authorizer + Clone> AuthorizedVerglasCatalog<A> {
    ...
    /// startup (`serve-craft` binds the tenant name as that database id — see
    /// `serve_craft::serve_craft`), not a UUID: it exists so a hosted request
    /// can be routed to the right durable database, not to mint an authz
    /// warehouse id. `self.warehouse.warehouse_id` already holds the correct
    /// value, deterministically derived from the warehouse name at
    /// construction (same `Uuid::new_v5` scheme `serve_craft` uses). Reparsing
    ...
    for AuthorizedVerglasCatalog<A>
{
    ...
    async fn get_config(
        ...
        // it while lakekeeper's own IO keeps the local path.
        ...
        Ok(lakekeeper::api::CatalogConfig {
            ...
            overrides,
            endpoints: lakekeeper::api::iceberg::supported_endpoints().to_vec(),
        })
    }
}
