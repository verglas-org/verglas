...
/// The durable database declarations are replayed after every Lakekeeper start.
...
impl MetadataRoots {
    /// Binds a Lakekeeper warehouse prefix to its isolated managed bucket.
    pub fn bind(
        &self,
        warehouse: &str,
        database_id: &str,
    ...
    /// Removes a warehouse root after its Lakekeeper profile is detached.
...
impl FileIoMetadataStore {
    ...
    #[must_use]
    pub fn new(io: Arc<dyn LakekeeperStorage>, roots: MetadataRoots) -> Self {
        Self { io, roots }
    }
...
mod tests {
    ...
    /// Each registered Lakekeeper warehouse prefix receives an independent
