...
- **Runtime shutdown is fenced.** Verglasd stops event admission, waits for the embedded Turso WAL checkpoint and Stream outbox fences, closes the event endpoint, and only then stops the child. Foyer contents may be discarded at any time because they are not durable state.
