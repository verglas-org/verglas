...
the daemon warmed the table's metadata into #50's pinned store _before any
...
Daemon warming is driven by the **`[catalog]` REST watcher**: verglasd polls an
...
This demo stands up an **Apache Polaris** container as the REST catalog verglasd
watches (same pinned digest as `benchmarks/polaris`), seeds a TPC-H table
_through Polaris_ with its data on the same origin S3 the daemon fills from, and
configures verglasd with `[catalog] uri` → Polaris and `[cache.warming] enabled
...
```
 warming_demo.py (host) ──seed (pyiceberg REST)─────────▶ Polaris (:8181) ──▶ origin S3 (OCI)
                        ──planning walk (Verglas S3)───▶ verglasd (:8555) ──▶ origin S3 (OCI)
                                                             ▲
 verglasd  ──[catalog] watch (REST poll)──▶ Polaris ; on a pointer swing the
...
runs each; the daemon's `/admin/stats` meta counters recorded as mechanism
evidence:
...
**Config B trigger.** A fresh warming-on daemon starts against the
...
B measures a genuinely daemon-warmed table. (Commit-driven re-warms exercise
...
Config B is pure **startup warming**: the daemon started against the
...
| B — warming ON, daemon-warmed at startup, cold | 469.1, 406.7 | **437.9** |
| C — warm floor | 383.6, 402.7 | **393.1** |

**A/B = 33.3× faster cold once the daemon warms it. B sits at 1.11× the warm
...
- **B/C serve the entire walk from the pinned store** — 115 `meta_hits` (24
  blocks + 91 footers), **zero backend bytes**. The daemon walked the metadata
...
- Built daemon binaries: `cargo build --release -p verglas -p verglasd` →
  `target/release/verglasd`.
...
verglasd and the driver reach it. The **prefix must be non-empty** — the guard
...
# 3. Teardown: kill verglasd, drop catalog/tables, prefix-scoped S3 delete,
...
- **Explicit PID kill (#170).** `stop_verglasd` kills every PID holding the S3
  ...
  supervisor is absent, teardown leaves no verglasd behind.
- **`.env` is gitignored and never logged.** The driver reads origin secrets
  ...
  `verglasd.toml` (which carries a short-lived Polaris bearer token) is
  gitignored.

## A bug this demo caught (fixed in #177)

The first working version of this demo could only trigger warming with a
commit: a table that **already existed** in the catalog when verglasd started
...
- `run.sh` — the single entrypoint (venv, Polaris lifecycle, verglasd lifecycle
