...
- **Sockets never pin compute.** Container edge sockets, the catalog change feed (§7.9), and the SDK's `follow` terminate on a hibernating edge object. Worker scheduling accepts bounded events, including complete HTTP callbacks; it never owns a client connection.
...
A deployment is one record: kind (worker | container) × trigger (cron | webhook | data_change, for a worker) × placement (local | cloud), plus the code or image, its configuration, the tables and databases it is granted (§7.5), and — for a stateful or stateless container — its scaling shape. Everything the platform does — scheduling, dispatch, scale, pause and resume, audit — is a function of this record, which is why it must be one record: a workload whose schedule lives in a crontab, whose code lives in a repository, and whose scaling lives in a separate orchestrator cannot be inspected, moved, or replayed as a unit. Locally the registry is the `verglas_sys` namespace: Iceberg tables whose rows are append-only revisions, so the registry inherits the audit and time-travel properties of §1.1 — the snapshot log is the complete lifecycle history of every deployment, readable by any engine. In the cloud the same records are typed entries in the tenant's Lakebase catalog. Lakebase may execute those object operations over its serverless Postgres implementation, but that storage detail is not part of the scheduler contract. These are two projections of one record shape; a deployment moves between them by changing its placement field, not by being rewritten.
...
Triggers are deployment configuration, not code. The SDK types them so a deployment can declare what it expects, but the platform's control plane registers them and dispatches the resulting event to `ctx.trigger`. Three kinds are defined: **cron** (a schedule), **webhook** (a complete inbound HTTP request), and **data_change** (a commit to a granted table — the streaming-transform trigger that a materialized view used to be). Manual dispatch is its own event. The scheduler stores bounded events; it never owns an HTTP or WebSocket connection.
...
Scheduling is a separate stateless service. Each tenant has one logical worker queue. Manual REST calls, complete HTTP callbacks, cron reconciliation, and catalog data-update subscriptions all materialize the same durable job shape in that queue. A deterministic trigger identity makes enqueue idempotent; lease generations fence competing or recovered consumers; a completion is accepted only for the live generation. The scheduler can therefore restart or migrate without a filesystem guard or process-local cursor. Postgres owns queue state in both deployments: an operator supplies a Postgres URL to the optional on-prem scheduler container, while cloud uses the tenant queue in Lakebase Postgres. If the on-prem scheduler is not configured, Verglas serves storage, catalog, and query normally and does not mount worker-trigger routes.

The scheduler is a running queue consumer. Pushed worker events are persisted before the event API acknowledges them and are eligible to run immediately. Cron, retry, and lease deadlines use exact timers derived from durable queue state; there is no periodic queue poll and no separate wake endpoint. On-prem, the catalog change feed pushes matching data-update events to the scheduler. In cloud, event producers write the same tenant queue contract and the fleet scheduler claims work and chooses a microVM host.
...
In both shapes the control plane monitors the active job and owns the scaling decision; the user declares only the shape and the allocation ceiling. Locally, a container just runs on the server's host as an ordinary process or image; in the cloud it decomposes onto a fleet microVM (§7.6). Container support in the cloud is the newest surface and is landing incrementally alongside the fleet.
...
A long-lived inbound socket must not pin a backend that is supposed to scale to zero. Verglas terminates such sockets on a hibernating edge object. The catalog change feed is the shipped instance: one websocket per client carries table-commit notifications (seq, table, snapshot id, commit time), multiplexed and filtered client-side, held by the edge object while the tenant backend sleeps. The SDK's `follow` rides that feed; `followRows` layers on it, turning each commit notification into a bounded `delta` read that wakes the backend only to fetch the new rows. Container connection brokering is a separate cloud concern. Worker scheduling accepts bounded HTTP callback events and does not retain the client connection.
