...
Triggers are deployment configuration, not runtime payload variants. The SDK types **cron**, **webhook**, and **event** subscriptions, while every dispatch is one CloudEvents 1.0 envelope delivered to `ctx.trigger`. Manual requests, cron ticks, and complete inbound HTTP requests are CloudEvent producers. Iceberg commits use `org.apache.iceberg.snapshot.committed`; an on-prem catalog poller emits that event, while cloud Lakebase emits the same event directly. An event subscription matches exact `type` and optional `source` and `subject` attributes. The scheduler stores bounded CloudEvents and never owns an HTTP or WebSocket connection.
