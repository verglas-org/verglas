...
echo "[server] building verglas-runtime" >&2
( cd "$REPO" && cargo build -q -p verglas-runtime )
VGD="$REPO/target/debug/verglas-runtime"
...
echo "[server] starting verglas-runtime (TLS) -> MinIO" >&2
AWS_ENDPOINT="$MINIO" \
