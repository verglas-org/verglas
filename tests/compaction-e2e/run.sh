...
# the live 8333/8334 server, never a real tenant), builds and runs the branch
# verglas-server + verglas against them, seeds NUM_BATCHES small appends (the real
# pathology: hundreds of tiny commits), then compacts twice via the server's
...
fail() { echo "FAIL: $*" >&2; [ -f "$WORK/server.log" ] && { echo "--- server.log tail ---" >&2; tail -40 "$WORK/server.log" >&2; }; exit 1; }
...
echo "== 2. build + run the branch verglas-server + verglas on high ports =="
...
cargo build --release -p verglas-server -p verglas --manifest-path "$root/Cargo.toml" 2>&1 | tail -3
DBIN="$root/target/release/verglas-server"
CLI="$root/target/release/verglas"

"$DBIN" --config "$WORK/verglas.toml" >>"$WORK/server.log" 2>&1 &
SERVER_PID=$!
wait_for "http://127.0.0.1:$DADMIN/admin/healthz" 60 || fail "server did not become healthy"
echo "server pid $SERVER_PID on admin:$DADMIN s3:$DS3"
...
"$CLI" --server-endpoint "$EP" table create qa.commits "$WORK/batches/batch_0000.jsonl" --json >/dev/null \
  || fail "table create failed"
for f in "$WORK"/batches/batch_*.jsonl; do
  [ "$f" = "$WORK/batches/batch_0000.jsonl" ] && continue
  "$CLI" --server-endpoint "$EP" table append qa.commits "$f" --json >/dev/null || fail "append $f failed"
done
...
BEFORE="$("$CLI" --server-endpoint "$EP" table show qa.commits --json)"
...
echo "== 5. compact (server: verglas table compact → POST /admin/compact) =="
COMPACT_JSON="$("$CLI" --server-endpoint "$EP" table compact --json)"
...
AFTER="$("$CLI" --server-endpoint "$EP" table show qa.commits --json)"
...
HISTORY_COUNT=$("$CLI" --server-endpoint "$EP" table history qa.commits --json | python3 -c 'import json,sys; d=json.load(sys.stdin); print(len(d.get("snapshots", d if isinstance(d, list) else [])))')
...
COUNT_STAR="$("$CLI" --server-endpoint "$EP" query "select count(*) as n from qa.commits" --json | python3 -c 'import json,sys;print(json.load(sys.stdin)["rows"][0]["n"])')"
...
PASS2_JSON="$("$CLI" --server-endpoint "$EP" table compact --json)"
