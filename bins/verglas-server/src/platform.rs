//! The server-hosted worker runtime (WHITEPAPER §7.3/§7.4).
//!
//! The server runs the agent-data platform's workers as internal tasks. A worker
...
//! - **data_change** — driven off the server's own commit path.
...
//! - **websocket** — a local stub: the server reports it is unsupported locally
//!   rather than pretending to serve it.
//!
//! # Cache-pathed I/O (§7.4)
//!
//! Each worker's client connects back to the server's own loopback endpoint, so
...
impl SysWatermarkStore {
    /// Wraps the server's system catalog handle.
...
/// fan-out run with. Held by the server for the process lifetime.
pub struct WorkerSupervisor {
    /// The private upstream catalog run logging commits through.
...
impl WorkerSupervisor {
    ...
    /// committing caller. Called off the server's commit path.
...
/// lifetime. It starts after recovery has opened the private catalog and never
/// blocks startup.
