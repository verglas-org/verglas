//! End-to-end TLS tests for the server's S3 endpoint (issue #11). The server is
...
//!
//! TLS termination happens at the server by design: production puts an L4 NLB
//! (TCP passthrough) in front, so the server is where plaintext first exists and
...
/// other server smoke tests make rather than parsing logs).
...
/// Spawns `verglas-server` with a TLS + base-domain config pointed at `origin_url`.
/// Returns the child, the `https://127.0.0.1:port` base URL, and the cert/key
/// file paths (so a test can rotate them).
/// Drains a spawned server's stderr on a background thread into a shared buffer.
/// The server logs one line per request (#61), so a test that piped stderr and
/// only read it at the end would fill the OS pipe and — with a blocking writer —
/// stall the server; draining continuously keeps the pipe empty. (The server's
...
struct StderrTap {
    ...
    /// The reader thread, joined when the server stops.
    reader: JoinHandle<()>,
}
...
fn spawn_tls_server(
    ...
    let scratch =
        std::env::temp_dir().join(format!("verglas-server-tls-{tag}-{}", std::process::id()));
    ...
    let tap = StderrTap::spawn(server.stderr.take().expect("piped stderr"));
    (server, format!("https://{addr}"), cert_path, key_path, tap)
}

/// A client that accepts the server's self-signed cert (verification off is the
...
/// Stops a spawned server, returning the stderr its tap drained.
fn stop_server(mut server: Child, tap: StderrTap) -> String {
    let _ = server.kill();
    let _ = server.wait();
    tap.finish()
}
...
async fn wait_for_ready(client: &reqwest::Client, base: &str, key: &str) {
    ...
    panic!("server TLS endpoint did not become ready at {base}");
}

#[tokio::test]
async fn server_serves_reads_over_tls_both_addressing_styles() {
    let body = pattern(50_000);
    let origin_url = spawn_origin(&[("warehouse/part-0.parquet", body.clone())]).await;
    let (server, base, _cert, _key, tap) = spawn_tls_server(&origin_url, "reads");
    ...
    let stderr = stop_server(server, tap);
    assert!(
...
async fn cert_rotation_under_load_drops_no_requests() {
    ...
    let origin_url = spawn_origin(&[("warehouse/part-0.parquet", body.clone())]).await;
    let (server, base, cert_path, key_path, tap) = spawn_tls_server(&origin_url, "rotate");
    let pid = server.id();
    let client = tls_client();
    wait_for_ready(&client, &base, "warehouse/part-0.parquet").await;
    ...
    // Midway through the flood, rotate the cert on disk and SIGHUP the server.
    ...
    assert!(
        ...
        "server must log the SIGHUP-driven cert reload; stderr: {stderr}"
    );
}
