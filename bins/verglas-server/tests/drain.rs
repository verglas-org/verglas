...
//! `verglas drain` performs against its local server, then observed to
//! rebalance.
//!
//! The server drain glue exercised here — the cluster agent gossiping itself
...
//! test drives it against real `verglas-server` processes and asserts the acceptance
...
/// A spawned server plus its admin endpoint and scratch dir (kept alive so it is
/// not deleted while the server runs).
struct Server {
    child: Child,
    node_id: String,
    endpoint: String,
...
impl Drop for Server {
    /// Kills and reaps the server when the handle drops (a no-op if it already
...
/// and spawns `verglas-server` for one pod node.
fn spawn_node(node_id: &str, gossip_port: u16, seed_port: u16, admin_port: u16) -> Server {
...
/// Fetches `GET /admin/members`, `None` until the server is up.
