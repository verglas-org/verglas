//! End-to-end proof of the #233 startup probe: a server configured with a real
...
//! opt-out — is covered by the other server integration tests, which point at an
...
/// A server pointed at a bucket whose origin refuses every connection must fail
...
fn unreachable_backend_fails_startup_with_a_clear_error() {
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            let _ = server.kill();
            panic!("server did not exit; it must fail startup on an unreachable backend");
        }
