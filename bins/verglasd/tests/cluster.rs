//! Integration tests for the daemon's cluster wiring (#27/#28).
//!
...
//! - a single-node daemon (no `[cluster]`) serves no membership route — the
//!   turn-off path is unchanged;
//! - a 3-daemon localhost pod with a shared seed converges so every daemon's
...
/// daemon's gossip/admin sockets. A small race window remains, acceptable for
/// tests.
fn free_port() -> u16 {
...
/// A spawned daemon plus its admin endpoint and scratch dir (kept alive so it
/// is not deleted while the daemon runs).
struct Daemon {
    child: Child,
    endpoint: String,
...
impl Drop for Daemon {
    /// Kills and reaps the daemon when the handle drops.
    fn drop(&mut self) {
...
/// Writes a `[cluster]` config and spawns `verglasd` for one pod node.
fn spawn_cluster_daemon(node_id: &str, gossip_port: u16, seed_port: u16) -> Daemon {
...
/// Fetches `GET /admin/members` from a daemon, returning `None` until it is up.
...
#[test]
fn single_node_daemon_omits_the_members_route() {
    let admin_addr = format!("127.0.0.1:{}", free_port());
    ...
    // is 200 (the daemon is up; it simply has no membership to report).
    ...
    assert_eq!(
        status.as_u16(),
        404,
        "single-node daemon must have no members route"
    );
...
/// Three daemons on localhost sharing one seed converge so every daemon's
/// `/admin/members` reports the same 3-member view.
#[test]
fn three_daemon_pod_converges_on_identical_members() {
    let seed_port = free_port();
    let gb = free_port();
    let gc = free_port();

    let a = spawn_cluster_daemon("node-a", seed_port, seed_port);
    let b = spawn_cluster_daemon("node-b", gb, seed_port);
    let c = spawn_cluster_daemon("node-c", gc, seed_port);
    let daemons = [&a, &b, &c];
    ...
    // Every daemon sees the same three node ids and the same pod.
    ...
    // Handles drop here, killing the daemons.
    let _ = (a, b, c);
}
