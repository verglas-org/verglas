//! End-to-end restart-recovery tests for the daemon (issue #16): warm the
//! cache through the real `verglasd` binary, `kill -9` it mid-life, restart it
...
//! verglas-s3's own front-end, a spawned daemon signing SigV4 against it, and
//! the client signing SigV4 against the daemon. The one difference that matters
//! here is the daemon's `cache.dir` is a fixed path passed in, so the second
//! daemon re-opens the first daemon's on-disk cache.
...
/// Signs a request against the daemon with the engine keypair.
...
/// A spawned daemon and the URLs the test drives it through.
struct Daemon {
    child: Child,
    s3_base: String,
    admin: String,
...
/// Spawns `verglasd` over a *fixed* `cache_dir` pointed at the mock origin, so a
/// later daemon over the same directory re-opens the first daemon's on-disk
...
fn spawn_daemon(origin_url: &str, cache_dir: &std::path::Path) -> Daemon {
    ...
    // restarted daemon over the same directory reads the same keypair.
...
async fn wait_until_ready(client: &reqwest::Client, admin: &str) {
    for _ in 0..100 {
            ...
            && response.status().is_success()
        {
            ...
            assert_eq!(info.status, "ok", "ready daemon must report ok");
            return;
        }
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
    panic!("daemon did not become ready at {admin}");
}

/// Reads the daemon's live read-path counters.
...
/// Signs and issues a full GET through the daemon, returning the body.
...
/// daemon, restart it over the same directory, and prove the previously cached
...
/// Health gating is proven too: the restarted daemon reports `ok` only once
...
async fn kill_dash_nine_restart_serves_from_nvme_with_zero_refills() {
    ...
    let scratch = std::env::temp_dir().join(format!("verglasd-restart-{}", std::process::id()));
    let cache_dir = scratch.join("cache");
    let client = reqwest::Client::new();
    let url = format!("http://HOST/{BUCKET}/{object}");

    // --- First daemon: warm the object onto the NVMe tier. ---
    let mut first = spawn_daemon(&origin_url, &cache_dir);
    ...
    first.child.kill().expect("SIGKILL the daemon");
    let _ = first.child.wait();

    // --- Second daemon over the SAME cache dir: recovery must serve the warm
    // cache. Fresh process ⇒ fresh counters, so post-restart fills start at 0. ---
    let mut second = spawn_daemon(&origin_url, &cache_dir);
    ...
    second.child.kill().expect("stop daemon");
    let _ = second.child.wait();
    let _ = std::fs::remove_dir_all(&scratch);
}

/// Metadata that remains in the DRAM front must still survive an unclean
/// daemon restart. The single object is smaller than the metadata DRAM budget,
...
async fn kill_dash_nine_restart_recovers_metadata_that_never_evicted_from_dram() {
    ...
    let scratch =
        std::env::temp_dir().join(format!("verglasd-meta-restart-{}", std::process::id()));
    let cache_dir = scratch.join("cache");
    let client = reqwest::Client::new();
    ...
    first.child.kill().expect("SIGKILL first daemon");
    let _ = first.child.wait();
    ...
    second.child.kill().expect("stop second daemon");
    let _ = second.child.wait();
