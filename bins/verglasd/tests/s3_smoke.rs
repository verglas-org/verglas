//! End-to-end smoke tests for the daemon's S3 endpoint: spawns the real
//! `verglasd` binary with a temp config, points its backend at a mock S3
...
//! the daemon with SigV4-signed HTTP. Covers reads (issue #6: bytes, headers,
...
/// Signs an HTTP request against the daemon with the engine keypair.
...
/// API tests: a tiny race window in exchange for not parsing daemon logs.
...
/// with `(key, bytes)` pairs. Used to prove the daemon serves a glob-matched
...
/// Spawns `verglasd` with a temp config whose `[backend]` body is `backend_toml`
...
fn spawn_daemon_with_backend(origin_url: &str, tag: &str, backend_toml: &str) -> (Child, String) {
    let scratch =
        std::env::temp_dir().join(format!("verglasd-s3-smoke-{tag}-{}", std::process::id()));
    ...
    (daemon, format!("http://{s3_addr}"))
}

/// Spawns `verglasd` with a temp config whose backend points at the mock
/// origin via the AWS env overrides the standard credential chain honors.
/// `tag` keeps each test's scratch dir distinct: the cache engine owns its
/// `cache.dir` exclusively, so two daemons must never share one.
fn spawn_daemon(origin_url: &str, tag: &str) -> (Child, String) {
    let scratch =
        std::env::temp_dir().join(format!("verglasd-s3-smoke-{tag}-{}", std::process::id()));
    let cache_dir = scratch.join("cache");
    std::fs::create_dir_all(&cache_dir).expect("create cache dir");
...
/// Stops a spawned daemon, returning its captured stderr.
fn stop_daemon(mut daemon: Child) -> String {
    let _ = daemon.kill();
    let output = daemon.wait_with_output().expect("reap daemon");
    String::from_utf8_lossy(&output.stderr).into_owned()
}

/// Polls the daemon's S3 endpoint until a signed GET succeeds.
async fn wait_for_ready(client: &reqwest::Client, url: &str) {
    ...
    panic!("daemon S3 endpoint did not become ready at {url}");
}

#[tokio::test]
async fn daemon_serves_reads_through_to_origin() {
    let body = pattern(100_000);
    let origin_url = spawn_origin(&[("warehouse/part-0.parquet", body.clone())]).await;
    let (daemon, base) = spawn_daemon(&origin_url, "reads");
    ...
    // rejected at the daemon with S3 NoSuchBucket (404), before the origin.
...
#[tokio::test]
async fn daemon_serves_a_glob_matched_bucket_and_rejects_non_matches() {
    // Bucket-set serving via a glob (#235): the daemon is configured with
    ...
    // A read for a bucket OUTSIDE the set is rejected at the daemon with
    ...
    let (daemon, base) =
        spawn_daemon_with_backend(&origin_url, "glob", "bucket_globs = [\"*--table-s3\"]");
    let client = reqwest::Client::new();
...
#[tokio::test]
async fn daemon_passes_writes_through_to_origin() {
    ...
    let (daemon, base) = spawn_daemon(&origin_url, "writes");
    let client = reqwest::Client::new();
    wait_for_ready(&client, &format!("{base}/{BUCKET}/seed.bin")).await;

    // PUT through the daemon: small (one atomic origin PUT) and large
    // (crosses the daemon's 8 MiB threshold, streaming to the origin as a
    ...
    for (key, size) in [
        ...
        // Readable back through the daemon...
        let response = client
            .get(&url)
            .headers(sign_request("GET", &url, &[]))
            .send()
            .await
            .expect("GET through daemon");
        assert_eq!(response.status(), 200);
    ...
    // DeleteObject through the daemon: gone from the origin, NoSuchKey back
    // through the daemon.
    ...
    // Client-driven multipart lifecycle through the daemon: the origin's
    // upload ID round-trips through daemon and client untouched.
    ...
    let response = client
        ...
        .send()
        .await
        .expect("CreateMultipartUpload through daemon");
    assert_eq!(response.status(), 200);
    ...
    for (number, part) in [(1, part1.clone()), (2, part2.clone())] {
        ...
        let response = client
            ...
            .send()
            .await
            .expect("UploadPart through daemon");
    ...
    let response = client
        ...
        .send()
        .await
        .expect("CompleteMultipartUpload through daemon");
    assert_eq!(response.status(), 200);
    ...
    // A bogus upload ID travels daemon -> origin 404 -> NoSuchUpload XML.
...
#[tokio::test]
async fn daemon_lists_through_to_origin_with_pagination_and_delimiter() {
    ...
    .await;
    let (daemon, base) = spawn_daemon(&origin_url, "list");
    let client = reqwest::Client::new();
    ...
    loop {
        ...
        let response = client
            ...
            .send()
            .await
            .expect("ListObjectsV2 through daemon");
        let status = response.status();
        let xml = response.text().await.expect("body");
        assert_eq!(
            status, 200,
            "LIST must be 200 through the daemon; body: {xml}"
        );
        for key in xml
            .split("<Key>")
            .skip(1)
    ...
    assert_eq!(
        ...
        "paginated LIST through daemon must return every key exactly once"
    );
    ...
    let response = client
        ...
        .send()
        .await
        .expect("delimited ListObjectsV2 through daemon");
    assert_eq!(response.status(), 200);
    let xml = response.text().await.expect("body");
    assert!(
