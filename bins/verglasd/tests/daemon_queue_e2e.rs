//! End-to-end proof that a real `verglasd` process serves the platform queue
//! routes (#328): spawn a throwaway daemon bound to a loopback port with a temp
//! cache dir, drive `enqueue`/`poll`/`ack` over HTTP, and tear the daemon down
//! by its captured PID only — no broad kill, no installed daemon touched.
//!
...
/// Spawns a throwaway daemon with a temp cache dir on an ephemeral admin port,
...
fn spawn_throwaway() -> (Child, String, tempfile::TempDir) {
    ...
    let daemon = Command::new(env!("CARGO_BIN_EXE_verglasd"))
        ...
        // Test opt-out: skip the origin probe so the daemon comes up without a
    ...
    (daemon, format!("http://{admin_addr}"), dir)
}
...
/// The daemon serves the queue routes end to end: enqueue three rows, poll them
/// for a group, ack, then poll again to see the watermark advanced.
#[test]
fn daemon_serves_queue_routes() {
    let (mut daemon, endpoint, _dir) = spawn_throwaway();
