//! Daemon-instance reporting: the privacy invariant and the register+heartbeat
//! wire behavior.
//!
//! The privacy invariant is release-blocking: a daemon with no [control_plane]
...
use verglasd::admin::StatsSlot;
use verglasd::node_report::{NodeIdentity, NodeReporter};
...
/// A daemon config from a minimal TOML, with an optional [control_plane] URL.
...
fn no_control_plane_config_means_no_reporter_and_no_network() {
    // The release-blocking privacy invariant: without [control_plane], the daemon
    ...
    assert!(
        ...
        "a daemon with no [control_plane] config must not build a reporter"
    );
}
