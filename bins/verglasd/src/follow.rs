...
//!
//! A follow runner writes through the daemon's own catalog, so the destination
//! follows the daemon's login state with no branch here: logged in, the lines
...
impl FollowManager {
    /// Builds a manager over the daemon's loopback catalog and system registry.
