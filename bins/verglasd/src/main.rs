//! `verglasd` — the Verglas cache daemon.
//!
...
use verglasd::{VERSION, admin, follow, logging, node_report, platform, shadow};

use std::sync::{Arc, OnceLock};

// Dependency edge reserved for the daemon's table-awareness wiring (M3).
...
/// The concrete cache engine the daemon runs: the hybrid cache over the
/// single-bucket passthrough backend, routing ownership through the live gossip
/// ring and the peer-fetch client (#29). A single-node daemon runs the
...
type DaemonEngine = HybridCacheEngine<PassthroughRead, PeerClient, LiveRing>;

/// The node id every single-node (no `[cluster]`) daemon uses. Matches the
/// cache engine's cluster-of-one id so turning gossip off leaves ownership
/// byte-identical to a pre-cluster daemon: one member owns every key.
...
/// The serving-path hooks the lifecycle pipeline hands back to the daemon: the
...
fn spawn_lifecycle(
    ...
    use verglas_tables::catalog::{CatalogFeed, RestCatalogSource, WatcherOptions, WsFeedConfig};
    ...
    if config.cache.warming.enabled {
        ...
        eprintln!(
            "verglasd {VERSION} eager metadata warming enabled (watching {})",
            catalog.uri
        );
    }
    if config.cache.prefetch.enabled {
        ...
        eprintln!(
            "verglasd {VERSION} snapshot-driven prefetch enabled (watching {})",
            catalog.uri
        );
    }
    Ok(hooks)
...
fn spawn_prefetch(
    ...
    // them, and commits made while the daemon was down are replayed and
...
fn load_config_from_args() -> Option<verglas_core::config::Config> {
    ...
    while let Some(arg) = args.next() {
        ...
        let Some(path) = args.next() else {
            eprintln!("verglasd: --config requires a path to a TOML config file");
            std::process::exit(1);
        };
        match verglas_core::config::Config::load(std::path::Path::new(&path)) {
            Ok(config) => return Some(config),
            Err(error) => {
                eprintln!("verglasd: {error}");
                std::process::exit(1);
            }
        }
    }
...
/// `verglasd --ports-file` (issue #194). When `verglas dev` (or a test) asks the
/// daemon to bind ephemeral ports (`:0`), it names a ports file the daemon
...
impl PortsReport {
    ...
    /// is logged and swallowed — it never fails the daemon, since serving does
            ...
            .open(&self.path)
        {
            Ok(mut file) => {
                if let Err(e) = file.write_all(line.as_bytes()) {
                    eprintln!("verglasd: could not write {role} port to ports file: {e}");
                }
            }
            Err(e) => eprintln!(
                "verglasd: could not open ports file `{}`: {e}",
                self.path.display()
            ),
        }
    }
...
/// when the daemon was launched without `--ports-file`.
...
/// path `verglas dev` named for the daemon to report its resolved ports to, or
/// `None` when the daemon binds fixed ports and needs no reporting.
...
/// Generates a dev access keypair for daemons started without `[auth]`. The
...
/// secret never lives in `config.toml`. With no `[auth]` the daemon generates
...
fn resolve_auth(config: &verglas_core::config::Config) -> Result<(String, String), String> {
    match &config.auth {
        ...
        None => {
            ...
            println!(
                "verglasd generated access keys: access_key_id={access_key_id} secret_access_key={secret_access_key}"
            );
...
/// it. The CLI reads the secret from the daemon's 0600 credentials file instead.
...
fn build_query_worker_dispatcher(
    ...
    if let Err(e) = std::fs::create_dir_all(&dir) {
        eprintln!(
            "verglasd {VERSION} could not create query worker dir {}: {e} — /v1/query stays embedded",
            dir.display()
        );
        return None;
    }

    // The same endpoint keypair the embedded path signs loopback requests
    // with, written to a mode-0600 AWS-INI file — never inlined into the
    // rendered TOML.
    ...
    if let Err(e) = std::fs::write(&credentials_path, credentials_text) {
        eprintln!(
            "verglasd {VERSION} could not write query worker credentials file: {e} — /v1/query stays embedded"
        );
        return None;
    }
    #[cfg(unix)]
    {
        ...
        {
            eprintln!(
                "verglasd {VERSION} could not restrict query worker credentials file permissions: {e}"
            );
        }
    }
    ...
    if let Err(e) = std::fs::write(&config_path, rendered) {
        eprintln!(
            "verglasd {VERSION} could not write query worker config: {e} — /v1/query stays embedded"
        );
        return None;
    }

    Some(Arc::new(
        verglasd::query_worker::QueryWorkerDispatcher::new(
...
/// Opens the in-process catalog handle the SDK table routes (`/v1/tables/...`)
/// commit and read through, over the daemon's own loopback gateway. Opening can
/// fail (an unreachable or misconfigured catalog); that leaves the routes
/// answering 503, like the other engine-dependent routes.
...
/// Drives the daemon's `/v1` serving router as a [`verglas_s3::ServingApi`], so
/// the SigV4-gated S3 data port answers the same `/v1/query` and
/// `/v1/tables/...` requests the loopback admin listener does. The Cloudflare
...
/// from the same [`admin::TablesSlot`] the admin listener uses, so both surfaces
/// observe the catalog handle the moment it is wired after recovery.
struct V1ServingApi {
    /// The state-erased `/v1` router (`tables` + `query`), shared-catalog-backed.
    router: axum::Router,
}
...
async fn serve_admin(
    ...
    eprintln!("verglasd {VERSION} admin API listening on http://{local_addr}");
...
async fn serve_s3(
    ...
    credentials: (String, String),
    serving: verglas_tables::lifecycle::heat::HeatFeed<DaemonEngine>,
    engine: DaemonEngine,
    ...
    W: verglas_core::write::ObjectWrite,
{
    ...
    // and — when `serving_api` is present — answers the daemon's `/v1` API on
...
async fn serve_s3_plain(
    ...
    eprintln!(
        "verglasd {VERSION} serving S3 on http://{local_addr} (backend bucket set: {bucket_set})"
    );
...
async fn serve_s3_tls(
    ...
    eprintln!(
        "verglasd {VERSION} serving S3 on https://{addr} (TLS terminated here; cert reloads on SIGHUP; backend bucket set: {bucket_set})"
    );
...
fn build_writeback_tier(
    ...
    WritebackTier<verglas_tables::lifecycle::heat::HeatFeed<DaemonEngine>, PassthroughWrite>,
    Box<dyn std::error::Error>,
> {
    let wb = &config.cache.writeback;
    let policy = Arc::new(build_writeback_policy(wb));
    let journals = Arc::new(verglas_writeback::JournalStore::open(&config.cache.dir)?);
    ...
    if single_node {
        eprintln!(
            "verglasd {VERSION} write-back tier enabled (single-node, ack_deadline={}ms) — opt-in per prefix; PUTs fast-ack from local durability (degenerate k=1 m=0 w=1) and propagate to the origin in the background; a commit awaits its data files' propagation via the barrier",
            wb.ack_deadline_ms
        );
    } else {
        eprintln!(
            "verglasd {VERSION} write-back tier enabled (k={} m={} w={}, ack_deadline={}ms) — opt-in per prefix, degrades to write-through below quorum (#180)",
...
/// either listener failing tears the daemon down (a half-alive daemon is worse
...
async fn serve(
    ...
    eprintln!(
        "verglasd {VERSION} backend resolved: {} (serving bucket set `{}`)",
        registry.describe(),
        registry.bucket_set()
    );
    ...
    if dev_allow_missing_origin() {
        eprintln!(
            "verglasd {VERSION} WARNING: VERGLAS_DEV_ALLOW_MISSING_ORIGIN set — skipping the backend startup probe; the origin is NOT verified (dev/test only)"
        );
    } else {
        registry.probe().await?;
        eprintln!("verglasd {VERSION} backend startup probe passed");
    }
    ...
    // Daemon-instance tracking: report this node to the tenant's control plane
    ...
    // returns None — no task, no network — so a self-hosted daemon never phones
    // home.
    ...
    // unfilled slot means no metering (older-daemon-compatible), and no
    ...
    {
        eprintln!(
            "verglasd {VERSION} control-plane node reporting enabled (register + 5m heartbeat)"
        );
    ...
    // graph-over-Iceberg engine through the same loopback catalog as the table
    ...
    // streaming Vamana engine over the same loopback catalog, persisting index
    // blobs to the cluster-local shadow store under the cache dir — never to the
    // source table (#95 / #91 divergence). Filled after recovery.
    let vector_slot: Option<admin::VectorSlot> =
        config.catalog.is_some().then(|| Arc::new(OnceLock::new()));
    // The `verglas_sys` registry and watermark routes (#322) write and read
    // through a `SystemCatalog` over the same loopback catalog — the daemon is
    ...
    // as the admin listener binds. Owned under the cache dir the daemon already
    ...
    if let Err(e) = std::fs::create_dir_all(queue_dir.as_ref()) {
        eprintln!(
            "verglasd {VERSION} could not create the platform queue dir {}: {e}",
            queue_dir.display()
        );
    }
    // The worker runtime runs workers over the loopback catalog, so — like the
    ...
    // `VERGLAS_S3_ADDR`/`VERGLAS_ADMIN_ADDR = 127.0.0.1:0`, the daemon binds the
    ...
    // endpoint (the port just resolved), the loopback catalog mount,
    // region/bucket, and the endpoint access key id. The paired secret is never
    // put on this snapshot — the CLI reads it from the local 0600 creds file.
    let access = Some(build_local_access(config, &credentials.0, resolved_s3_port));
    ...
    // at this daemon's own just-resolved loopback surface with the same
    // signing keypair the embedded path uses. `None` (the common case) when
    // `[query_worker]` is unset or the files could not be rendered — then
    ...
    let data_plane = async {
        ...
        eprintln!(
            "verglasd {VERSION} cache instance `{}` ready — commit seam: {} (ring member; the fleet plugs a PG quorum + clustered ring in behind the CommitLog/RingMembership traits)",
        ...
        eprintln!(
            "verglasd {VERSION} cache recovery complete in {recovery_elapsed:?} — serving was gated on it (#16)"
        );
        ...
        // before a cold backend fill. A single-node daemon has no pod, so it
            ...
            && cluster.warm_from_peers_secs > 0
        {
            ...
            eprintln!(
                "verglasd {VERSION} warm-from-peers window open for {}s — owned cold misses pull from their previous holder before the backend (#30)",
        ...
        // a circular startup wait on every catalog-backed daemon.
        ...
        // When a catalog is configured, the SigV4-gated S3 port also serves the
        // daemon's `/v1` API (query and tables), driving a router built from the
        // same catalog slot the loopback admin listener uses. The edge re-signs
        // cache-pathed `/v1` forwards with the cache keypair; they land there.
        let s3_serving_api: Option<Arc<dyn verglas_s3::ServingApi>> =
            tables_slot.clone().map(|tables| {
                Arc::new(V1ServingApi {
                    router: admin::v1_serving_router(tables, query_worker_dispatcher.clone()),
        ...
        // The SDK table routes read and commit through the loopback catalog too.
            ...
            match build_tables_catalog(config, &credentials, resolved_s3_port).await {
                Ok(catalog) => {
                    ...
                    if let Some(platform) = &platform_slot {
                        ...
                        // lakehouse when this daemon is logged in).
                        let follow_manager =
                            Arc::new(follow::FollowManager::new(catalog.clone(), sys_catalog));
                        follow::spawn_follow_manager(follow_manager);
                    }
                    // The shared vector-index service (declare/search/refresh over
                    // the cache-managed shadow store), built once for the
                    // vector-index routes. Present only when the vector routes are
                    // configured.
                    let vector_service = vector_slot.as_ref().map(|_| {
                        let cache_shadow = Arc::new(
                            verglas_cache::shadow::ShadowStore::under_cache_dir(
                                &config.cache.dir,
                                config.cache.shadow_capacity_bytes.0,
                            )
                            .expect("open shadow store under cache dir"),
                        );
                        let store = Arc::new(shadow::CacheShadowStore::new(cache_shadow));
                        Arc::new(verglas_vector::service::VectorService::new(store))
                    });
                    // The graph routes drive the graph engine through the same
                    // loopback catalog — one write authority for the nodes and
                    // edges tables, exactly as for any other table.
                    if let Some(graphs) = &graphs_slot {
                        let _ = graphs.set(catalog.clone());
                    }
                    // The vector-index routes run the streaming Vamana engine
                    // over the same loopback catalog, persisting blobs to the
                    // cache-managed shadow store (#95) — NVMe-resident under the
                    // cache dir, budget-bounded, and Verglas-owned. The shadow
                    // store is deliberately NOT the source table: a derived,
                    // serving artifact (#90) the standing invariant keeps out of
                    // customer tables. The vector engine sees only its
                    // `ShadowBlobStore` trait; `shadow::CacheShadowStore` is the
                    // daemon-side adapter onto `verglas_cache`, so neither engine
                    // crate depends on the other.
                    if let (Some(vector), Some(service)) = (&vector_slot, vector_service) {
                        // The durable index registry is the source of truth; the
                        // service's in-memory registry is its projection. The
                        // cluster id (hostname by default) scopes which rows this
                        // daemon rehydrates and stamps on the rows it writes.
                        let runtime = Arc::new(admin::VectorRuntime {
                            catalog: catalog.clone(),
                            service,
                            sys: Arc::new(verglas_platform::SystemCatalog::new(catalog.clone())),
                            cluster_id: config.resolve_cluster_id(),
                        });
                        // Rehydrate this cluster's declared indexes off the
                        // serving path: load present blobs, rebuild missing ones.
                        // Detached so it never blocks recovery; a present blob
                        // still serves lazily meanwhile.
                        let rehydrate = runtime.clone();
                        tokio::spawn(async move {
                            admin::rehydrate_indexes(&rehydrate).await;
                        });
                ...
                Err(e) => eprintln!(
                    "verglasd {VERSION} table API unavailable: {e} — /v1/tables answers 503 until the catalog is reachable"
                ),
            }
        }
        health.mark_ready();
...
/// `local_block` endpoint. Returns `None` for a single-node daemon or a
...
async fn start_peer_server(
    ...
    eprintln!(
        "verglasd {VERSION} serving peer fetch on http://{} (cache-only; owners serve owned blocks)",
        server.local_addr()
    );
    Ok(Some(server))
}
...
fn peer_source(engine: DaemonEngine) -> LocalBlockFn {
    Arc::new(move |block| {
...
/// membership over the daemon's live ring, and its transaction seam.
///
...
/// the turn-off path, ownership identical to a pre-cluster daemon.
async fn build_ring(
    config: &verglas_core::config::Config,
...
> {
    match &config.cluster {
        Some(cluster) => {
            ...
            eprintln!(
                "verglasd {VERSION} joining pod `{}` as `{}` — gossip on {} (weight {} bytes, {} seed(s))",
                agent_config.pod_id,
...
fn drain_source(agent: Arc<ClusterAgent>, default_timeout: u64) -> admin::DrainHandler {
    ...
    Arc::new(move |request| {
        ...
        Box::pin(async move {
            ...
            eprintln!(
                "verglasd {VERSION} node `{node_id}` draining — serving as a donor for up to {timeout_secs}s, then exiting so the ring rebalances (#31)"
            );
            tokio::spawn(async move {
                tokio::time::sleep(std::time::Duration::from_secs(timeout_secs)).await;
                eprintln!(
                    "verglasd {VERSION} drain window elapsed ({timeout_secs}s) — exiting cleanly; gossip announces the departure and the ring rebalances (#31)"
                );
                std::process::exit(0);
            });
            DrainAck {
...
/// Env var the `verglas dev` parent sets to its own pid, arming this daemon's
...
/// orphaned daemon can hold its port after the parent dies; small enough that a
...
/// Arms the parent-death watch when `verglas dev` launched this daemon (issue
...
/// and exits the daemon the moment it diverges from the recorded parent pid,
...
///
/// The watch arms ONLY when the env var is present, so a production daemon under
...
fn spawn_parent_death_watch() {
    ...
    let Ok(parent_pid) = raw.parse::<i32>() else {
        eprintln!("verglasd: ignoring unparseable {PARENT_PID_ENV}={raw:?}");
        return;
    };
        ...
        .spawn(move || {
            loop {
                ...
                if parent_is_gone(parent_pid, ppid) {
                    eprintln!(
                        "verglasd: parent {parent_pid} exited (reparented to {ppid}); shutting down so its port is not left squatted by an orphan (#170)"
                    );
                    std::process::exit(0);
                }
            }
...
async fn main() {
    ...
    let result = match &config {
        Some(config) => {
            println!("verglasd {VERSION} config ok: {}", config.summary());
    ...
    if let Err(error) = result {
        eprintln!("verglasd failed: {error}");
        std::process::exit(1);
    }
}
...
mod tests {
    ...
    async fn admin_stats_reports_the_configured_budgets_in_process() {
        ...
        // Build the engine over the same ring path the daemon uses (a
    ...
    /// access key id: the loopback S3 endpoint, the catalog mount only when a
    /// catalog is watched, the backend region and bucket, and the access key id.
...
[catalog]\nuri = \"http://127.0.0.1:8181\"\nwarehouse = \"s3://warehouse/tenant\"\n";
        ...
        // With no catalog, the mount is absent (the gateway route is too).
        let toml_no_catalog = "\
[listen]\ns3_port = 8333\nadmin_port = 8334\n\
[cache]\ndir = \"/tmp/vg\"\ncapacity_bytes = \"64MB\"\n\
[backend]\nbucket = \"b\"\n";
    ...
    /// a changed ppid means the parent died and this daemon must exit rather
    /// than squat its port.
    #[test]
    ...
    /// daemon never fabricates zeros when there is no engine to report on.
    #[tokio::test]
