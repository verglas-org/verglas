//! The daemon's `tracing` subscriber (#61, building on #241/#247).
//!
...
//! queue sheds load instead of the daemon stalling.
...
/// falls back to `info` so a typo never silences the daemon (#241).
...
/// defaults when the daemon runs config-less). Idempotent by way of
...
pub fn install(format: LogFormat, level: &str) {
    ...
    if let Err(error) = installed {
        eprintln!("verglasd: failed to install tracing subscriber: {error}");
    }
}
