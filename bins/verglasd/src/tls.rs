...
//!
//! Production puts an L4 NLB (TCP passthrough) in front of the daemons, so TLS
//! terminates here, at the daemon — the NLB never sees plaintext and SigV4's
...
/// means restarting the daemon. Everything else about serving TLS is unchanged.
#[cfg(unix)]
pub fn spawn_reload_on_sighup(config: RustlsConfig, cert_path: PathBuf, key_path: PathBuf) {
    tokio::spawn(async move {
        let mut hangup = match tokio::signal::unix::signal(tokio::signal::unix::SignalKind::hangup())
        {
            ...
            Err(error) => {
                eprintln!(
                    "verglasd {VERSION} could not install the SIGHUP handler for TLS reload: {error}"
                );
                return;
            }
        };
        while hangup.recv().await.is_some() {
            match config.reload_from_pem_file(&cert_path, &key_path).await {
                Ok(()) => eprintln!(
                    "verglasd {VERSION} reloaded TLS certificate from {} on SIGHUP (live connections kept)",
                    cert_path.display()
                ),
                Err(error) => eprintln!(
                    "verglasd {VERSION} TLS certificate reload failed ({error}); keeping the current certificate"
                ),
            }
        }
    });
...
/// daemon still serves TLS with the certificate loaded at startup; rotating it
/// means restarting the daemon. Logged once so the operator knows the reload
/// signal is inert here.
#[cfg(not(unix))]
pub fn spawn_reload_on_sighup(_config: RustlsConfig, cert_path: PathBuf, _key_path: PathBuf) {
    eprintln!(
        "verglasd {VERSION} live TLS certificate reload (SIGHUP) is not available on this platform; \
         restart the daemon to load a new certificate ({})",
        cert_path.display()
    );
}
...
mod tests {
    ...
    /// call (a provider already installed) is ignored, never a panic. The daemon
