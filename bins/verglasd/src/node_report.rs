//! Daemon-instance reporting to the control plane (issue: node tracking).
//!
//! A logged-in daemon registers itself with the tenant's control plane and
...
//! identified by [`Config::resolve_cluster_id`] — the daemon's existing cluster
//! id (env > `[cluster].id` > hostname > `"localhost"`), never a second identity
//! and never an API key.
//!
//! PRIVACY INVARIANT (release-blocking): a daemon with NO `[control_plane]`
...
/// metering, which is exactly what an older daemon or a pre-recovery daemon
...
/// build/platform facts. Reuses the daemon's own cluster-id resolution.
#[derive(Clone, Debug)]
...
impl NodeIdentity {
    /// Resolves this daemon's identity from its config. `node_id` is exactly
...
/// Reports this daemon to the control plane: one register on boot, then a
...
impl NodeReporter {
    ...
    /// token file exists and is non-empty. `stats` is the daemon's stats slot,
    ...
    pub fn from_config(config: &Config, stats: Option<StatsSlot>) -> Option<NodeReporter> {
        ...
        // the token file — a self-hosted daemon does nothing here.
    ...
    /// attached, heartbeats carry no metering — an older-daemon-compatible body
    ...
    /// Snapshots the daemon's live stats into the flat metrics object the
...
/// carries. Computed entirely from counters the daemon already tracks (what
