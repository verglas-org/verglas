...
//! with a server. Every command targets one of two backends, resolved here:
//!
//! - a **server endpoint** — the global `--server-endpoint` / `VERGLAS_ENDPOINT`
//!   value (default `http://127.0.0.1:8334`). It may be localhost OR a remote
//!   server: a cloud node, or another host's server (the case a `data_sidecar`
//!   box points at so it can declare workers without running its own server).
//!   The data-plane verbs (`table`, `query`, `graph`) and the registry verbs
//!   (`workers`, `index`, `tables`) all target it through [`server`]. WHICH
//!   server owns a declaration is a property of the endpoint, not of where the
//!   CLI runs — a remote endpoint means the remote server owns and executes it.
//!
...
//! install, run, or drain the server on THIS machine (the CLI is local-only for
//! cluster/service ops). They are about DEPLOYING a server, not about using the
//! platform — running a local/edge server is for cache and read latency, never
//! a requirement.
//!
