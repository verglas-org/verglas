//! The CLI data-plane verbs speak the daemon HTTP API and nothing else (#323).
//!
//! These stand up a local mock of the daemon's admin API (an axum server bound
...
//! the CLI, no daemon-less fallback. The daemon-down test proves the other
...
/// What the mock daemon records: how many ingest bytes arrived, so the test
...
/// Builds the mock daemon router: the table, query, and graph routes the CLI
/// verbs are expected to call.
fn mock_daemon(state: MockState) -> Router {
    Router::new()
        .route(
            "/v1/tables",
...
/// Binds the mock daemon on an ephemeral port and serves it from a background
...
/// Every table verb speaks the daemon routes: list/show/history read the
/// inspect routes, create streams the source file to the ingest route, and
/// query posts to /v1/query — all rendered from the daemon's JSON.
#[test]
fn table_and_query_verbs_speak_the_daemon_api() {
    ...
    assert_eq!(
        ...
        "id\n1\n2\n".len(),
        "the CLI streamed the file body to the daemon"
    );
...
/// The registry verbs speak the daemon registry routes: register posts the
/// The graph verbs speak the daemon graph routes: create/show on the namespace
...
#[test]
fn graph_verbs_speak_the_daemon_api() {
    let state = MockState::default();
    let endpoint = spawn_mock(state.clone());
...
/// With no daemon listening, every data-plane verb fails fast with a clear
/// error that names the endpoint and how to start a daemon — and there is no
/// fallback path that could touch a backend.
#[test]
fn daemon_down_is_a_clear_error_with_no_fallback() {
    ...
    for args in [
        ...
        assert!(!status.success(), "{args:?} must fail without a daemon");
        assert!(
            stderr.contains("127.0.0.1:9"),
            "{args:?} error names the endpoint: {stderr}"
        );
        assert!(
            stderr.contains("verglas dev") || stderr.contains("daemon"),
