//! SQL over this node's own Iceberg catalog: `POST /v1/query`.
//!
//! Mounted on the admin listener, and only when the catalog runs — the query
//! reads through this node's catalog gateway, so with no catalog there is
//! nothing to query. No authentication guards this surface in v1:
//! org-network-only, the same trust model the rest of the admin port uses. A
//! request runs to completion, times out, or is refused with a clean JSON
//! error; nothing panics and nothing answers a raw 5xx for a caller-fixable
...
pub type TokenFactory = Arc<dyn Fn() -> Result<Option<String>, String> + Send + Sync>;

/// How a request reaches the catalog.
///
/// The hosted catalog listens in this same process and starts *after* the
/// admin router is built, so its client cannot be opened at startup — the
/// listener is not up yet. `Deferred` opens one per query instead, which also
/// keeps the node's self-issued credential inside its short TTL: a client
/// cached for the process lifetime would carry an expired bearer within
/// minutes. `run_query` already rebuilds its DataFusion session per request
/// for a separate reason, so this adds no round trip that was not there.
#[derive(Clone)]
enum CatalogHandle {
    /// An already-open catalog, injected by tests.
    #[cfg(test)]
    Open(Arc<dyn Catalog>),
    /// Coordinates to open per query, with a freshly minted credential.
    Deferred {
        connection: Arc<verglas_iceberg::Connection>,
        token: TokenFactory,
    },
}

impl CatalogHandle {
    /// Opens the catalog client this query runs against.
    async fn resolve(&self) -> Result<Arc<dyn Catalog>, AgentError> {
        match self {
            #[cfg(test)]
            CatalogHandle::Open(catalog) => Ok(Arc::clone(catalog)),
            CatalogHandle::Deferred { connection, token } => {
                let mut connection = connection.as_ref().clone();
                connection.token = token().map_err(AgentError::Query)?;
                verglas_iceberg::catalog::open_catalog(&connection)
                    .await
                    .map_err(|error| {
                        AgentError::Query(format!(
                            "catalog at {} is unavailable: {error}",
...
/// The DataFusion memory ceiling when `VERGLAS_QUERY_MEMORY_LIMIT_BYTES` is
/// unset.
...
impl QueryState {
    /// Builds query state over an already-open catalog client handle, reading
    /// its two limits from the environment.
    ///
    /// A malformed limit is a startup error rather than a silent default: an
    /// operator who set the value meant it, and quietly ignoring it would hide
    /// the mistake until a query behaved unexpectedly.
    pub fn from_env(
        ...
        let memory_limit_bytes = match std::env::var("VERGLAS_QUERY_MEMORY_LIMIT_BYTES") {
            Ok(raw) => raw.trim().parse().map_err(|_| {
                format!("VERGLAS_QUERY_MEMORY_LIMIT_BYTES is not a byte count: {raw}")
            })?,
            Err(_) => DEFAULT_MEMORY_LIMIT_BYTES,
        };
        let timeout_secs = match std::env::var("VERGLAS_QUERY_TIMEOUT_SECS") {
            Ok(raw) => raw
                .trim()
                .parse()
                .map_err(|_| format!("VERGLAS_QUERY_TIMEOUT_SECS is not a second count: {raw}"))?,
            Err(_) => DEFAULT_QUERY_TIMEOUT_SECS,
        };
        Ok(Self {
            catalog: CatalogHandle::Deferred {
    ...
    /// Builds query state over an already-open catalog, bypassing the
    /// environment.
    #[cfg(test)]
    pub fn new(
...
async fn handle_query(State(state): State<QueryState>, body: Bytes) -> Response {
    ...
    // Resolving the catalog is inside the timeout: an unreachable catalog must
    // answer within the same budget a slow query gets, never hang the caller.
    let outcome = tokio::time::timeout(state.query_timeout, async {
        let catalog = state.catalog.resolve().await?;
        run_query(catalog, state.memory_limit_bytes, &sql).await
    })
    .await;
    match outcome {
...
mod tests {
    ...
    async fn count_query_answers_the_v0_result_shape() {
        ...
        let state = QueryState::new(catalog.clone(), 64 * 1024 * 1024, Duration::from_secs(5));
    ...
    /// caching a snapshot from before the table existed (see `QueryState`'s
    ...
    async fn absent_engine_field_runs_on_datafusion() {
        ...
        let state = QueryState::new(catalog, 64 * 1024 * 1024, Duration::from_secs(5));
        let addr = serve(router(state)).await;
    ...
    async fn an_over_cap_memory_request_is_refused() {
        let catalog = memory_catalog().await;
        let state = QueryState::new(catalog.clone(), 1024, Duration::from_secs(5));
        let addr = serve(router(state)).await;
    ...
    async fn a_query_past_the_timeout_answers_408() {
        ...
        let state = QueryState::new(catalog.clone(), 64 * 1024 * 1024, Duration::from_millis(20));
        let addr = serve(router(state)).await;
