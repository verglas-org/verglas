...
//! `bins/cache-node/src/logging.rs`): same output shapes (`json` for pipelines,
...
/// Env var overriding the configured format, matching verglas-cache-node/cache-node.
...
/// level. An unparseable value in either falls back to `info`.
...
pub fn install(format: LogFormat, level: &str) {
    ...
    if let Err(error) = installed {
        eprintln!("verglas-query: failed to install tracing subscriber: {error}");
    }
}
...
mod tests {
    ...
    /// `VERGLAS_LOG_FORMAT` overrides the configured format both ways; an
    /// unset or unrecognised value keeps the configured one.
    #[test]
