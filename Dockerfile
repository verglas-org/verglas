...
RUN --mount=type=cache,id=verglas-cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    ...
    && cp /src/target/release/verglas-ec-keeper /tmp/verglas-build/ \
...
USER verglas
EXPOSE 8333 8334 8335 8336
ENTRYPOINT ["verglas-cache-node-start"]

FROM runtime AS verglas-ec-keeper
RUN apt-get update \
    && apt-get install -y --no-install-recommends netcat-openbsd \
    && rm -rf /var/lib/apt/lists/* \
    && mkdir -p /var/lib/verglas-ec-keeper \
    && chown -R verglas:verglas /var/lib/verglas-ec-keeper
COPY --from=build /tmp/verglas-build/verglas-ec-keeper /usr/local/bin/verglas-ec-keeper
COPY deploy/ec-keeper/start.sh /usr/local/bin/verglas-ec-keeper-start
RUN chmod 0755 /usr/local/bin/verglas-ec-keeper-start
USER verglas
EXPOSE 5454
ENTRYPOINT ["verglas-ec-keeper-start"]
