...
test("listLakehouse discovers resources and uses only database-scoped catalogs", async () => {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (url === "http://access/v1/access/authorize")
      return Response.json({ allowed: true });
    if (url === "http://access/v1/databases")
    ...
    if (
      url ===
      "http://data/v1/databases/analytics/catalog/v1/namespaces/events/tables"
    ) {
      return Response.json({
  ...
  try {
    ...
    assert.deepEqual(
      result.databases.map(({ name, type }) => ({ name, type })),
      [
    ...
    assert.ok(
      calls.every(
        ([url]) =>
          url.startsWith("http://access") || url.startsWith("http://data"),
      ),
    );
    assert.ok(
      calls.every(
        ([, , , headers]) => headers.Authorization === "Bearer scoped-token",
      ),
    );
    assert.deepEqual(
      calls
        .filter(([url]) => url === "http://access/v1/access/authorize")
...
test("queryLakehouse sends SQL only to the selected database runtime", async () => {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (url === "http://access/v1/access/authorize")
      return Response.json({ allowed: true });
    if (url === "http://access/v1/databases/analytics")
      return Response.json({
        type: "lakehouse",
        name: "analytics",
        storage: { mode: "managed" },
  ...
  try {
    ...
    const result = await execute("queryLakehouse", {
      database: "analytics",
      sql: "SELECT 1 AS id",
    });
