...
export const toolDefinitions = [
  ...
  },
  {
    ...
    function: {
      name: "listLakehouse",
      description:
        "List bounded Verglas databases and each Lakehouse's scoped tables and graphs.",
      parameters: schema({}, []),
    },
  },
  {
    ...
    function: {
      name: "queryLakehouse",
      description:
        "Run bounded SQL against one named Verglas Lakehouse database.",
      parameters: schema({
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Build and start a standalone Application Vessel from a complete TypeScript project.",
      parameters: schema({
        name: { type: "string" },
        title: { type: "string" },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Build and start a standalone Integration Vessel API from a complete TypeScript project.",
      parameters: schema(
        {
          name: { type: "string" },
          files: { type: "object", additionalProperties: { type: "string" } },
  ...
  },
  {
    ...
    function: {
      name: "deployJob",
      description:
        "Register a Verglas TypeScript worker Job with triggers and an output table.",
      parameters: schema(
        {
          name: { type: "string" },
          code: { type: "string" },
...
async function request(base, token, path, options = {}) {
  ...
  if (!response.ok)
    throw new Error(
      `Verglas ${path} failed: HTTP ${response.status} — ${text}`,
    );
...
export function createToolExecutor(env, emit) {
  ...
  if (!admin || !runtime || !access || !scopedToken || !principalId) {
    throw new Error(
      "Agent runtime is missing Verglas data or container-runtime configuration.",
    );
  }
  ...
  return async (name, args) => {
    switch (name) {
      case "requestPermission": {
        ...
        const actions = [
          ...new Set(Array.isArray(args.actions) ? args.actions : []),
        ];
        const hasControlCharacter = [...resourceId].some(
          (character) =>
      ...
      case "listLakehouse": {
        ...
        const databaseBody = await request(
          access,
          scopedToken,
          "/v1/databases",
        );
        ...
        for (const database of databases) {
          ...
          const namespaceBody = await request(
            admin,
            scopedToken,
            `${catalog}/namespaces`,
          );
          for (const namespace of (namespaceBody.namespaces ?? []).slice(
            0,
            100,
          )) {
            ...
            const listed = await request(
              admin,
              scopedToken,
              `${catalog}/namespaces/${encoded}/tables`,
            );
