...
describe("access presentation", () => {
    ...
    const tree = buildResourceTree([
      {
        ...
        id: "namespace:a",
        kind: "namespace",
        parentId: "namespace:b",
      },
      {
        tenantId: "acme",
        id: "namespace:b",
        kind: "namespace",
        parentId: "namespace:a",
      },
    ]);

    expect(tree.map(({ resource }) => resource.id)).toEqual([
      "namespace:a",
      "namespace:b",
    ]);
  });
  ...
  it("requires a name, future expiration, resource, and action before creating a token", () => {
      ...
      isTokenRequestComplete(
        {
          name: "",
          expiresAt: future,
          resourceIds: ["database:analytics"],
          actions: ["query"],
        },
        now,
      ),
    ...
    expect(
      isTokenRequestComplete(
        { name: "cli", expiresAt: future, resourceIds: [], actions: ["query"] },
        now,
      ),
    ).toBe(false);
    expect(
  ...
  it("formats action identifiers for UI labels", () => {
    expect(formatAccessAction("create_child")).toBe(
      "Create databases or tables",
    );
    expect(describeAccessAction("connect")).toBe(
      "Open a database or service connection. This does not grant data access by itself.",
    );
    expect(describeAccessAction("use_secret")).toBe(
      "Use a saved credential without revealing its value.",
    );
  });
  ...
  it("applies selected actions to every selected resource when building a token request", () => {
    expect(
      toTokenGrants(
        ["database:analytics", "database:operations"],
        ["discover", "query"],
      ),
    ).toEqual([
    ...
    const tokens = [
      {
        id: "session-record",
        name: "OS session",
        principalId: "session/browser",
        parentPrincipalId: "user/dev@example.com",
        audience: "access",
        createdAt: 1,
        expiresAt: 2,
      },
