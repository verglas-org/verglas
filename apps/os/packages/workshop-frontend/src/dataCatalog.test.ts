...
describe("dynamic database catalog presentation", () => {
  it("filters catalog assets by their top-level database resource", () => {
    expect(databaseAssets(catalog, "analytics")).toMatchObject({
      tables: [{ name: "log" }, { name: "nodes" }],
      graphs: [{ namespace: "knowledge" }],
      vectors: [],
    });
    expect(databaseAssets(catalog, "operations")).toEqual({
      tables: [],
      vectors: [],
      graphs: [],
    });
  ...
  it("groups Lakehouse tables into namespaces without promoting namespaces to databases", () => {
    expect(
      namespaceGroups(databaseAssets(catalog, "analytics").tables),
    ).toEqual([
      {
        name: "events",
        namespace: ["events"],
        tables: [expect.objectContaining({ name: "log" })],
      },
      {
        name: "knowledge",
        namespace: ["knowledge"],
        tables: [expect.objectContaining({ name: "nodes" })],
      },
    ]);
  });
