...
const RUNTIMES: RuntimeBrand[] = [
  {
    ...
    accent: "#10a37f",
    description:
      "Use Codex with your ChatGPT subscription or an OpenAI API key.",
  },
  {
    id: "claude-code",
    name: "Claude",
    ...
    accent: "#d97757",
    description:
      "Use Pi with your Claude Pro or Max subscription, or an Anthropic API key.",
  },
  {
    id: "github-copilot",
    name: "GitHub Copilot",
    ...
    accent: "#24292f",
    description: "Use Pi with your GitHub Copilot subscription.",
  },
];
...
const PROVIDER_LABELS: Record<
  Exclude<AiModelProvider, "local-runtime">,
  string
> = {
...
export default function AddModelModal({
  ...
  const [directProvider, setDirectProvider] =
    useState<Exclude<AiModelProvider, "local-runtime">>("openai");
  ...
  const linkSubscription = async () => {
    ...
    try {
      ...
      const result = await authenticatedApi.startModelRuntimeLogin(
        runtime.id,
        sessionId,
      );
      setLogin(result);
  ...
  const advanceWizard = async () => {
    ...
    try {
      ...
      const needsAnswer = ["text", "select", "confirm", "multiselect"].includes(
        step.type,
      );
  ...
  const back = async () => {
    if (view === "wizard" && login && !login.done) {
      try {
        await authenticatedApi.cancelModelRuntimeLogin(login.sessionId);
      } catch {
        /* best effort */
      }
    }
          ...
          <div className="space-y-5">
            <div>
              <p className="mb-3 text-sm text-kumo-subtle">
                Sign in with an existing coding-agent subscription or connect
                with an API key.
              </p>
              <div className="grid gap-3 sm:grid-cols-3">
                {RUNTIMES.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-3 text-sm font-semibold text-kumo-default">
                      Link {item.name}
                    </div>
                    <div className="mt-1 text-xs leading-4 text-kumo-subtle">
                      {item.description}
                    </div>
                  </button>
                ))}
          ...
          <div className="space-y-4">
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-0.5 text-xs text-kumo-subtle">
                    Sign in with your {runtime.accountName}
                  </div>
                </div>
              </button>
            )}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="mt-0.5 text-xs text-kumo-subtle">
                  Connect with {runtime.tokenName}
                </div>
              </div>
            </button>
          </div>
        ...
        {view === "token" && runtime && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold text-kumo-default">
                  Use {runtime.tokenName}
                </div>
                <div className="text-sm text-kumo-subtle">
                  The token is stored with your Workshop model.
                </div>
              </div>
            </div>
            <SensitiveInput
        ...
        {view === "direct" && (
          <div className="space-y-4">
            <Select
              ...
              onValueChange={(value) =>
                setDirectProvider(value as typeof directProvider)
              }
            >
            ...
            <Input
              label="Model ID"
              placeholder={
                Object.keys(SUGGESTED_MODELS[directProvider])[0] || "model-id"
              }
              value={directModel}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="font-semibold text-kumo-default">
            {step.title || `Sign in to ${runtime.name}`}
          </div>
          {step.message && (
            <div className="mt-0.5 text-sm text-kumo-subtle">
              {step.message}
      ...
      {step.type === "select" && step.options && (
        <Select
          label={step.title || "Choose an option"}
          value={String(value ?? "")}
