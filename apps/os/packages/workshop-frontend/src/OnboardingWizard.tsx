...
export default function OnboardingWizard({
  ...
  const [connectedVendorIds, setConnectedVendorIds] = useState<Set<string>>(
    new Set(),
  );
  const [vendorsLoading, setVendorsLoading] = useState(true);
  const [connectingVendorId, setConnectingVendorId] = useState<string | null>(
    null,
  );
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      setSelectedModelId((current) => {
        ...
        if (
          preferredModelId &&
          modelList.some((model) => model.id === preferredModelId)
        ) {
  ...
  const handleFinish = async () => {
    if (
      ...
      setStep(1);
      toasts.add({
        title: "Choose an AI model before finishing setup.",
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (useAsQuickModel)
        await authenticatedApi.setQuickModel(selectedModelId);
  ...
  const modelStatus = onboardingModelStatus(
    models,
    selectedModelId,
    modelsLoading,
    modelsLoadError,
      ...
      <div className="fixed inset-0 bg-kumo-base dotted-bg flex items-center justify-center overflow-y-auto py-8">
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            >
              Connect information, run governed workflows, and deliver
          ...
          <div className="overflow-hidden rounded-2xl border border-kumo-line bg-kumo-elevated shadow-xl shadow-black/[0.04]">
            ...
            >
                    ...
                    >
                      {avatarPreview ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full">
                          <Camera
                            size={22}
                            className="text-kumo-inactive group-hover:text-kumo-subtle transition-colors"
                          />
                        </div>
                      )}
                      {avatarProcessing && (
              ...
              <div className="w-full flex-shrink-0 p-8 min-h-[420px]">
                <div>
                  ...
                  <p className="text-sm text-kumo-subtle mb-6">
                    This model powers chats, builds workflows, and creates
                    Applications. A working model is required.
                  </p>
                  ...
                  {modelStatus === "loading" ? (
                    ...
                    <div className="rounded-xl border border-kumo-danger/40 bg-kumo-danger/5 px-5 py-6 text-center">
                      <p className="text-sm font-medium text-kumo-default">
                        We couldn&apos;t load your AI models
                      </p>
                      <p className="mt-1 text-xs leading-5 text-kumo-subtle">
                        Setup cannot finish until Verglas can confirm an
                        available model.
                      </p>
                      <button
                        type="button"
                      ...
                      <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                        ...
                        {modelStatus === "empty" && (
                          <div className="rounded-xl border border-dashed border-kumo-line bg-kumo-base px-5 py-8 text-center">
                            ...
                            <p className="mt-1 text-xs leading-5 text-kumo-subtle">
                              Verglas needs one model for agents, workflows, and
              ...
              {/* ── Step 2: Connections ───────────────────────────────────────── */}
              <div
                className={`w-full flex-shrink-0 p-8 min-h-[420px] ${showConnectionsStep ? "" : "hidden"}`}
              >
                <div>
                  <h2 className="text-lg font-medium text-kumo-default mb-1">
                  ...
                  <p className="text-sm text-kumo-subtle mb-6">
                    Link your accounts so your workspaces can access them. You
                        ...
                        return (
                          ...
                          >
                            <div
                              ...
                              style={{
                                backgroundColor: getVendorIconBackground(
                                  vendor.id,
                                  resolvedThemeMode,
                                ),
                              }}
  ...
  return (
    <div>
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-kumo-subtle">
          {siteName} will start with the choices below. You can change them
          later.
        </p>
      </div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-2">
        <div className="rounded-xl border border-kumo-line bg-kumo-base px-3.5 py-3">
          <p className="text-[10px] font-medium uppercase tracking-[0.1em] text-kumo-inactive">
            Primary agent
          </p>
          <p className="mt-1 truncate text-sm font-medium text-kumo-default">
            {selectedModel?.name ?? "Not selected"}
          </p>
          <p className="mt-0.5 text-[11px] text-kumo-subtle">
            {useAsQuickModel
              ? "Also handles quick tasks"
              : "Quick model unchanged"}
          </p>
        </div>
        <div className="rounded-xl border border-kumo-line bg-kumo-base px-3.5 py-3">
          <p className="text-[10px] font-medium uppercase tracking-[0.1em] text-kumo-inactive">
          ...
          <p className="mt-0.5 text-[11px] text-kumo-subtle">
            {connectionCount === 1
              ...
              : connectionCount > 1
                ? "Services connected"
                : "Add them whenever you are ready"}
          </p>
        </div>
      </div>
