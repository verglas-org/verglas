...
export function databaseKindLabel(
...
export function databaseCapabilityLabels(
...
export function databaseResourceDescription(
  database: VerglasDatabaseDefinition,
): string {
  ...
  const storage =
    database.storage.mode === "managed"
      ? "Managed storage"
      : database.storage.dataPath;
  const catalog =
    database.catalog.mode === "managed-lakekeeper"
      ? "Managed Lakekeeper"
