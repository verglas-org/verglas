...
describe('onboarding model selection', () => {
  it('requires an explicit configured model before continuing', () => {
    expect(canContinueWithOnboardingModel(MODELS, null, false, false)).toBe(false)
    expect(canContinueWithOnboardingModel(MODELS, MODELS[0].id, false, false)).toBe(true)
  })

  it('does not continue while models are loading or failed to load', () => {
    expect(canContinueWithOnboardingModel(MODELS, MODELS[0].id, true, false)).toBe(false)
    expect(canContinueWithOnboardingModel(MODELS, MODELS[0].id, false, true)).toBe(false)
  })

  it('rejects a stale model identifier', () => {
    expect(canContinueWithOnboardingModel(MODELS, 'runtime:codex:removed', false, false)).toBe(false)
  })
  ...
  it('describes blocking states honestly', () => {
    ...
    expect(onboardingModelStatus(MODELS, null, false, false)).toBe('selection-required')
    expect(onboardingModelStatus(MODELS, MODELS[1].id, false, false)).toBe('ready')
  })
})
