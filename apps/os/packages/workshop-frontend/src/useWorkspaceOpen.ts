...
export function useWorkspaceOpen({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const hadOpenWorkspace =
      id !== undefined && openWorkspaceIdRef.current === id;
    ...
    const load = async () => {
      ...
      try {
        ...
        const shareKey = hash.startsWith("#share=")
        ...
        const resolvedSubscription = await overseerStub.subscribeToMetadata(
          (nextMetadata) => {
            if (cancelled) return;
