...
function PasswordField({
  ...
  return (
    <div>
      ...
      {error ? (
        <p className="mt-1 text-[12px] tracking-[-0.1px] text-kumo-danger">
          {error}
        </p>
      ) : description ? (
...
export default function SettingsPage() {
  ...
  const [accessIdentity, setAccessIdentity] =
    useState<VerglasAccessIdentity | null>(null);
  const [accessResources, setAccessResources] = useState<
            ...
            <div className="flex items-start gap-3 px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[14px] font-medium text-kumo-default">
                  {accessIdentity?.tenantOwner
                    ? "Tenant owner"
                    : "Scoped tenant member"}
                </p>
                <p className="mt-1 text-[12px] leading-5 text-kumo-subtle">
                  Agents, Jobs, Applications, and Integrations receive separate
                  process identities. Access is delegated from your existing
                  permissions and checked at the Verglas boundary.
                </p>
                <dl className="mt-3 grid gap-2 text-[12px] sm:grid-cols-2">
                  <div>
                    <dt className="text-kumo-inactive">Tenant</dt>
                    <dd className="mt-0.5 font-mono text-kumo-default">
                      {accessIdentity?.tenantId ?? "Unavailable"}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-kumo-inactive">Principal</dt>
                    <dd className="mt-0.5 truncate font-mono text-kumo-default">
                      {accessIdentity?.principalId ?? "Unavailable"}
                    </dd>
                  </div>
                </dl>
              </div>
