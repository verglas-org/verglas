...
const worker = (
  overrides: Partial<VerglasWorkerSummary> = {},
...
describe("workers presentation", () => {
  ...
  it("renders portable trigger declarations as a concise schedule", () => {
    expect(workerScheduleSummary(worker())).toEqual({
      label: "Cron · 0 9 * * *",
      kind: "scheduled",
    });
    expect(workerScheduleSummary(worker({ triggers: "invalid" }))).toEqual({
      label: "Invalid trigger declaration",
      kind: "unknown",
    });
  });
  ...
  it("derives operational totals from workers and their bounded history", () => {
    expect(
      summarizeWorkers([
        worker({
          activeRun: true,
          recentRuns: [{ jobId: "1", state: "running", createdAt: "now" }],
        }),
        worker({
          name: "hourly",
          triggers: "[]",
          recentRuns: [{ jobId: "2", state: "failed", createdAt: "now" }],
        }),
      ]),
