...
describe("chatWidgetStatus", () => {
  it("gives approvals clear lifecycle labels", () => {
    expect(chatWidgetStatus("pending")).toEqual({label: "Pending approval", tone: "pending"});
    expect(chatWidgetStatus("approved")).toEqual({label: "Approved", tone: "success"});
    expect(chatWidgetStatus("denied")).toEqual({label: "Denied", tone: "danger"});
  });
