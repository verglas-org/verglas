...
export function DialogContent({
  className,
  ...props
}: ComponentProps<typeof KumoDialog>) {
  return (
    <KumoDialog
      className={cn(
        "w-[min(32rem,calc(100vw-2rem))] rounded-xl border border-kumo-line bg-kumo-base p-0 shadow-xl",
        className,
      )}
      {...props}
    />
...
export function DialogFooter({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-row justify-end gap-2 px-5 pb-5", className)}
      {...props}
    />
  );
}
...
export function DialogTitle({
  ...
  return (
    <KumoDialog.Title
      className={cn(
        "text-base font-medium tracking-[-0.25px] text-kumo-default",
        className,
      )}
      {...props}
    />
