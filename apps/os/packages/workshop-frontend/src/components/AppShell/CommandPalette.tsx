...
export default function CommandPalette({
  ...
  useEffect(() => {
    ...
    const isFresh =
      paletteCache &&
      Date.now() - paletteCache.fetchedAt < PALETTE_CACHE_TTL_MS;
    if (!isFresh) {
      Promise.all([
  ...
  const { groups, flat } = useMemo(() => {
    ...
    const refine = (cmds: Command[], limit: number): ScoredCommand[] => {
      if (!searching)
        return cmds.slice(0, limit).map((c) => ({ ...c, indices: [] }));
    ...
    const resultGroups = built.filter((group) => group.items.length > 0);
    const resultItems = resultGroups.flatMap((group) => group.items);
    return { groups: resultGroups, flat: resultItems };
  ...
  return (
    ...
    >
      ...
      <div className="themed-floating-shadow-lg relative w-full max-w-xl overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
        ...
        <div
          ref={listRef}
          className="sidebar-scroll max-h-[min(60vh,420px)] overflow-y-auto p-1.5"
        >
          {flat.length === 0 ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1 truncate">
                          {highlight(cmd.label, cmd.indices)}
                        </span>
                        {cmd.hint && (
        ...
        <div className="flex items-center gap-3 border-t border-kumo-line px-3.5 py-2 text-[11px] text-kumo-inactive">
          <span className="flex items-center gap-1">
            <kbd className="rounded border border-kumo-line px-1 py-0.5 font-sans leading-none">
              ↑
            </kbd>
            <kbd className="rounded border border-kumo-line px-1 py-0.5 font-sans leading-none">
              ↓
