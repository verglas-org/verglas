...
const INPUT =
  "h-10 w-full rounded-lg border border-kumo-line bg-kumo-base px-3 text-sm text-kumo-default outline-none transition focus:border-kumo-ring focus:ring-2 focus:ring-kumo-ring/15";

function ResourceKindBadge({ kind }: { kind: string }) {
  return (
    <span className="rounded-md bg-kumo-tint px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.08em] text-kumo-subtle">
      {kind.replaceAll("_", " ")}
    </span>
  );
}
...
export default function AdminAccessPanel({
  ...
  const load = async () => {
    ...
    setPrincipalId(
      (value) =>
        value ||
        next.principals.find((principal) => principal.kind !== "user")?.id ||
        "",
    );
    setResourceId(
      (value) =>
        value ||
        next.resources.find((resource) => resource.kind === "database")?.id ||
        next.resources[0]?.id ||
        "",
    );
  };
  ...
  const resources = useMemo(
    () => (snapshot ? buildResourceTree(snapshot.resources) : []),
    [snapshot],
  );
  ...
  const delegate = async () => {
    ...
    try {
      ...
      toasts.add({
        title:
          error instanceof Error ? error.message : "Unable to delegate access",
        variant: "error",
      });
    } finally {
      setSaving(false);
  ...
  if (!snapshot) {
    return (
      <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-6 text-sm text-kumo-subtle">
    ...
    <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-3">
        <Summary
          ...
          label="Principals"
          value={snapshot.principals.length}
          detail="People and workloads"
        />
        <Summary
          icon={<Database size={18} />}
          label="Resources"
          value={snapshot.resources.length}
          detail="Databases and runtimes"
        />
        <Summary
          icon={<Key size={18} />}
          label="Explicit grants"
          value={snapshot.grants.length}
          detail="Inherited access is live"
        />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(360px,1.1fr)]">
        <section className="overflow-hidden rounded-xl border border-kumo-line bg-kumo-elevated">
          <div className="border-b border-kumo-line px-5 py-4">
            ...
            <p className="mt-1 text-xs leading-5 text-kumo-subtle">
              Database grants inherit down to namespaces, tables, graph, and
              vector resources.
            </p>
          </div>
          <div className="max-h-[352px] overflow-y-auto p-2">
            {resources.map(({ resource, depth }) => {
              ...
              return (
                ...
                >
                  <span
                    className={`h-1.5 w-1.5 shrink-0 rounded-full ${selected ? "bg-kumo-brand" : "bg-kumo-line"}`}
                  />
            ...
            {resources.length === 0 && (
              <p className="px-3 py-8 text-center text-sm text-kumo-subtle">
                No protected resources registered yet.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="rounded-xl border border-kumo-line bg-kumo-elevated p-5">
          ...
          <p className="mt-1 text-xs leading-5 text-kumo-subtle">
            The grant is accepted only when your principal can pass every
            selected action on this resource.
          </p>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <label className="text-xs font-medium text-kumo-subtle">
              ...
              >
                ...
                {processPrincipals.map((principal) => (
          ...
          {selectedResource && (
            <p className="mt-2 font-mono text-[11px] text-kumo-inactive">
              Selected {selectedResource.kind}: {selectedResource.id}
            </p>
          )}
          <fieldset className="mt-5">
            <legend className="text-xs font-medium text-kumo-subtle">
              Actions
            </legend>
              ...
              {DELEGATABLE_ACTIONS.map((action) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-full border px-2.5 py-1 text-xs transition ${selected ? "border-kumo-brand bg-kumo-brand/10 text-kumo-brand" : "border-kumo-line text-kumo-subtle hover:bg-kumo-tint"}`}
                  >
          ...
          <div className="mt-5 flex justify-end">
            <Button
              ...
              size="sm"
              loading={saving}
              disabled={!principalId || !resourceId || actions.length === 0}
              onClick={delegate}
            >
              Delegate access
            </Button>
        ...
        <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-kumo-subtle">
              Revoking a grant takes effect for the next request. Root owner
              grants cannot be removed here.
            </p>
          </div>
            ...
            {snapshot.grants.map((grant) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate font-mono text-xs text-kumo-default">
                    {grant.principalId}{" "}
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {grant.actions.map((action) => (
                      <span
                        key={action}
                        className="rounded bg-kumo-tint px-1.5 py-0.5 text-[10px] font-medium text-kumo-subtle"
                      >
                        {formatAccessAction(action)}
                      </span>
                    ))}
                ...
                {!grant.id.startsWith("local-owner/") && (
                  <Button
                    variant="ghost"
                    size="sm"
      ...
      <section className="overflow-hidden rounded-xl border border-kumo-line bg-kumo-elevated">
        <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-kumo-subtle">
              Credential values are never retained or displayed here.
            </p>
          </div>
          ...
          <div className="divide-y divide-kumo-line">
            {snapshot.tokens.map((token) => {
              ...
              return (
                ...
                >
                  <span
                    className={`h-2 w-2 shrink-0 rounded-full ${active ? "bg-kumo-success" : "bg-kumo-inactive"}`}
                  />
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-kumo-default">
                      {token.name}
                    </p>
                    <p className="mt-0.5 truncate font-mono text-[11px] text-kumo-subtle">
                      {token.parentPrincipalId} → {token.principalId}
                    </p>
...
function Summary({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-kumo-line bg-kumo-elevated px-4 py-3.5">
      <div className="grid h-9 w-9 place-items-center rounded-lg bg-kumo-tint text-kumo-brand">
        {icon}
      </div>
      <div>
        <p className="text-lg font-semibold leading-5 text-kumo-default">
          {value}
        </p>
