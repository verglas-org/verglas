...
const ACTIONS: VerglasAccessAction[] = [
  "discover",
  "describe",
  "query",
  "append",
...
const INPUT =
  "h-9 w-full rounded-lg border border-kumo-line bg-kumo-base px-3 text-[13px] text-kumo-default outline-none transition focus:border-kumo-ring focus:ring-2 focus:ring-kumo-ring/15";

type AccessToken = Awaited<
  ReturnType<RpcStub<AuthenticatedApi>["listAccessTokens"]>
>[number];
type CreateTokenInput = Parameters<
  RpcStub<AuthenticatedApi>["createAccessToken"]
>[0];
...
export default function PersonalAccessTokenPanel({
  ...
  const [actions, setActions] = useState<VerglasAccessAction[]>([
    "discover",
    "describe",
    "query",
  ]);
  ...
  const simpleResources = useMemo(
    () => simpleTokenResources(resources),
    [resources],
  );
  const expiresAt = useMemo(
    () => new Date(Date.now() + expiration * 1_000),
    [expiration],
  );
  ...
  return (
    <section className="flex flex-col gap-3">
      <div className="flex items-end justify-between gap-4 px-1">
        <div>
          ...
          <p className="mt-1 text-[12px] leading-5 text-kumo-subtle">
            Connect the Verglas CLI, an SDK, or a local script without using
        ...
        <div className="overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
          <div className="border-b border-kumo-line px-5 py-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-[14px] font-medium text-kumo-default">
                  Create a token
                </h3>
                <p className="mt-0.5 text-[12px] text-kumo-subtle">
                  Choose what this device can access. You can revoke it at any
                  time.
                </p>
              </div>
              <Button
          ...
          <div className="space-y-5 p-5">
            ...
            {!advanced && (
              <>
                <label className="block text-[12px] font-medium text-kumo-subtle">
                  Access to
                  <select
                    value={simpleResourceId}
                    onChange={(event) =>
                      setSimpleResourceId(event.target.value)
                    }
                    className={`mt-1.5 ${INPUT}`}
                  >
                    ...
                    {simpleResources.map((resource) => (
                      <option key={resource.id} value={resource.id}>
                        {resource.kind === "tenant"
                          ? "All databases in this tenant"
                          : resource.id.replace(/^database\//, "")}
                      </option>
                    ))}
                  </select>
                  <span className="mt-1 block font-normal leading-5 text-kumo-inactive">
                    Database access automatically includes its tables.
                  </span>
                </label>

                <fieldset>
                  <legend className="text-[12px] font-medium text-kumo-subtle">
                    Permission
                  </legend>
            ...
            <label className="block text-[12px] font-medium text-kumo-subtle">
              ...
              <span className="mt-1 block font-normal leading-5 text-kumo-inactive">
                One year is the maximum. Expiring tokens limit the damage from
                forgotten or leaked credentials.
              </span>
            </label>
            ...
            >
              <summary className="cursor-pointer px-3 py-2.5 text-[12px] font-medium text-kumo-subtle">
                Custom permissions
              </summary>
              <div className="space-y-4 border-t border-kumo-line p-4">
                <div>
                  <p className="text-[12px] font-medium text-kumo-subtle">
                    Resources
                  </p>
                  <div className="mt-2 max-h-48 overflow-y-auto rounded-lg border border-kumo-line bg-kumo-base p-1.5">
                    {resourceTree.map(({ resource, depth }) => {
                      ...
                      return (
                        <label
                          ...
                          className={`flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-xs transition ${selected ? "bg-kumo-brand/10 text-kumo-brand" : "text-kumo-default hover:bg-kumo-tint"}`}
                        >
                          <input
                            checked={selected}
                            onChange={() => toggleResource(resource.id)}
                            type="checkbox"
                            className="accent-[var(--color-kumo-brand)]"
                          />
                    ...
                    {resourceTree.length === 0 && (
                      <p className="px-3 py-4 text-center text-xs text-kumo-subtle">
                        No resources are currently delegable.
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-2">
                    {ACTIONS.map((action) => {
                      ...
                      return (
                        <label
                          key={action}
                          className={`flex cursor-pointer gap-2.5 rounded-lg border p-3 transition ${selected ? "border-kumo-brand bg-kumo-brand/5" : "border-kumo-line bg-kumo-base hover:bg-kumo-tint"}`}
                        >
                          <input
                            type="checkbox"
                            checked={selected}
                          ...
                          />
                          <span>
                            <span className="block text-[12px] font-medium text-kumo-default">
                              {formatAccessAction(action)}
                            </span>
                            <span className="mt-0.5 block text-[11px] leading-4 text-kumo-subtle">
                              {describeAccessAction(action)}
                            </span>
                          </span>
                        </label>
                      );
            ...
            <div className="flex items-center justify-between gap-4 border-t border-kumo-line pt-4">
              <p className="text-[11px] leading-4 text-kumo-inactive">
      ...
      {revealedToken && (
        <div className="rounded-xl border border-kumo-success/40 bg-kumo-success/10 p-4">
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs leading-5 text-kumo-subtle">
                Copy it now. Verglas cannot show it again after you leave this
                page.
              </p>
              <div className="mt-3 flex gap-2">
                <code className="min-w-0 flex-1 overflow-x-auto rounded-md border border-kumo-line bg-kumo-base px-3 py-2 text-xs text-kumo-default">
                  {revealedToken}
                </code>
                <Button
                  variant="secondary"
              ...
              <p className="mt-2 text-[11px] text-kumo-inactive">
                Use it as the <code className="font-mono">VERGLAS_TOKEN</code>{" "}
      ...
      <div className="overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
        <div className="flex items-center gap-2 border-b border-kumo-line px-5 py-4">
          <ShieldCheck size={17} className="text-kumo-brand" />
          <div>
            <h3 className="text-[14px] font-medium text-kumo-default">
              Your tokens
            </h3>
            <p className="mt-0.5 text-[12px] text-kumo-subtle">
              Tokens you created for devices and scripts. Browser login sessions
              are not shown here.
            </p>
          </div>
        </div>
        {loading ? (
          <p className="px-5 py-8 text-center text-sm text-kumo-subtle">
            Loading tokens…
          </p>
        ) : tokens.length === 0 ? (
          <div className="px-5 py-8 text-center">
            <Key size={22} className="mx-auto text-kumo-inactive" />
          ...
          <div className="divide-y divide-kumo-line">
            {tokens.map((token) => {
              const active =
                !token.revokedAt && token.expiresAt * 1_000 > Date.now();
              return (
                <div
                  key={token.id}
                ...
                >
                  <div
                    className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${active ? "bg-kumo-tint text-kumo-brand" : "bg-kumo-fill text-kumo-inactive"}`}
                  >
                    <Key size={15} />
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-xs text-kumo-subtle">
                      {active
                        ? `Expires ${new Date(token.expiresAt * 1_000).toLocaleDateString()}`
                        : token.revokedAt
                          ? "Revoked"
                          : "Expired"}
                      {token.lastUsedAt
                        ? ` · Last used ${new Date(token.lastUsedAt * 1_000).toLocaleDateString()}`
                        : ""}
                    </p>
                  </div>
                  {active && (
...
function PermissionChoice({
  ...
  return (
    <label
      className={`flex cursor-pointer gap-3 rounded-lg border p-3 transition ${checked ? "border-kumo-brand bg-kumo-brand/5" : "border-kumo-line hover:bg-kumo-tint"}`}
    >
      <input
        type="radio"
      ...
      />
      <span>
        <span className="block text-[13px] font-medium text-kumo-default">
          {title}
        </span>
        <span className="mt-0.5 block text-[11px] leading-4 text-kumo-subtle">
          {description}
