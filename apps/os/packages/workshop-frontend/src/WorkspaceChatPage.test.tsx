...
const testState = vi.hoisted(() => ({
  chatProps: [] as Array<{ selectedChatId: number | null; singleChat?: boolean; onNavigateToChat: (id: number | null) => void }>,
  navigate: vi.fn<() => void>(),
}))
...
vi.mock('./useWorkspaceOpen', () => ({
  useWorkspaceOpen: () => ({ overseer: { stub: {} }, error: null, retry: vi.fn<() => void>() }),
}))
...
vi.mock('./components/WorkshopControls', () => ({ WorkshopIconButton: ({ children }: { children: ReactNode }) => <button>{children}</button> }))
