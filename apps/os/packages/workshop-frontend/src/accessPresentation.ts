...
const sortResources = (
  left: VerglasAccessResource,
  right: VerglasAccessResource,
) => left.id.localeCompare(right.id);

const ACCESS_ACTION_COPY: Record<
  VerglasAccessAction,
  { label: string; description: string }
> = {
  discover: {
    label: "List resources",
    description: "See that a resource exists in lists and search results.",
  },
  describe: {
    label: "View details",
    description: "Read names, schemas, configuration, and other metadata.",
  },
  query: {
    label: "Read data",
    description:
      "Query or read the contents of a database, table, or data resource.",
  },
  append: {
    label: "Add data",
    description: "Add new records without changing or deleting existing data.",
  },
  modify: {
    label: "Edit data and settings",
    description: "Read, add, change, or delete existing data and metadata.",
  },
  create_child: {
    label: "Create databases or tables",
    description:
      "Create a new resource inside the selected tenant or database.",
  },
  execute: {
    label: "Run jobs or integrations",
    description:
      "Start a Worker, Job, Integration operation, or other callable resource.",
  },
  use_secret: {
    label: "Use saved credentials",
    description: "Use a saved credential without revealing its value.",
  },
  deploy: {
    label: "Deploy applications",
    description:
      "Deploy or update an Application, Worker, or other executable resource.",
  },
  connect: {
    label: "Connect to databases",
    description:
      "Open a database or service connection. This does not grant data access by itself.",
  },
  pass_grants: {
    label: "Delegate existing access",
    description:
      "Pass along permissions already held on the selected resource.",
  },
  manage_grants: {
    label: "Manage access",
    description: "Add or remove permissions on the selected resource.",
  },
  own: {
    label: "Own resource",
    description: "Perform every operation available on the selected resource.",
  },
};
...
export function simpleTokenResources(
...
export function toPresetTokenGrants(
  resourceId: string,
  level: TokenAccessLevel,
): {
  resourceId: string;
  actions: VerglasAccessAction[];
}[] {
...
export function buildResourceTree(
  ...
  const visit = (resource: VerglasAccessResource, depth: number) => {
    ...
    for (const child of (children.get(resource.id) ?? []).toSorted(
      sortResources,
    ))
      visit(child, depth + 1);
  };
...
export function isTokenRequestComplete(
