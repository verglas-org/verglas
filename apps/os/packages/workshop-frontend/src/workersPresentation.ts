...
export function workerScheduleSummary(worker: Pick<VerglasWorkerSummary, 'triggers'>): WorkerSchedule {
  try {
    const trigger = (JSON.parse(worker.triggers) as Array<{ type?: string; schedule?: string; path?: string; eventType?: string }>)[0]
    if (!trigger) return { label: 'Manual only', kind: 'manual' }
    if (trigger.type === 'cron') return { label: trigger.schedule ? `Cron · ${trigger.schedule}` : 'Scheduled', kind: 'scheduled' }
    if (trigger.type === 'webhook') return { label: trigger.path ? `Webhook · ${trigger.path}` : 'Webhook', kind: 'event' }
    if (trigger.type === 'event') return { label: trigger.eventType ? `Event · ${trigger.eventType}` : 'Event trigger', kind: 'event' }
    return { label: trigger.type || 'Manual only', kind: trigger.type ? 'unknown' : 'manual' }
  } catch {
...
export function summarizeWorkers(workers: VerglasWorkerSummary[]) {
  return {
    ...
    scheduled: workers.filter((worker) => workerScheduleSummary(worker).kind === 'scheduled').length,
    failed: workers.filter((worker) => worker.recentRuns?.some((run) => run.state === 'failed')).length,
  }
}
