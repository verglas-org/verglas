...
import { groupModels, modelRuntimeLabel } from '../modelRuntimePresentation'
...
function ProvidersPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await authenticatedApi.deleteModel(deleteTarget.id)
      await fetchAll()
      setDeleteTarget(null)
      toasts.add({ title: 'Model deleted', variant: 'success' })
    } catch (err) {
      console.error('Failed to delete model:', err)
      toasts.add({ title: 'Failed to delete model', variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
    ...
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col px-6 sm:px-10">
        ...
        {loading ? (
          ...
          groupModels(filtered).map((group) => (
            <section key={group.id} className="mb-3">
              <h2 className="px-3 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-[0.45px] text-kumo-inactive">
                {group.label}
              </h2>
      ...
      <DeleteConfirmationDialog
        ...
        description={deleteTarget
          ? `Delete ${deleteTarget.name} from ${modelRuntimeLabel(deleteTarget.id) ?? 'this provider'}? You can link the provider again to restore its catalog.`
          : ''}
