...
import {
  groupIntegrationFields,
  missingRequiredIntegrationFields,
...
function IntegrationsPage() {
  ...
  const [configuration, setConfiguration] =
    useState<VerglasIntegrationConfiguration | null>(null);
  ...
  const open = async (name: string) => {
    ...
    try {
      setConfiguration(
        await authenticatedApi.getVerglasIntegrationConfiguration(name),
      );
    } catch (err) {
  ...
  const save = async () => {
    ...
    const missing = missingRequiredIntegrationFields(
    ...
    if (missing.length) {
      setValidationError(
        `Enter ${missing.join(", ")} before verifying this connection.`,
      );
      return;
    }
    setSaving(true);
    ...
    try {
      ...
      setConfiguration(
        await authenticatedApi.getVerglasIntegrationConfiguration(selected),
      );
      setValues({});
      await load();
    } catch (err) {
      ...
      {selected !== null ? (
        <CatalogDetailCard
          ...
          meta={
            configuration?.configured ? (
              ...
              <CatalogStatus
                value={selectedVessel.health}
                good={selectedVessel.health === "ready"}
              />
            ) : undefined
          }
          onBack={close}
        ...
        >
          ...
          {!configuration && !detailError ? (
            <div className="py-10 text-center text-sm text-kumo-subtle">
            ...
            <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_13rem]">
              <div className="min-w-0 space-y-6">
                ...
                {configuration.fields.length > 0 ? (
                  <div className="space-y-5">
                    {groupIntegrationFields(configuration.fields).map(
                      (group) => (
                        <section
                          key={group.id}
                          aria-labelledby={`${group.id}-settings`}
                        >
                          <div className="mb-2 flex items-center gap-2 px-1">
                            ...
                            <h3
                              ...
                              className="text-[12px] font-medium uppercase tracking-[0.08em] text-kumo-inactive"
                            >
                              {group.title}
                            </h3>
                          </div>
              ...
              <aside className="space-y-4 lg:border-l lg:border-kumo-line lg:pl-5">
                <section>
                  <h3 className="text-[12px] font-medium uppercase tracking-[0.08em] text-kumo-inactive">
                    Connection
                  </h3>
                  <p className="mt-2 text-[12px] leading-5 text-kumo-subtle">
                    Secret values are masked and never shown again after they
                    are saved.
                  </p>
                </section>
                ...
                {isSafeExternalUrl(configuration.helpUrl) && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-[12px] font-medium text-kumo-brand hover:underline"
                  >
                    Setup documentation
  ...
  return (
    <section className="rounded-xl border border-kumo-line bg-kumo-tint/30 px-4 py-3.5">
      <div className="flex items-start gap-3">
        <div
          className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${configured ? "bg-kumo-success-tint text-kumo-success" : "bg-kumo-fill text-kumo-brand"}`}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
            <div>
              <dt className="text-kumo-inactive">Vessel</dt>
              <dd className="mt-0.5 truncate font-mono text-kumo-subtle">
                {vessel?.name ?? "Loading"}
              </dd>
            </div>
            <div>
              <dt className="text-kumo-inactive">Runtime</dt>
              <dd className="mt-0.5 capitalize text-kumo-subtle">
                {vessel?.state ?? health}
              </dd>
            </div>
          </dl>
...
function IntegrationField({
  ...
  return (
    <label
      className={`block px-4 py-3.5 ${bordered ? "border-t border-kumo-line" : ""}`}
    >
      <span className="flex flex-wrap items-center gap-1.5 text-[13px] font-medium text-kumo-default">
        {field.label}
        {field.required && (
          <span className="text-kumo-danger" aria-label="Required">
            *
          </span>
        )}
        {secret && (
          <span className="rounded bg-kumo-fill px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.06em] text-kumo-inactive">
            Secret
          </span>
        )}
      </span>
      {field.description && (
        <span className="mt-1 block text-[12px] leading-4 text-kumo-subtle">
          {field.description}
        </span>
      )}
      <input
...
function inputType(
  ...
  return ["email", "number", "url"].includes(type)
    ? (type as "email" | "number" | "url")
    : "text";
}
...
function SetupInstruction({
  ...
  instruction: string | IntegrationSetupInstruction;
  number: number;
}) {
