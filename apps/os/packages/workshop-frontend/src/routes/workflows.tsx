...
function triggerLabels(raw: string): string[] {
  try {
    const triggers = JSON.parse(raw) as Array<{
      type?: string;
      schedule?: string;
      path?: string;
      eventType?: string;
    ...
    return triggers.map((trigger) => {
      ...
      if (trigger.type === "webhook")
        return trigger.path ? `Webhook · ${trigger.path}` : "Webhook";
      if (trigger.type === "event")
        return trigger.eventType ? `Event · ${trigger.eventType}` : "Event";
...
function WorkersPage() {
  ...
  const [selectedRun, setSelectedRun] =
    useState<VerglasWorkerRunSummary | null>(null);
    ...
    async (name: string) => {
      ...
      try {
        ...
        setSelectedRun((current) => {
          ...
          return (
            next.recentRuns?.find((run) => run.jobId === current.jobId) ??
            next.recentRuns?.[0] ??
            null
          );
        });
  ...
  useAutomaticRefresh(
    () => (selected ? loadDetail(selected) : undefined),
    2_000,
  );
  ...
  return (
    <CatalogPage
      ...
      actions={
        <Link
          to="/"
          search={{ prompt: "Create a worker that " }}
    ...
    >
      ...
      {selected ? (
        ...
        >
          {detailLoading && !worker ? (
            ...
            <div className="space-y-6">
              <section>
                <h3 className="mb-2 text-[11px] font-medium uppercase tracking-wide text-kumo-inactive">
                  Execution graph
                </h3>
                <div className="grid items-center gap-2 sm:grid-cols-[1fr_auto_1fr_auto_1fr]">
                  <GraphNode
                    label="Trigger"
                    value={triggers[0] ?? "Manual only"}
                  />
                  ...
                  <GraphNode
                    label="Output"
                    value={worker.output || "No output"}
                  />
                </div>
                <h3 className="mb-2 mt-5 text-[11px] font-medium uppercase tracking-wide text-kumo-inactive">
                  Triggers
                </h3>
                <div className="flex flex-wrap gap-1.5">
                  {triggers.map((trigger, index) => (
                    <span
                      key={`${trigger}-${index}`}
                      className="rounded-md bg-kumo-tint px-2 py-1 text-[11px] text-kumo-subtle"
                    >
                      {trigger}
                    </span>
                  ))}
              ...
              <section>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <h3 className="text-[11px] font-medium uppercase tracking-wide text-kumo-inactive">
                    Runs
                  </h3>
                  <RunHistoryDots
              ...
              <section>
                ...
                {worker.sourceCode ? (
                  ...
                  <p className="text-sm text-kumo-subtle">
                    No TypeScript source was bundled in this worker revision.
                  </p>
                )}
              </section>
            </div>
        ...
        <WorkersBoard workers={workers} onOpen={open} />
      )}
...
function GraphNode({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-xl border border-kumo-line bg-kumo-tint/30 p-3">
      <div className="text-[10px] font-medium uppercase tracking-wide text-kumo-inactive">
        {label}
      </div>
      <div
        title={value}
        className="mt-2 truncate font-mono text-[12px] text-kumo-default"
      >
        {value}
      </div>
    </div>
