    ...
    >
      ...
      {workspace ? (
        <CatalogDetailCard
          ...
          subtitle={workspace.owner ? `Shared by ${workspace.owner.name || workspace.owner.id}` : 'Owned by you'}
          meta={workspace.pinned ? (
          ...
          footer={
            <>
              ...
              <button
                type="button"
                onClick={() => {
                  void navigate({ to: '/workspace/$id', params: { id: workspace.id } })
                }}
        ...
        >
          <dl className="grid gap-4 text-[13px] sm:grid-cols-2">
            <div>
              <dt className="text-[11px] font-medium uppercase tracking-wide text-kumo-inactive">Last active</dt>
              <dd className="mt-1 text-kumo-default">{formatRelativeTime(workspace.lastActive)}</dd>
            </div>
            <div>
              <dt className="text-[11px] font-medium uppercase tracking-wide text-kumo-inactive">Created</dt>
              <dd className="mt-1 text-kumo-default">{formatRelativeTime(workspace.created)}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-[11px] font-medium uppercase tracking-wide text-kumo-inactive">Workspace id</dt>
              <dd className="mt-1 break-all font-mono text-[12px] text-kumo-subtle">{workspace.id}</dd>
            </div>
          </dl>
        </CatalogDetailCard>
      ) : loading ? (
        ...
        <CatalogTable
          ...
          cards={workspaces.map((entry) => ({
            id: entry.id,
            icon: (
              <span className="text-[12px] font-medium text-kumo-subtle">{initials(entry.title)}</span>
            ),
            primary: entry.title || 'Untitled',
            secondary: entry.owner ? `Shared by ${entry.owner.name || entry.owner.id}` : 'Owned by you',
            tertiary: `Active ${formatRelativeTime(entry.lastActive)}`,
            meta: (
              <>
                {entry.pinned && <Star size={12} weight="fill" className="text-kumo-brand" />}
