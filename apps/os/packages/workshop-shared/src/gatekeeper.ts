// This file defines the API that the AI Verglas OS uses to talk to Adapters. Each Adapter
// provides connectivity to some external service which AI Workspaces can then manipulate. Each
// installation of the Verglas OS may have access to different adapters, typically based on
...
// to such resources, enabling the Workspace Workshop to grant a particular Workspace fine-grained
// access to just the things the user wants that Workspace to access.
//
// Each adapter is deployed as a completely independent Workers application from the Workspaces
...
export type VendorDescription = {
  ...
  // E.g. "Connect your Google account to give Workspaces access to Gmail, Google Docs, and BigQuery.
...
export type AccountDescription = {
  ...
  // GatekeeperUser.getSingletonGatekeeperClass) that the Workshop installs into the owner's workspaces
...
export type ResourceDescription = {
  ...
  // reveal the resource title to the agent, or may intend the same Workspace to be connected to
...
// The root interface of an Adapter, as provided to the Workspace Workshop.
//
// An installation of the Workspace Workshop is provided with a set of Adapters to allow it to
...
export interface GatekeeperVendor extends WorkerEntrypoint {
  ...
  //
  // TODO: How does the Workspace Workshop know when the supported URLs have changed, without polling?
  ...
  //
  // The Workspaces system will parse this file to construct a type database, which will be made
  ...
  // TODO: How does the Workspace Workshop know when the types have changed, without polling?
...
// RPC interface to an Adapter. This is a privileged interface exposed to the Workspace Workshop UI
// itself, not to Workspaces nor AI agents.
//
// The Adapter is already specialized for a particular human user of the Workspace Workshop. The
...
export interface GatekeeperUser extends WorkerEntrypoint {
  ...
  //
  // Note that the Overseer of a Workspace will call this immediately when the user pastes in a URL,
  // *before* the user has actually chosen to grant the Workspace any permissions on the resource.
  ...
  // owner's workspaces like any other gatekeeper — as a Facet under the Overseer — and auto-provides
  // its session to the agent as an unnamed capsule. Because it is a normal Gatekeeper, the session
  // (Gatekeeper.startSession) and catalog (Gatekeeper.getAgentCatalog) run workspace-side in the
...
// interface is exposed to the Overseer, not directly to the Workspace.
export interface Gatekeeper<Session> extends DurableObject {
  ...
  // been applied. This allows the Workspace to keep working, potentially queuing up additional
  ...
  // Informs the gatekeeper that a new user is being added to the Workspace with the potential to see
  ...
  // access. However, if the Workspace itself carefully reads only emails addressed to a particular
  ...
  // associated storage.
  //
  // If the returned `restart` flag is true, rejecting this action requires restarting the Workspace.
  // This is sometimes needed by gatekeepers that simulate actions as if they had been approved --
  // the session may be in a state that is difficult to roll back without confusing the Workspace.
...
export interface ObservationAuthorizer extends RpcTarget {
  // Check whether the workspace should be permitted to make an observation (that is, to read some
  // data from an external service). The gatekeeper calls this on every read operation, and must
  // wait for the response before returning anything to the workspace. The method will return normally
  ...
  // data to the workspace, this is OK.
...
export interface ApprovalQueue extends ObservationAuthorizer {
  // TODO: Method to indicate that the workspace tried to perform an action that the gatekeeper itself
  ...
  // Notifies the overseer that the workspace (or an agent) has requested to register a persistent
  // callback hook.
  //
  // `callback` is the stub received from the Workspace, which is intended to be called whenever some
  ...
  //
  // In a typical implementation, the gatekeeper may expose an API to the workspace like:
  //
  ...
  // either way the stub must be a persistent stub (see below). The workspace could call `onSomeEvent`
  ...
  // Finally, it invokes the `callback` object to deliver the event to the workspace.
  //
  ...
  //
  //     class Workspace extends DurableObject {
...
export type ObservationDescription = {
  ...
  //   can help detect situations where the workspace could leak information.
  ...
  // - If the workspace is shared already, authorizeObservation() must throw an exception to block
  //   the observation.
  // - All future sharing of the workspace is prohibited.
  // - Once observed, the workspace goes into "lockdown mode" where it can no longer perform any
  //   actions, only make observations. This prevents the workspace from leaking data through other
  //   gatekeepers.
  //
...
export type ActionDescription = {
  ...
  //   the workspace might be influenced by humans to perform actions that said humans couldn't
  //   perform directly.
...
export type HookTargetMetadata = {
  ...
  // The specific vessel workpiece within that workspace, when the hook is pinned to one. Absent
  // means the workspace's current default vessel.
  vesselId?: number;
}
