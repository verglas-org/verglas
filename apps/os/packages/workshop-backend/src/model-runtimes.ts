...
const RUNTIME_IDS: ModelRuntimeId[] = [
  "codex",
  "claude-code",
  "github-copilot",
];
...
export class ModelRuntimeManager {
  ...
  /** Creates a manager for one user's isolated Pi credential scope. */
  constructor(env: Cloudflare.Env, credentialScope: string) {
  ...
  async detect(): Promise<ModelRuntimeDetection> {
    if (!this.#config) {
      return {
        ...
        runtimeError:
          "The native model runtime adapter is not running for this deployment.",
        runtimes: this.#emptyStatuses(),
      };
    }
    try {
      const result = await this.#request<RuntimeAdapterDetection>(
        "GET",
        "/v1/runtimes",
      );
  ...
  async continueLogin(
    sessionId: string,
    _answer?: ModelRuntimeWizardAnswer,
  ): Promise<ModelRuntimeLoginResult> {
    return await this.#request(
      "POST",
    ...
    if (!detection.runtimeAvailable) {
      throw new Error(
        detection.runtimeError ||
          "The native model runtime adapter is unavailable.",
      );
    }
    const status = detection.runtimes.find((item) => item.id === runtime);
    if (!status?.available)
      throw new Error(status?.detail || `${runtime} is not installed.`);
    if (!status.linked)
      throw new Error(status.detail || `${runtime} is not signed in.`);
  }

  /** Runs a real bounded inference before a runtime is exposed in the model picker. */
  async verifyLinked(runtime: ModelRuntimeId, model: string): Promise<void> {
    await this.requireLinked(runtime);
    await this.#request(
      "POST",
  ...
  async listModels(
    runtime: ModelRuntimeId,
  ): Promise<ModelRuntimeCatalogEntry[]> {
  ...
  async #request<T>(
    ...
    const response = await fetch(`${this.#config.endpoint}${path}`, {
      ...
      headers: {
        ...
        "X-Verglas-Credential-Scope": this.#credentialScope,
    ...
    if (!response.ok) {
      throw new Error(
        result.error ||
          `Native runtime request failed: HTTP ${response.status}`,
      );
    }
    return result;
  }
