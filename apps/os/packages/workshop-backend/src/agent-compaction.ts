...
// A message that begins an agent turn: the user or a workspace prompted, a callback or nudge arrived,
...
// a batch that records only workspace creations or binding additions.
...
// it. Provisional workspace creations and binding additions need no such protection: the checkpoint
    ...
    if (message.type === "message") {
      ...
      for (let call of message.toolCalls ?? []) {
        ...
        if (normalizeLegacyToolName(call.toolName) === "createWorkpiece") {
          const c = call as Extract<typeof call, {toolName: "createWorkpiece"}> & {output?: {workspaceId: number}};
          if (c.output !== undefined) {
            chatBindings.set(c.input.bindingName, {type: "workpiece", id: c.output.workspaceId});
          }
        }
      }
      ...
      for (let {workspaceId, bindingName} of message.createdVessels ?? []) {
