...
import type {
  ActionLogEntry, ActionsSubscriber, AffectedCollaborator, AgentSpawnerConfig,
  AiChatAuthorInfo, AiChatHistoryPage, AiChatMessage, AiChatMetadata, AiChatSubscriber,
  BlueprintScreenshotUpload, BlueprintVesselSummary, BoundHookInfo, CapsuleSpecifier,
  ChatAttachmentHandle, ChatAttachmentUpload, CodeSubscriber, CollaboratorInfo, CollaboratorRole,
  ConsoleLogSubscriber, GatekeeperClient, IntegrationVerification, MessageFormatRef, ObserverBindingNeed,
  Overseer, PreApprovableAction, PresenceSubscriber, ShareLinkInfo, SlashCommandChoice,
  SlashCommandRequest, VesselClient, VerglasQueryActivity, VerglasQueryResult, WorkpieceId,
  WorkpiecesSubscriber, WorkspaceMetadata,
...
export function resolveAgentRuntimeConfig(env: Cloudflare.Env): AgentRuntimeConfig {
...
export class VerglasAgentRuntimeClient {
  ...
  async request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Verglas Agent Runtime ${path} failed: HTTP ${response.status} — ${detail}`);
    }
    return (text ? JSON.parse(text) : undefined) as T;
  }

  /** Creates the durable Workspace record idempotently. */
  createWorkspace(id: string, ownerId: string, title: string): Promise<WorkspaceWire> {
...
function mapChat(chat: ChatWire): AiChatMetadata {
  return {
    ...
    ...(chat.active && chat.model_profile ? {activeAgent: chat.model_profile} : {}),
  };
}
...
export class AgentWorkspace extends RpcTarget implements Overseer {
  ...
  #deny(): never {
    throw new Error("This legacy Workspace operation is not available in the Verglas agent runtime.");
  }
  ...
  async #emitMetadata(): Promise<void> {
    ...
    for (const subscriber of this.#metadataSubscribers) subscriber(metadata).catch(() => {});
  }

  /** Ensures the Postgres Workspace exists before returning it to a client. */
  async ensure(title = "Untitled Workspace"): Promise<void> {
    await this.#runtime.createWorkspace(this.workspaceId, this.#ownerId(), title);
  }
  ...
  async subscribeToMetadata(callback: RpcStub<(metadata: WorkspaceMetadata) => void>)
      : Promise<RpcStub<{}>> {
  ...
  async setTitle(title: string): Promise<void> {
    ...
    if (!normalized || normalized.length > 200) throw new Error("Workspace title is invalid.");
    await Promise.all([
      this.#runtime.request(this.#path(`?ownerId=${encodeURIComponent(this.#ownerId())}`), {
  ...
  async getChatHistory(chatId: number, _beforeSequence?: number): Promise<AiChatHistoryPage> {
  ...
  async getChatMessage(chatId: number, sequence: number): Promise<AiChatMessage | undefined> {
    return (await this.getChatHistory(chatId)).messages.find(message => message.sequence === sequence);
  }

  async subscribeToChat(subscriber: RpcStub<AiChatSubscriber>, _startAfter?: Date)
      : Promise<RpcStub<{}>> {
  ...
  async newChat(initialMessage: string | SlashCommandRequest, modelId: string | null,
                ...
                _formats?: MessageFormatRef[]): Promise<number> {
    if (typeof initialMessage !== "string") throw new Error("Slash commands are not yet supported.");
  ...
  async sendChatMessage(chatId: number, message: string | SlashCommandRequest,
                        modelId: string | null, capsules?: CapsuleSpecifier[],
                        attachments?: ChatAttachmentHandle[], _formats?: MessageFormatRef[]): Promise<void> {
    if (typeof message !== "string") throw new Error("Slash commands are not yet supported.");
    if (capsules?.length || attachments?.length) {
      throw new Error("Capsules and attachments are not yet supported by the agent runtime.");
    }
    const context = await this.#chatContext(modelId);
  ...
  async deleteChat(chatId: number): Promise<void> {
    await this.#runtime.request(this.#path(`/chats/${chatId}`), {method: "DELETE"});
  }

  async #permissionRequest(requestId: string): Promise<AiChatMessage & {type: "permissionRequest"}> {
    const separator = requestId.indexOf(":");
    if (separator < 1) throw new Error(`Malformed permission request id: ${requestId}`);
    const chatId = Number(requestId.slice(0, separator));
    if (!Number.isInteger(chatId)) throw new Error(`Malformed permission request id: ${requestId}`);
    const request = (await this.getChatHistory(chatId)).messages.find(message =>
    ...
    if (request.principalId !== `agent/${this.workspaceId}`) {
      throw new Error("Permission request principal does not belong to this Workspace.");
    }
    return request;
  }
  ...
  async stopAgent(chatId: number): Promise<void> {
    await this.#runtime.request(this.#path(`/chats/${chatId}/stop`), {method: "POST"});
  }
  ...
  async listSlashCommands(): Promise<SlashCommandChoice[]> { return []; }
  async subscribeToActions(subscriber: RpcStub<ActionsSubscriber>): Promise<RpcStub<{}>> {
    const retained = subscriber.dup();
    await retained.ready();
    // @ts-expect-error Native RPC targets implement the Cap'n Web disposal contract at runtime.
    return new NativeRpcStub<{}>({[Symbol.dispose]() { retained[Symbol.dispose](); }});
  }
  async listActions(): Promise<ActionLogEntry[]> { return []; }
  async listHooks(): Promise<BoundHookInfo[]> { return []; }
  async listPreApprovableActions(): Promise<PreApprovableAction[]> { return []; }
  async listAutoApprovedActionKinds(): Promise<Array<{gatekeeperId: WorkpieceId; actionKind: ActionKind}>> {
    return [];
  }
  async listBlueprints(): Promise<BlueprintVesselSummary[]> { return []; }

  async subscribeToPresence(_subscriber: RpcStub<PresenceSubscriber>): Promise<RpcStub<{}>> { this.#deny(); }
  async subscribeToWorkpieces(_subscriber: RpcStub<WorkpiecesSubscriber>): Promise<RpcStub<{}>> { this.#deny(); }
  async createWorkpiece(_title: string): Promise<RpcStub<VesselClient>> { this.#deny(); }
  async getVessel(_id: WorkpieceId): Promise<RpcStub<VesselClient>> { this.#deny(); }
  async subscribeToCode(_subscriber: RpcStub<CodeSubscriber>, _fromVersion?: number): Promise<RpcStub<{}>> { this.#deny(); }
  async updateCode(_update: Uint8Array, _chatId?: number): Promise<void> { this.#deny(); }
  async getGatekeeperById(_id: WorkpieceId): Promise<GatekeeperClient<unknown>> { this.#deny(); }
  async newGatekeeper(_accountId: number, _resourceUrl: string): Promise<GatekeeperClient<unknown> | null> { this.#deny(); }
  async newAiModelGatekeeper(_modelId: string): Promise<GatekeeperClient<unknown>> { this.#deny(); }
  async newAgentSpawnerGatekeeper(_config: AgentSpawnerConfig): Promise<GatekeeperClient<unknown>> { this.#deny(); }
  ...
  async setAutoApprovedActionKind(_gatekeeperId: WorkpieceId, _actionKind: ActionKind): Promise<void> { this.#deny(); }
  async removeAutoApprovedActionKind(_gatekeeperId: WorkpieceId, _tag: string): Promise<void> { this.#deny(); }
  async acceptConnectionRequest(_requestId: string, _result: {gatekeeperId: WorkpieceId}): Promise<void> { this.#deny(); }
  async denyConnectionRequest(_requestId: string): Promise<void> { this.#deny(); }
  async configureSource(_requestId: string, _values: Record<string, string>): Promise<void> { this.#deny(); }
  async runSource(_requestId: string): Promise<{jobId: string; created: boolean}> { this.#deny(); }
  async configureIntegration(_requestId: string, _values: Record<string, string>): Promise<void> { this.#deny(); }
  async testIntegration(_requestId: string): Promise<IntegrationVerification> { this.#deny(); }
  ...
  async uploadChatAttachment(_attachment: ChatAttachmentUpload, _modelId: string | null): Promise<ChatAttachmentHandle> { this.#deny(); }
  ...
  async subscribeToConsoleLogs(_subscriber: RpcStub<ConsoleLogSubscriber>): Promise<RpcStub<{}>> { this.#deny(); }
  async updateBlueprint(_blueprintId: string, _options: {
    title?: string; description?: string; updateCode?: boolean; updateBindings?: boolean;
  ...
  async listObserverRequirements(_role: CollaboratorRole): Promise<ObserverBindingNeed[]> { return []; }
  async listCollaborators(): Promise<CollaboratorInfo[]> { return []; }
  async addCollaborator(_username: string, _role: CollaboratorRole,
                        _note?: string): Promise<CollaboratorInfo | null> { this.#deny(); }
  async removeCollaborator(_profileId: string, _keepUsers: string[]): Promise<AffectedCollaborator[]> { this.#deny(); }
  async previewRemoveCollaborator(_profileId: string): Promise<AffectedCollaborator[]> { this.#deny(); }
  async createShareLink(_role: CollaboratorRole, _note?: string): Promise<{key: string; linkId: string}> { this.#deny(); }
  async newShareLinkKey(_linkId: string): Promise<{key: string}> { this.#deny(); }
  async listShareLinks(): Promise<ShareLinkInfo[]> { return []; }
  async updateShareLink(_linkId: string, _note?: string): Promise<void> { this.#deny(); }
  async revokeShareLink(_linkId: string, _keepUsers: string[]): Promise<AffectedCollaborator[]> { this.#deny(); }
