...
export type AiChatAgentContext = {
  ...
  // Initial `env` binding set gathered when this chat was started, typically including all workspaces
  // and all gatekeepers which those workspaces bind to, but the contents may be different depending
...
export type SeedBindingInfo = {
  ...
  // Human title of the target (a workspace's title, or a gatekeeper's resource title).
  title: string;

  // Whether the target is a workspace (vs. an external resource gatekeeper).
  isVessel: boolean;
...
// Either a workpiece (a workspace or gatekeeper -- the overseer distinguishes at env-build time) or
...
export type CompactionCheckpoint = {
  ...
  //
  // Provisional workspace creations and binding additions from before the boundary are deliberately
  // absent: they carry no Y.Doc update, and the registry rows they created (`WorkspaceRecord.pending`,
...
// Summary of one of the workspace's workspaces, as needed by the agent: identity, the name of the
// Y.Doc root map holding its files, and its named bindings. See AgentHooks.listVesselInfo().
export type AgentVesselInfo = {
  id: WorkpieceId;
  title: string;
  rootName: string;
  // Whether this is the workspace's default workspace: the workspace that tools operate on when their
...
export interface AgentHooks {
  ...
  // means the workspace's default workspace; throws an agent-readable error if there is none. When
  ...
  // the "changes" message that records the creation (see WorkspaceRecord.pending in overseer.ts).
  ...
  // bindings the blueprint's code expects the agent to wire up. Used by the createWorkpiece tool to
...
let SYSTEM_PROMPT = `
You are the agent inside Verglas OS, an agentic data-lake environment that helps a company connect its knowledge, build repeatable jobs, model relationships and evidence, and create interactive applications over its lakehouse.

Use only capabilities actually present in the workspace. Inspect tables, vectors, graphs, queues, KV namespaces, workers, and reflected Integration namespaces before relying on them. Preserve provenance and never fabricate live or historical records.

# Runtime model

Verglas Vessels compose three generated workload surfaces into one versioned release:

* Integrations are standalone API containers that connect to external systems. Create one with createIntegration when a reusable API client, OAuth callback, WebSocket feed, or external service adapter is needed. Its setup schema must ask only for values the user must supply, credentials must never be embedded in source, and readiness requires a real verification call.
* Jobs are stateless Verglas TypeScript workers invoked by manual, cron, webhook, or CloudEvent triggers. Create one with createSource when external data must be ingested or normalized into lakehouse tables. Jobs own movement and transformation; they do not host application UIs.
* Applications are standalone TypeScript HTTP containers. Create one with createApplication for every dashboard, map, form, monitor, interactive report, or workflow control surface. Each Application supplies a complete project with package.json, source files, build script when needed, and any exact npm dependencies it requires. Verglas builds it into its own OCI image and returns a local preview URL.

When a request needs more than one surface, prefer createIntegration and createApplication (and createSource / Jobs as needed) as separate tool calls, then compose with createVessel only when the projects are already small and complete. Huge single-tool createVessel payloads are unreliable through subscription model runtimes. Every component and the Vessel itself require independent versions. The runtime validates and builds the complete release before reconciling it, and Jobs created from Worker definitions remain owned by that exact Vessel release.

**Stepwise activation:** After createIntegration leaves an Integration in needs_configuration, end the turn — the same way requestConnection waits for the user. You may create multiple Integrations in that turn, but do not call createApplication, createSource, or createVessel until every required Integration for the plan is verified (ready). When the user Save-and-tests (from the chat card or the Integrations page), you are resumed: check Integration readiness via history or inspectIntegration, then proceed to Apps and Jobs.

Do not use legacy embedded-application runtimes or workspace file tools to build an Application. Build the complete standalone project with createApplication. Keep each tool payload modest: a few source files per call beats one megabyte JSON blob.

# Applications

An Application may use React, Vite, deck.gl, MapLibre, charting systems, or any other declared npm dependency. It must:

* include package.json with scripts.start and exact dependency versions;
* place TypeScript or TSX source under src/;
...
If package.json defines scripts.build, Verglas runs it while building the image. Browser code can use bundled dependencies normally. Never load executable code from a CDN and never put lakehouse credentials into browser assets.

# Integrations and Jobs

When no installed Integration exposes the external API the user needs, createIntegration from scratch. Include exact setup instructions and only necessary configuration fields. Implement a real verify(ctx) that returns structured details. A reflected api has title, description, and methods; every method has mode (read, write, or stream), description, and handler. Use normalized CloudEvents for handoff to Jobs and durable queues between workflow stages. Never embed credentials or claim success before verification. When the user fails Save and test, you are resumed: call inspectIntegration, fix with updateIntegration, then testIntegration — do not ask them to run docker or paste container logs.

Generated Job modules run with Bun and default-export defineWorker imported from "/sdks/typescript/src/index.ts". Implement async handler(ctx), use ctx.client for lakehouse operations, and make writes idempotent. For ingestion, create or verify the target table before appending. Webhook jobs receive an org.verglas.http.request CloudEvent.

# Workspace analysis

Do not create Workspaces, edit Workspace files, or call executeCode — those legacy Dynamic Worker paths are gone. Prefer createApplication for UIs, createSource for Jobs, and createIntegration for external APIs. Durable data belongs in Verglas tables, vectors, graphs, queues, or KV—not in an Application container filesystem.
`.trim();

let SPAWNER_SYSTEM_PROMPT = `
You are a Verglas job agent started to perform one bounded task inside a company knowledge workspace. Use the supplied scoped Verglas SDK and Integration namespace capabilities to inspect or update authorized data. Prefer idempotent, observable operations and return a concise result. Do not create application runtime artifacts from a job.
`.trim();

let READ_FILE_TOOL_DESCRIPTION = `
Read the content of a file owned by one of the workspace's workspaces. Note that you will be informed any time a file changes, so it is not necessary to read a file again after you have already read it once. This cannot read chat attachments; attachments are provided directly in the conversation.
`.trim();

let LIST_BLUEPRINTS_TOOL_DESCRIPTION = `
List the blueprints available to the user: their own published blueprints, their blueprint library, and this deployment's featured blueprints. Blueprints are legacy Workspace snapshots and cannot be instantiated. Prefer createApplication / createSource instead.
`.trim();
...
\`webFetch\` is an agent research tool. Application and Integration network access is governed by their own Vessel runtime policy and declared capabilities.
...
By default, supported document responses are converted to Markdown for readability. Plain text, JSON, and unknown content types are returned as-is. Pass \`raw: true\` to receive the exact bytes the server sent.
...
Do not guess an Integration API from prior knowledge. Use this tool to read its reflected methods and schemas before invoking it.
`.trim();

let SET_VESSEL_BINDING_TOOL_DESCRIPTION = `
Wire a resource from your \`env\` into a Workspace's own \`env\`, so the Workspace's code can use it.

The bindings in your \`env\` belong to this chat; a Workspace's code sees only the Workspace's own bindings, which are listed in the system prompt. Use this tool to add one of your bindings to a Workspace: \`workspace\` names the target Workspace (by its name in your env), \`source\` names the resource binding to wire in, and \`name\` is the name the Workspace's code will see it as (\`env.<name>\` in server.js), defaulting to the same name as \`source\`.
...
Create a Verglas TypeScript Job that ingests or transforms data. Use this for a manual, cron, webhook, or CloudEvent-triggered operation.

Provide a complete worker module that default-exports defineWorker(), the output table, triggers, and only the configuration values the user must supply. Secret values become scheduler-owned bindings and never enter source or chat storage. The handler uses ctx.client, creates or verifies its output table, writes idempotently, and returns rowsWritten for run observability.
`.trim();

let CREATE_INTEGRATION_TOOL_DESCRIPTION = `
Create an isolated Integration Vessel for a long-lived external API client, WebSocket feed, OAuth callback service, or private API that Jobs and Applications can reuse.

Provide executable source, concrete setup instructions, and only the configuration values the user must supply. Implement a real verify(ctx) call and a declared reflected API. Credentials stay in ctx.config; never embed or return them. Readiness requires successful live verification.

verify(ctx) must return { ok, message, details } where details is a bounded map of non-secret diagnostic facts (e.g. wsOpen, subscribed, firstMessageType, closeCode, error, timeoutMs). Treat auth and malformed-subscription errors as immediate failures — do not wait out a silence timeout. On timeout, set details explaining what succeeded (open/subscribe) versus what never arrived.

When this Integration needs user secrets (\`needs_configuration\`), your turn ends after the tool — like requestConnection. Do not create Applications, Jobs, or compositional Vessels until the user verifies those Integrations. On resume after Save and test (success or failure): if failed, call inspectIntegration, fix with updateIntegration, then testIntegration; if ready, continue the plan.
`.trim();

let INSPECT_INTEGRATION_TOOL_DESCRIPTION = `
Inspect a generated Integration in this chat by requestId. Returns stored state, last verification (including details), HTTP health, and runtime status. Use this when verification fails or the Integration misbehaves. Never expects or returns secrets.
`.trim();

let TEST_INTEGRATION_TOOL_DESCRIPTION = `
Re-run live verification for a generated Integration without changing its saved configuration. Returns { ok, message, details, latencyMs }. On failure, read details and patch with updateIntegration rather than asking the user to debug containers.
`.trim();

let UPDATE_INTEGRATION_TOOL_DESCRIPTION = `
Redeploy a fixed Integration module onto the same Vessel (same requestId / vesselName). Use after inspectIntegration / testIntegration shows a verify or runtime bug. You may also refresh instructions or configFields. Credentials already saved by the user stay in runtime KV; do not ask them to re-paste unless fields changed.
`.trim();

let CREATE_APPLICATION_TOOL_DESCRIPTION = `
Build a standalone TypeScript Application Vessel. Use this whenever the user asks for an interactive application, dashboard, form, map, monitor, or workflow control surface.
...
Build and apply one complete, versioned Verglas Vessel. Prefer createIntegration and createApplication first for large UIs (maps, dashboards, multi-file React apps), then call createVessel with already-small project payloads. Avoid stuffing an entire multi-file Application into one createVessel call when the model runtime is a subscription CLI.

The manifest must use the canonical flat schema below. Do not use Kubernetes-style metadata or spec wrappers. Configuration field help uses \`help\`, not \`description\`; setup is an array inside \`config\`; Worker triggers use keys such as \`event\`, \`cron\`, or \`table\`; and the Interface must declare its port.
...
integrations:
  - name: external-api
    version: 1.0.0
    project: integrations/external-api
    config:
      fields:
        - name: API_TOKEN
          label: API token
          type: secret
          required: true
          help: Create a read-only token in the provider console.
      setup:
        - title: Create an API token
          description: Generate a read-only token and paste it above.
...
It contains zero or more independently versioned integrations, zero or more independently versioned workers, and exactly one independently versioned interface. Each component names a relative project path, and projects supplies the complete files for every referenced path with no extras.

Integration projects include package.json and src/integration.ts. package.json must start vendor/verglas-integration-runtime/runtime.mjs and may declare exact npm dependencies. src/integration.ts default-exports the reflected api, verify(ctx), and optional start(ctx) or fetch(request, ctx). The api requires title, description, and methods; each method requires mode (read, write, or stream), description, and handler. Integration configuration fields and setup steps belong in the manifest; never put credential values in files.

Worker projects include src/worker.ts using the Verglas defineWorker contract. Interface projects are standalone full-stack TypeScript applications with package.json, server-side @verglas/sdk access, GET /health, and the requested graphical UI. The runtime returns the local preview and creates configuration cards for Integrations requiring user input.
`.trim();
...
export async function runAgent(
  ...
  // enumeration source of truth for which Y.Doc roots hold workspace files (roots of workspaces
  // deleted from the registry are inert). A workspace created mid-turn (via createWorkpiece) isn't
  // in this snapshot, but nothing here needs it: the system prompt was already built, and
  // replayed "changes" messages predate it.
  let vesselInfos = hooks.listVesselInfo(chatId);

  // On first use, we'll build a copy of the Y.Doc, then reuse it for further tool calls in
  // this session. Each workspace's files live in the doc's root map named by
  // AgentVesselInfo.rootName; file tools resolve their optional `workpiece` parameter to a root
  // via hooks.resolveWorkpieceRoot.
  let ydoc: Y.Doc | undefined;
  let versionLock = checkpoint?.observedCodeVersion;
  let capturedYdocChanges: Uint8Array[] = [];
  // Workspaces created this turn, awaiting attachment to the next flushed "changes" message (see
  // flushCapturedYdocChanges and the createWorkpiece tool) -- which is what durably records, and
  ...
  // during history replay (pasted resources, accepted connections, created workspaces, agent
  ...
  let applyReplayedChanges = (update: Uint8Array, includeDiff: boolean): string | undefined => {
    ...
    // Diffs are grouped by workspace: each workspace with changes contributes a heading line naming it
    ...
    for (let {info, files, touchedFiles} of observed) {
      ...
      // A workspace with no in-scope binding gets no diff output: the agent can't reference it, so
  ...
  // Same idea for workspace creations, but exact and order-immune: a creation is durably recorded
  // iff some "changes" message lists it in `createdVessels` -- possibly even *before* the tool
  ...
  // Unlike creations, additions have no unique id: (workspaceId, name) can legitimately recur when
    ...
    switch (msg.type) {
      ...
      case "changes": {
        ...
        if (chatMessageStatus[msg.sequence - firstSequence] !== "reverted") {
          ...
          if (msg.author.type === "user") {
            ...
            // workspaces they created and bindings they added from the workspace UI
      ...
      case "integrationConfiguration": {
        const status = msg.state === "ready"
            ...
            : msg.state === "error"
              ...
              : "awaiting user setup and live verification";
        const detailParts: string[] = [
          `Generated Integration "${msg.title}" (${msg.requestId}, vessel ${msg.vesselName}) is ${status}.`,
        ];
      ...
      case "applicationPreview": {
        modelMessages.push({
          role: "user",
          content:
            `Application "${msg.title}" (${msg.vesselName}) is available` +
            (msg.previewUrl ? ` at ${msg.previewUrl}` : " (deleted)") +
            `. ${msg.description}`,
          timestamp: msgTimestamp,
        });
        break;
  ...
  // Likewise, re-adopt workspace creations and binding additions from a crashed turn that were
  ...
  // spares them because their tool calls appear in the log (see reconcilePendingVessels in
  // overseer.ts).
  for (let creation of replayedCreations) {
    if (!recordedCreations.has(creation.workspaceId)) {
      pendingCreatedVessels.push(creation);
    }
  }
  let seenAdditionCounts = new Map<string, number>();
  for (let addition of replayedBindingAdditions) {
    let key = bindingAdditionKey(addition.workspaceId, addition.name);
  ...
  let integrationsAwaitingActivation = false;
  ...
  let flushCapturedYdocChanges = () => {
    ...
    let update = capturedYdocChanges.length > 0
        ...
        : undefined;
    capturedYdocChanges = [];
    let createdVessels = pendingCreatedVessels;
  ...
  if (agentContext.spawnerConfig) {
    ...
    // Let's include each workspace's list of files in the system prompt so that the agent doesn't
    ...
    if (vesselInfos.length == 0) {
      ...
      systemPromptWorkspace = `# This workspace's workspaces\n\n${sections.join("\n\n")}`;
    }

    // Standalone Applications do not use the legacy blueprint archive.
    let standardFormats = "";
  ...
  let tools: Record<string, AgentTool> = {
    ...
    setVesselBinding: defineTool({
      name: "setVesselBinding",
      label: "Bind resource to workspace",
    ...
    createIntegration: defineTool({
      ...
      execute: async (toolCallId, input) => {
        try {
          const output = await hooks.createIntegration(chatId, input);
    ...
    createVessel: defineTool({
      ...
      execute: async (toolCallId, input) => {
        try {
          refuseUntilIntegrationsReady();
          const output = await hooks.createVessel(chatId, input);
  ...
  if (agentContext.spawnerConfig) {
    ...
    tools = {
      ...
      describeBinding: tools.describeBinding,
      observeUserChanges: tools.observeUserChanges,
      createSource: tools.createSource,
      createIntegration: tools.createIntegration,
      inspectIntegration: tools.inspectIntegration,
      testIntegration: tools.testIntegration,
      updateIntegration: tools.updateIntegration,
      createApplication: tools.createApplication,
      createVessel: tools.createVessel,
    };
  }
