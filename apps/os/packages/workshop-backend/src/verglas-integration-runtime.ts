...
export function validateGeneratedIntegrationModule(module: string): void {
  if (module.length === 0 || module.length > 256 * 1024) {
    throw new Error(
      "An Integration module must contain between 1 byte and 256 KiB.",
    );
  }
  if (!/export\s+default\s+\{/.test(module)) {
    throw new Error(
      "An Integration module must default-export an object literal.",
    );
  }
  ...
  if (!/\b(?:start|fetch)\s*\(/.test(module)) {
    throw new Error(
      "An Integration module must implement start(ctx) or fetch(request, ctx).",
    );
  }
  if (/\b(?:eval|Function)\s*\(/.test(module)) {
    throw new Error(
      "Generated Integration modules cannot evaluate additional source text.",
    );
  }
}
...
export class VerglasIntegrationRuntimeClient {
  ...
  constructor(
    ...
    this.#image =
      env.VERGLAS_INTEGRATION_RUNTIME_IMAGE?.trim() ||
  ...
  async deploy(deployment: GeneratedIntegrationDeployment): Promise<void> {
    ...
    await this.#runtime.putVessel(deployment.name, {
      ...
      environment: {
        ...
        VERGLAS_INTEGRATION_DEFINITION: encodeBase64(
  ...
  async deployApplication(
    deployment: GeneratedApplicationDeployment,
  ): Promise<string> {
  ...
  async deployVessel(
    deployment: GeneratedVesselDeployment,
  ): Promise<AppliedVesselResult> {
  ...
  async configure(
    ...
    return await verificationResult(
      response,
      "configure and verify Integration",
    );
  }
  ...
  async #waitForHealth(name: string): Promise<void> {
    ...
    throw new Error(
      `Integration Vessel did not become healthy; last HTTP status was ${lastStatus}.`,
    );
  }
}

/** Enforces the minimum standalone project contract before invoking a Docker build. */
export function validateApplicationProject(
  files: Record<string, string>,
): void {
  const packageJson = files["package.json"];
  if (!packageJson)
    throw new Error("An Application project must include package.json.");
  let packageDefinition: { scripts?: { start?: unknown } };
  try {
    packageDefinition = JSON.parse(packageJson) as {
  ...
  if (!Object.keys(files).some((path) => /^src\/.*\.(?:ts|tsx)$/.test(path))) {
    throw new Error(
      "An Application project must include TypeScript source under src/.",
    );
  }
}
...
export function parseIntegrationVerificationResult(
  ...
  const failed: IntegrationVerification =
...
async function expectSuccess(
  ...
  throw new Error(
    `Verglas failed to ${operation}: HTTP ${response.status}${detail ? ` — ${detail}` : ""}`,
  );
}
