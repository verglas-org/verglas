...
describe("VerglasIntegrationRuntimeClient", () => {
  ...
  it("deploys source and setup metadata into an isolated Integration Vessel", async () => {
    ...
    await new VerglasIntegrationRuntimeClient(
      ...
      module,
      instructions: [
        { title: "Create a key", description: "Register for a free key." },
      ],
      fields: [
    ...
    expect(body.environment.VERGLAS_INTEGRATION_MODULE).not.toContain(
      "API_KEY",
    );
  });
  ...
  it("returns only a passing live verification result", async () => {
    ...
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(Response.json({ configured: true, verification }));

    await expect(
      new VerglasIntegrationRuntimeClient(env, fetcher, "data-token").configure(
        "ais",
        { API_KEY: "secret" },
      ),
  ...
  it("preserves structured verification details on failure", async () => {
    ...
    const fetcher = vi
      ...
      .mockImplementation(() =>
        Promise.resolve(
          Response.json(
            { configured: false, verification, error: verification.message },
            { status: 422 },
          ),
        ),
      );
    ...
    try {
      ...
      expect((error as IntegrationVerificationFailed).verification).toEqual(
        verification,
      );
    }
  });
  ...
  it("parseIntegrationVerificationResult keeps details when ok is false", () => {
    ...
    ).toThrow(IntegrationVerificationFailed);
    ...
    try {
      ...
      expect(
        (error as IntegrationVerificationFailed).verification.details,
      ).toEqual({ wsOpen: true });
    }
  });
  ...
  it("builds an Application project with declared npm dependencies", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response("{}"));
    const previewUrl = await new VerglasIntegrationRuntimeClient(
      env,
      fetcher,
      "data-token",
    ).deployApplication({
  ...
  it("applies a complete compositional Vessel through one runtime request", async () => {
    ...
    const client = new VerglasIntegrationRuntimeClient(
      env,
      fetcher,
      "data-token",
    );
    ...
    expect(result.previewUrl).toBe(
      "http://runtime:8360/apps/hormuz-tracker-map/",
    );
    const [url, init] = fetcher.mock.calls[0];
    expect(url).toBe(
      "http://runtime:8360/v1/vessels/hormuz-tracker/composition",
    );
    const body = JSON.parse(String(init?.body));
  ...
  it("treats a missing Vessel as already deleted", async () => {
    ...
    await expect(
      new VerglasIntegrationRuntimeClient(
        env,
        fetcher,
        "data-token",
      ).deleteVessel("gone"),
