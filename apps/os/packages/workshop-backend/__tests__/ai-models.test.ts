...
import type { AiChatAuthorInfo, AiModelConfig } from "@verglas/workshop-shared/api";
...
describe("getModel direct routing", () => {
  ...
  it("routes native CLIs through the deployment adapter with chat-scoped state", async () => {
    const handle = getModel(env({
    ...
    }, INITIATOR, { sessionAffinity: "chat-session" });

    expect(handle.model.api).toBe("openai-completions");
    expect(handle.model.baseUrl).toBe("http://127.0.0.1:8790/v1");

    const request = await captureRequest(handle);
    expect(request.url).toBe("http://127.0.0.1:8790/v1/chat/completions");
    expect(request.headers.get("authorization")).toBe("Bearer adapter-token");
    expect(request.headers.get("x-runtime-session-key")).toBe("workshop:chat-session");
    expect(request.headers.get("x-model-runtime")).toBe("codex");
    expect(JSON.parse(request.body)).toMatchObject({model: "gpt-5.6-terra"});
  }, 15000);

  it("keeps adapter auth deployment-owned and forwards an optional Cursor API key", async () => {
    const handle = getModel(env({
      ...
      runtime: "cursor",
      model: "auto",
      apiToken: "cursor-api-key",
    }, INITIATOR);

    const request = await captureRequest(handle);
    expect(request.headers.get("authorization")).toBe("Bearer deployment-token");
    expect(request.headers.get("x-model-runtime")).toBe("cursor");
    expect(request.headers.get("x-provider-api-key")).toBe("cursor-api-key");
  }, 15000);
  ...
  it("requires the deployment-owned native adapter configuration", () => {
    expect(() => getModel(env(), {
      ...
      model: "codex",
      apiToken: "",
    }, INITIATOR)).toThrow("native model runtime adapter is not configured");
  });
