...
describe("compaction boundary", () => {
  ...
  it("protects a pending connection request but not a provisional creation", () => {
    let creation = [
      ...
      record(1, agent, {
        type: "changes", createdVessels: [{workspaceId: 2, title: "New", bindingName: "NEW"}],
      }),
...
describe("compaction checkpoint state", () => {
  ...
  it("keeps a provisional creation's binding name without inventing an update", () => {
    let state = buildState([
      record(0, agent, {
        type: "changes",
        createdVessels: [{workspaceId: 2, title: "New", bindingName: "NEW"}],
        addedBindings: [{workspaceId: 2, name: "DB", target: 9}],
      }),
    ], 1);
