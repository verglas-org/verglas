...
describe("VerglasCatalogClient", () => {
  it("projects active worker rows without returning executable configuration", async () => {
    ...
    const result = await new VerglasCatalogClient(
      env,
      fetcher,
      "access-secret",
    ).listWorkers();
  ...
  it("discovers databases as resources and scopes Iceberg tables to their lakehouse", async () => {
    const fetcher = vi.fn<typeof fetch>(async (input) => {
      ...
      if (
        url.endsWith(
          "/v1/databases/analytics/catalog/v1/analytics-warehouse/namespaces",
        )
      ) {
      ...
      if (
        url.endsWith(
          "/v1/databases/analytics/catalog/v1/analytics-warehouse/namespaces/events/tables",
        )
      ) {
        return Response.json({
          identifiers: [{ namespace: ["events"], name: "log" }],
        });
      }
      if (
        url.endsWith(
          "/v1/databases/analytics/catalog/v1/analytics-warehouse/namespaces/knowledge/tables",
        )
      ) {
        return Response.json({
          identifiers: [
    ...
    await expect(
      new VerglasCatalogClient(env, fetcher, "access-secret").getCatalog(),
  ...
  it("creates database resources and manages tables through the selected database catalog", async () => {
    const fetcher = vi.fn<typeof fetch>(async (input) => {
      const url = String(input);
      if (url.endsWith("/v1/databases/analytics"))
        return Response.json({
          type: "lakehouse",
          name: "analytics",
          storage: { mode: "managed" },
      ...
      if (
        url.endsWith("/v1/databases") &&
        !input.toString().endsWith("/analytics")
      ) {
        return Response.json(
          {
            type: "lakehouse",
  ...
  it("rejects deleting a non-empty lakehouse before calling the access service delete", async () => {
    ...
    await expect(
      new VerglasCatalogClient(env, fetcher, "access-secret").deleteDatabase(
        "analytics",
      ),
    ...
    expect(
      fetcher.mock.calls.some(
        ([input, init]) =>
          String(input).endsWith("/v1/databases/analytics") &&
          init?.method === "DELETE",
      ),
    ).toBe(false);
  });
  ...
  it("wraps read SQL with a limit on the selected Lakehouse query route", async () => {
    const fetcher = vi.fn<typeof fetch>(async (input) => {
      if (String(input).endsWith("/v1/databases/analytics"))
        return Response.json({
          type: "lakehouse",
          name: "analytics",
          storage: { mode: "managed" },
    ...
    expect(fetcher).toHaveBeenCalledWith(
      "http://localhost:8334/v1/databases/analytics/query",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          sql: "SELECT * FROM (SELECT value FROM numbers) AS __verglas_query LIMIT 3",
        }),
      }),
    );
    expect(result).toEqual({
  ...
  it("rejects Postgres queries without probing the data service", async () => {
    ...
    await expect(
      new VerglasCatalogClient(env, fetcher, "access-secret").query(
        "operations",
        "SELECT 1",
      ),
  ...
  it("persists lifecycle state only for Application Vessels", async () => {
    ...
    await new VerglasCatalogClient(
      env,
      fetcher,
      "access-secret",
    ).setApplicationState("dashboard", "stopped");

    expect(
      fetcher.mock.calls.map(([input, init]) => [String(input), init?.method]),
    ).toEqual([
  ...
  it("combines an Integration Vessel's schema with its configured state", async () => {
    ...
    await expect(
      new VerglasCatalogClient(
        env,
        fetcher,
        "access-secret",
      ).getIntegrationConfiguration("linear"),
