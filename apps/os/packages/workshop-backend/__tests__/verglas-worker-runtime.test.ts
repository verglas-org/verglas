...
describe("VerglasWorkerRuntimeClient", () => {
  it("keeps secret values out of the worker declaration", async () => {
    ...
    await client.register({
      ...
      config: JSON.stringify({
        env: { API_TOKEN: "@secret:source.API_TOKEN" },
      }),
      created_by: "test",
    });
  ...
  it("sends a fresh idempotency key when manually running a Source", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(JSON.stringify({ job_id: "job-1", created: true }), {
        status: 202,
    ...
    await expect(client.run("source", "request-1")).resolves.toEqual({
      job_id: "job-1",
      created: true,
    });
...
describe("validateVerglasWorkerModule", () => {
  ...
  it("rejects the obsolete run method", () => {
    expect(() =>
      validateVerglasWorkerModule(valid.replace("handler", "run")),
    ).toThrow(/handler\(ctx\)/);
  });
  ...
  it("rejects treating ctx.output as a table object", () => {
    expect(() =>
      validateVerglasWorkerModule(
        valid.replace(
          "ctx.client.table(ctx.output).append",
          "ctx.output.append",
        ),
      ),
