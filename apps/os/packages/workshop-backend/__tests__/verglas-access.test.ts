...
describe("VerglasAccessClient", () => {
  it("requires an endpoint and identity assertion key without accepting a service token", () => {
    ...
    expect(() =>
      resolveVerglasAccessConfig({ VERGLAS_ACCESS_URI: "http://access:8345" }),
  ...
  it("exchanges a signed OS identity assertion before making actor-free access requests", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (path === "/v1/access/sessions") {
        ...
        return Response.json({
          token: `${audience}-session-token`,
          expires_at: claims.exp + 840,
        });
      }
      expect(new Headers(init?.headers).get("authorization")).toBe(
        "Bearer access-session-token",
      );
    ...
    const access = new VerglasAccessClient(
      config!,
      "Person@Example.com",
      fetcher,
    );
    await expect(access.identity()).resolves.toEqual({
      tenantId: "tenant-a",
      principalId: "user/person@example.com",
      tenantOwner: true,
      policyVersion: 4,
    });
    await expect(access.sessionToken("data-plane")).resolves.toBe(
      "data-plane-session-token",
    );
    await expect(access.sessionToken("access")).resolves.toBe(
  ...
  it("creates, lists, and revokes tokens without accepting an actor principal", async () => {
    const fetcher = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (
        url.pathname === "/v1/access/tokens/token%2Fcli" &&
        init?.method === "DELETE"
      ) {
    ...
    await expect(access.listTokens()).resolves.toEqual([
      expect.objectContaining({ id: "token/cli" }),
    ]);
  ...
  it("derives the actor and tenant instead of accepting them in delegation bodies", async () => {
    ...
    await expect(
      new VerglasAccessClient(config, "owner@example.com", fetcher).delegate({
  ...
  it("reuses an existing grant that already covers the requested delegation", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>(async (input, init) => {
      ...
      return new Response("duplicate delegation must not be posted", {
        status: 500,
      });
    });
    const config = resolveVerglasAccessConfig({
  ...
  it("replaces a partial grant with the union of its existing and requested actions", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (url.pathname === "/v1/access/revocations")
        return new Response(null, { status: 204 });
    ...
    expect(requests).toEqual([
      ...
      },
      {
        ...
        body: {
          grant: expect.objectContaining({ actions: ["discover", "describe"] }),
        },
      },
    ]);
  });
