...
const port = Number.parseInt(process.env.LOCAL_MODEL_RUNTIME_PORT || "8790", 10);
  ...
  "claude-code": {
    ...
    linkedDetail: output => {
      try {
        ...
        return status.email ? `Signed in as ${status.email}.` : "Signed in with Claude Code.";
      } catch { return "Signed in with Claude Code."; }
    },
  },
  cursor: {
    ...
    linkedDetail: output => output.replace(/^\s*[✓✔]\s*/u, "").trim() || "Signed in with Cursor.",
  },
};
...
function runCli(executable, args, input, env = process.env) {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", code => {
      ...
      else reject(new Error(stderr.trim() || stdout.trim() || `${executable} exited with code ${code}.`));
    });
...
async function invokeRuntime(runtimeId, modelId, body, providerApiKey) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    let output;
    if (runtimeId === "codex") {
      output = await runCli("codex", [
        "exec", "--ignore-user-config", "--ignore-rules", "--ephemeral", "--skip-git-repo-check",
        "--sandbox", "read-only", "--color", "never",
        "--model", modelId, "--output-schema", schemaPath, "-",
      ], prompt);
    } else if (runtimeId === "claude-code") {
      output = await runCli("claude", [
        "-p", "--model", modelId, "--tools", "", "--permission-mode", "bypassPermissions",
        "--strict-mcp-config", "--output-format", "json", "--json-schema", JSON.stringify(schema),
      ], prompt);
      ...
      output = await runCli(
        ...
        prompt,
        providerApiKey ? { ...process.env, CURSOR_API_KEY: providerApiKey } : process.env,
      );
    } else {
  ...
  return {
    ...
    choices: [{
      ...
      message: {
        ...
        ...(toolCalls ? { tool_calls: toolCalls.map(({ index: _index, ...call }) => call) } : {}),
      },
...
async function inspectRuntime(id) {
  ...
  try {
    const { stdout, stderr } = await execFileAsync(runtime.executable, runtime.statusArgs, {
      timeout: 10_000,
      maxBuffer: 256 * 1024,
    });
    ...
    return {
      ...
      available: true,
      linked,
      detail: linked ? runtime.linkedDetail(output) : `${runtime.executable} is installed but not signed in.`,
    ...
    return {
      ...
      detail: output || `${runtime.executable} login status could not be determined.`,
      supportsGuidedLogin: true,
    };
  }
}
...
function startLogin(id, sessionId) {
  ...
  if (loginSessions.has(sessionId)) throw new Error("Login session already exists.");

  const child = spawn(runtime.executable, runtime.loginArgs, {
    env: process.env,
    stdio: ["ignore", "pipe", "pipe"],
  });
  const session = { child, runtimeId: id, output: "", done: false, error: undefined };
  ...
  child.on("exit", code => {
    if (code && !session.error) session.error = session.output.trim() || `Login exited with code ${code}.`;
    session.done = true;
  });
  return session;
}
...
async function loginResult(sessionId) {
  ...
  if (session.done) {
    loginSessions.delete(sessionId);
    return { sessionId, done: true, status: "error", error: session.error || status.detail };
  }
  const externalUrl = session.output.match(/https:\/\/\S+/)?.[0].split(String.fromCharCode(27))[0];
  const deviceCode = id === "codex"
  ...
  return {
    ...
    step: {
      ...
      message: session.output.trim() || "Complete the sign-in flow in the browser, then continue.",
...
async function verifyRuntime(runtimeId, modelId, providerApiKey) {
  ...
  const result = await invokeRuntime(runtimeId, modelId, {
    model: modelId,
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === "GET" && url.pathname === "/v1/runtimes") {
      const statuses = await Promise.all(Object.keys(runtimes).map(inspectRuntime));
      sendJson(response, 200, { runtimes: statuses });
      return;
    }
    const modelsMatch = url.pathname.match(/^\/v1\/runtimes\/([^/]+)\/models$/);
    if ((request.method === "GET" || request.method === "POST") && modelsMatch) {
      ...
      sendJson(response, 200, {
        models: await discoverRuntimeModels(
          id, typeof body.apiToken === "string" ? body.apiToken : undefined,
        ),
      });
      return;
    }
    if (request.method === "POST" && url.pathname === "/v1/chat/completions") {
      ...
      const result = await invokeRuntime(
        runtimeId, body.model, body, request.headers["x-provider-api-key"],
      );
    ...
    if (request.method === "POST" && verifyMatch) {
      ...
      await verifyRuntime(id, modelId, typeof body.apiToken === "string" ? body.apiToken : undefined);
      sendJson(response, 200, { ok: true });
      return;
    }
...
server.listen(port, host, () => {
  console.log(`Native model runtime adapter listening on http://${host}:${port}`);
});
