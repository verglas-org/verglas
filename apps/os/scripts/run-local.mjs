...
// Single command to try out the Verglas OS locally:
//
//   1. Installs dependencies (if needed).
//   2. Builds only what's required to *run* the app:
//        - @verglas/typed-storage  (the backend imports it via its built `dist` output)
...
if (needsBuild) {
  ...
  run("pnpm", ["--filter", "@verglas/typed-storage", "build"]);
  run("pnpm", ["--filter", "@verglas/workshop-frontend", "exec", "vite", "build"]);
