...
function requireLineageExplorationWeight(value) {
  if (!Number.isFinite(value) || value < 0) {
    throw new Error(
      "lineageExplorationWeight must be a finite, non-negative number.",
    );
  }
  return value;
}
...
function proposal(parent, operation, ordinal, debugDepth, lineageId = null) {
...
function lineages(candidates) {
  const candidateById = new Map(
    candidates.map((candidate) => [candidate.id, candidate]),
  );
  const lineageByCandidateId = new Map();
  const arms = new Map();

  for (const candidate of candidates) {
    if (
      candidate.operation === "draft" &&
      candidateById.get(candidate.parentId)?.operation === "root"
    ) {
      lineageByCandidateId.set(candidate.id, candidate.id);
      arms.set(candidate.id, []);
    }
  }
  for (const candidate of candidates) {
    let current = candidate;
    while (current && !lineageByCandidateId.has(current.id)) {
      current = current.parentId
        ? candidateById.get(current.parentId)
        : undefined;
    }
    const lineageId = current && lineageByCandidateId.get(current.id);
    if (lineageId) {
      lineageByCandidateId.set(candidate.id, lineageId);
      arms.get(lineageId).push(candidate);
    }
  }
  return { arms, lineageByCandidateId };
}

function boundedExplorationRange(score) {
  return Math.max(0, 1 - score);
}

function lineageUcb(arm, explorationWeight, totalVisits) {
  return (
    arm.parent.evaluation.score +
    explorationWeight *
      boundedExplorationRange(arm.parent.evaluation.score) *
      Math.sqrt(Math.log(totalVisits) / arm.members.length)
  );
}

function selectLineage(candidates, explorationWeight) {
  const { arms, lineageByCandidateId } = lineages(candidates);
  const viable = [...arms.entries()]
    .map(([lineageId, members]) => ({
      lineageId,
      members,
      parent: bestValid(members),
    }))
    .filter((arm) => arm.parent);
  if (viable.length === 0) {
    const parent = bestValid(candidates);
    return parent
      ? { parent, lineageId: lineageByCandidateId.get(parent.id) ?? null }
      : null;
  }
  ...
  const totalVisits = viable.reduce(
    (total, arm) => total + arm.members.length,
    0,
  );
  return viable.toSorted(
    (left, right) =>
      lineageUcb(right, explorationWeight, totalVisits) -
        lineageUcb(left, explorationWeight, totalVisits) ||
      left.members[0].ordinal - right.members[0].ordinal ||
      left.parent.ordinal - right.parent.ordinal,
  )[0];
}
...
export function selectProposalBatch(snapshot, config) {
  ...
  const lineageExplorationWeight = requireLineageExplorationWeight(
    ...
    return Object.freeze(
      Array.from({ length: count }, (_, index) =>
        proposal(
          ...
          0,
          `candidate-${candidates.length + index}`,
        ),
      ),
    );
  }
    ...
    return Object.freeze(
      Array.from({ length: capacity }, (_, index) =>
        proposal(
          ...
          candidates.length + index,
          buggy.debugDepth + 1,
          lineages(candidates).lineageByCandidateId.get(buggy.id) ?? null,
        ),
      ),
    );
  }

  const lineage = selectLineage(candidates, lineageExplorationWeight);
  if (!lineage) {
    return Object.freeze(
      Array.from({ length: capacity }, (_, index) =>
        proposal(
...
export function normalizeSearchConfig(config) {
  const normalized = {
    ...
    lineageExplorationWeight: requireLineageExplorationWeight(
      config.lineageExplorationWeight ?? 1,
    ),
  };
