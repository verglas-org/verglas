...
function requireContextOperation(operation) {
  if (!new Set(["draft", "debug", "improve"]).has(operation)) {
    throw new Error("context operation must be draft, debug, or improve.");
  }
  return operation;
}
...
/** Compiles an operator-specific, bounded decision context without solution bodies. */
export function compileCandidateContext(
  checkpoint,
  { candidateId, operation = "debug", maxCandidates = 8, maxArtifacts = 12 },
) {
  requireLimit(maxCandidates, "maxCandidates");
  requireLimit(maxArtifacts, "maxArtifacts");
  const contextOperation = requireContextOperation(operation);
  ...
  if (contextOperation === "draft") {
    for (const candidate of candidates.toReversed()) include(candidate);
  } else {
    includeAncestors();
    if (contextOperation === "improve") {
      for (const candidate of candidates) {
  ...
  if (contextOperation === "improve") {
    ...
    context.plateau = Object.freeze({
      scores: Object.freeze(scores),
      stalled:
        scores.length === 4 && scores.every((score) => score <= scores[0]),
    });
  }
