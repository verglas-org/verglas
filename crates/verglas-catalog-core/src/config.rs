...
fn get_config() -> DynAppConfig {
    ...
    if !config.openid_providers.is_empty() && config.openid_provider_uri.is_none() {
        tracing::warn!(
            "VERGLAS_CATALOG__OPENID_PROVIDERS is set but VERGLAS_CATALOG__OPENID_PROVIDER_URI is not. \
             API authentication will work, but the UI login button is disabled — the UI \
             redirects only to the primary provider. Set VERGLAS_CATALOG__OPENID_PROVIDER_URI to \
             enable UI login."
        );
    }
...
pub struct DynAppConfig {
    ...
    /// Catalog user ID (`kubernetes~<subject>`). Defaults to `uid`.
    ///
    ...
    /// `WriteData`). The privilege cannot be revoked from within Catalog at
    ...
    ///
    /// ```text
    /// VERGLAS_CATALOG__INSTANCE_ADMINS=["kubernetes~system:serviceaccount:lk:op","oidc~alice"]
    /// ```
    ///
    ...
    /// `VERGLAS_CATALOG__TASK_TABULAR_EXPIRATION_WORKERS` working.
...
/// Catalog user ID (`kubernetes~<subject>`).
...
mod test {
    ...
    fn test_external_authz_backend() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__AUTHZ_BACKEND", "my-authz");
            let config = get_config();
            assert_eq!(
    ...
    fn test_allow_all_authz_backend() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__AUTHZ_BACKEND", "allowall");
            let config = get_config();
            assert_eq!(config.authz_backend, AuthZBackend::AllowAll);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__AUTHZ_BACKEND", "allow-all");
            let config = get_config();
            assert_eq!(config.authz_backend, AuthZBackend::AllowAll);
            Ok(())
        });
    ...
    fn test_kubernetes_subject_source() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__KUBERNETES_AUTHENTICATION_SUBJECT_SOURCE",
                "username",
            );
            assert_eq!(
    ...
    fn test_instance_admins_parses_json_array() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__INSTANCE_ADMINS",
    ...
    fn test_instance_admins_single_element() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__INSTANCE_ADMINS", r#"["oidc~solo"]"#);
            let config = get_config();
    ...
    fn test_instance_admins_rejects_bare_string() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__INSTANCE_ADMINS", "oidc~alice");
            let defaults = figment::providers::Serialized::defaults(DynAppConfig::default());
            let env = figment::providers::Env::prefixed("VERGLAS_CATALOG_TEST__").split("__");
    ...
    fn test_maintenance_mode_read_only_via_env() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__MAINTENANCE_MODE", "read-only");
            let config = get_config();
    ...
    fn test_maintenance_mode_off_via_env() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__MAINTENANCE_MODE", "off");
            let config = get_config();
            assert_eq!(config.maintenance_mode, MaintenanceMode::Off);
            Ok(())
        });
    ...
    fn test_base_uri_trailing_slash_stripped() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__BASE_URI",
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__BASE_URI", "https://localhost:8181");
            let config = get_config();
            assert_eq!(
                config.base_uri.as_ref().unwrap().to_string(),
    ...
    fn test_wildcard_allow_origin() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__ALLOW_ORIGIN", "*");
            let config = get_config();
            assert_eq!(
                config.allow_origin,
    ...
    fn test_single_audience() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__OPENID_AUDIENCE", "abc");
            let config = get_config();
    ...
    fn test_audience_only_numbers() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__OPENID_AUDIENCE", "123456");
            let config = get_config();
    ...
    fn test_multiple_allow_origin() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__ALLOW_ORIGIN",
    ...
    fn test_queue_config() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__TASK_POLL_INTERVAL", "5s");
            let config = get_config();
    ...
    fn test_task_queue_config_millis() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__TASK_POLL_INTERVAL", "5ms");
            let config = get_config();
            assert_eq!(
    ...
    fn test_task_queue_config_legacy_seconds() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__TASK_POLL_INTERVAL", "\"5\"");
            let config = get_config();
            assert_eq!(config.task_poll_interval, std::time::Duration::from_secs(5));
            Ok(())
        });
    ...
    fn test_bind_ip_address_v4_all() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__BIND_IP", "0.0.0.0");
            let config = get_config();
    ...
    fn test_bind_ip_address_v4_localhost() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__BIND_IP", "127.0.0.1");
            let config = get_config();
    ...
    fn test_bind_ip_address_v6_loopback() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__BIND_IP", "::1");
            let config = get_config();
    ...
    fn test_bind_ip_address_v6_all() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__BIND_IP", "::");
            let config = get_config();
    ...
    fn test_legacy_service_account_acceptance() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__KUBERNETES_AUTHENTICATION_ACCEPT_LEGACY_SERVICEACCOUNT",
                "true",
            );
            let config = get_config();
    ...
    fn test_s3_disable_system_credentials() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__ENABLE_AWS_SYSTEM_CREDENTIALS",
                "true",
            );
            let config = get_config();
    ...
    fn test_use_x_forwarded_headers() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__USE_X_FORWARDED_HEADERS", "true");
            let config = get_config();
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__USE_X_FORWARDED_HEADERS", "false");
            let config = get_config();
            assert!(!config.use_x_forwarded_headers);
            Ok(())
        });
    ...
    fn test_disable_storage_validation() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__SKIP_STORAGE_VALIDATION", "true");
            let config = get_config();
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__SKIP_STORAGE_VALIDATION", "false");
            let config = get_config();
            assert!(!config.skip_storage_validation);
            Ok(())
        });
    ...
    fn test_debug_log_request_bodies() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__DEBUG__LOG_REQUEST_BODIES", "true");
            let config = get_config();
            assert!(config.debug.log_request_bodies);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__DEBUG__LOG_REQUEST_BODIES", "false");
            let config = get_config();
            assert!(!config.debug.log_request_bodies);
            Ok(())
        });
    ...
    fn test_debug_log_authorization_header() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__DEBUG__LOG_AUTHORIZATION_HEADER",
                "true",
            );
            let config = get_config();
            assert!(config.debug.log_authorization_header);
    ...
    fn test_stc_cache() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__STC__ENABLED", "false");
            let config = get_config();
            assert!(!config.cache.stc.enabled);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__STC__ENABLED", "true");
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__STC__CAPACITY", "5000");
            let config = get_config();
            assert!(config.cache.stc.enabled);
    ...
    fn test_warehouse_cache() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__WAREHOUSE__ENABLED", "false");
            let config = get_config();
            assert!(!config.cache.warehouse.enabled);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__WAREHOUSE__ENABLED", "true");
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__WAREHOUSE__CAPACITY", "2000");
            let config = get_config();
            assert!(config.cache.warehouse.enabled);
    ...
    fn test_namespace_cache() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__NAMESPACE__ENABLED", "false");
            let config = get_config();
            assert!(!config.cache.namespace.enabled);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__NAMESPACE__ENABLED", "true");
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__NAMESPACE__CAPACITY", "2000");
            let config = get_config();
            assert!(config.cache.namespace.enabled);
    ...
    fn test_role_cache() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__ROLE__ENABLED", "false");
            let config = get_config();
            assert!(!config.cache.role.enabled);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__ROLE__ENABLED", "true");
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__ROLE__CAPACITY", "5000");
            let config = get_config();
            assert!(config.cache.role.enabled);
    ...
    fn test_openid_providers_structured_env() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__URI",
                "https://company.okta.com",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__AUDIENCE",
                "catalog,warehouse",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__ADDITIONAL_ISSUERS",
                "https://issuer.example.com",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__SCOPE",
                "openid",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__SUBJECT_CLAIMS",
                "sub,oid",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__ROLES_CLAIM",
                "resource_access.catalog.roles",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA__REQUIRE_CONNECTED_ON_STARTUP",
                "false",
            );
            ...
            assert_eq!(
                provider.audience,
                Some(vec!["catalog".to_string(), "warehouse".to_string()])
            );
            assert_eq!(
            ...
            assert_eq!(
                ...
                Some("resource_access.catalog.roles".to_string())
            );
    ...
    fn test_openid_provider_id_rejects_separator() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OKTA~PROD__URI",
                "https://company.okta.com",
            );
    ...
    fn test_openid_provider_id_rejects_underscore() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__MY_PROVIDER__URI",
                "https://example.com",
            );
            let _config = get_config();
            Ok(())
    ...
    fn test_openid_provider_id_rejects_kubernetes() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__KUBERNETES__URI",
                "https://company.okta.com",
            );
            let _config = get_config();
            Ok(())
    ...
    fn test_openid_provider_id_rejects_oidc() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__OIDC__URI",
                "https://company.okta.com",
            );
            let _config = get_config();
            Ok(())
    ...
    fn test_user_assignments_cache() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__CACHE__USER_ASSIGNMENTS__ENABLED",
                "false",
            );
            let config = get_config();
            assert!(!config.cache.user_assignments.enabled);
        ...
        figment::Jail::expect_with(|jail| {
            ...
            jail.set_env(
                "VERGLAS_CATALOG_TEST__CACHE__USER_ASSIGNMENTS__CAPACITY",
                "100000",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__CACHE__USER_ASSIGNMENTS__TIME_TO_LIVE_SECS",
                "60",
            );
            let config = get_config();
            assert!(config.cache.user_assignments.enabled);
    ...
    fn test_role_members_cache() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__CACHE__ROLE_MEMBERS__ENABLED",
                "false",
            );
            let config = get_config();
            assert!(!config.cache.role_members.enabled);
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__CACHE__ROLE_MEMBERS__ENABLED", "true");
            jail.set_env(
                "VERGLAS_CATALOG_TEST__CACHE__ROLE_MEMBERS__CAPACITY",
                "5000",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__CACHE__ROLE_MEMBERS__TIME_TO_LIVE_SECS",
                "30",
            );
            let config = get_config();
            assert!(config.cache.role_members.enabled);
    ...
    fn openid_subject_claims() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__OPENID_SUBJECT_CLAIM", "custom_sub");
            let config = get_config();
            assert_eq!(
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_SUBJECT_CLAIM",
                "custom_sub,oid",
            );
            let config = get_config();
            assert_eq!(
    ...
    fn test_audit_tracing_enabled() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__AUDIT__TRACING__ENABLED", "false");
            let config = get_config();
            assert!(!config.audit.tracing.enabled);
            Ok(())
        });
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__AUDIT__TRACING__ENABLED", "true");
            let config = get_config();
            assert!(config.audit.tracing.enabled);
            Ok(())
        });
    ...
    fn test_trusted_engine_configuration() {
        ...
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__TRUSTED_ENGINES__TRINO__TYPE",
                "trino",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__TRUSTED_ENGINES__TRINO__OWNER_PROPERTY",
                "trino.run-as-owner",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__TRUSTED_ENGINES__TRINO__IDENTITIES__OIDC__AUDIENCES",
                "[trino_dev, trino_prod]",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__TRUSTED_ENGINES__TRINO__IDENTITIES__KUBERNETES__SUBJECTS",
                "[trino-sa, trino-sa-2]",
            );
    ...
    fn test_role_max_nesting_depth_env_override() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__ROLE__MAX_NESTING_DEPTH", "3");
            let config = get_config();
    ...
    fn test_idempotency_env_vars() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__IDEMPOTENCY__ENABLED", "false");
            jail.set_env("VERGLAS_CATALOG_TEST__IDEMPOTENCY__LIFETIME", "PT1H");
            jail.set_env("VERGLAS_CATALOG_TEST__IDEMPOTENCY__GRACE_PERIOD", "PT10M");
            let config = get_config();
            assert!(!config.idempotency.enabled);
    ...
    fn test_idempotency_partial_override() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__IDEMPOTENCY__LIFETIME", "PT15M");
            let config = get_config();
    ...
    fn test_metrics_env_vars() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__METRICS__PORT", "2");
            let config = get_config();
    ...
    fn test_flat_metrics_port_config_is_mapped() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("VERGLAS_CATALOG_TEST__METRICS_PORT", "1");
            let config = get_config();
    ...
    fn test_openid_providers_multiple() {
        figment::Jail::expect_with(|jail| {
            ...
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__EKS-PROD__URI",
                "https://oidc.eks.us-east-1.amazonaws.com/id/ABC123",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__EKS-PROD__AUDIENCE",
                "sts.amazonaws.com",
            );
    ...
    fn test_openid_providers_with_all_fields() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__URI",
                "https://login.microsoftonline.com/tenant-id/v2.0",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__AUDIENCE",
                "api://my-app,second-audience",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__ADDITIONAL_ISSUERS",
                "https://sts.windows.net/tenant-id/",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__SCOPE",
                "catalog",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__SUBJECT_CLAIMS",
                "oid,sub",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__ROLES_CLAIM",
                "groups",
            );
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDERS__ENTRA__DISPLAY_NAME_TEMPLATE",
            ...
            assert_eq!(provider.scope, Some("catalog".to_string()));
            assert_eq!(
                provider.subject_claims,
    ...
    fn test_authn_enabled_with_kubernetes_only() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__ENABLE_KUBERNETES_AUTHENTICATION",
                "true",
            );
            let config = get_config();
            assert!(config.authn_enabled());
    ...
    fn test_authn_enabled_with_single_provider() {
        figment::Jail::expect_with(|jail| {
            jail.set_env(
                "VERGLAS_CATALOG_TEST__OPENID_PROVIDER_URI",
