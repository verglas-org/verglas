...
use uuid::Uuid;
use verglas_iceberg_ext::catalog::rest::{ErrorModel, IcebergErrorResponse};
...
pub enum PrivilegeSource {
    /// In-process caller via [`RequestMetadata::new_catalog_internal`].
    /// Full bypass including data-plane actions.
    Internal,
    /// Principal listed in `VERGLAS_CATALOG__INSTANCE_ADMINS`. Control-plane bypass
...
impl From<ProjectIdMissing> for verglas_iceberg_ext::catalog::rest::ErrorModel {
...
impl From<ProjectIdMissing> for verglas_iceberg_ext::catalog::rest::IcebergErrorResponse {
    fn from(e: ProjectIdMissing) -> Self {
...
impl RequestMetadata {
    ...
    /// `VERGLAS_CATALOG__INSTANCE_ADMINS`). Set by the authn middleware after the actor
    ...
    /// configured via `VERGLAS_CATALOG__INSTANCE_ADMINS` and bypass authorization for all
    ...
    pub fn user_id(&self) -> Option<&crate::service::UserId> {
        match &self.actor {
            ...
            InternalActor::External(Actor::Anonymous) | InternalActor::CatalogInternal => None,
        }
    }
    ...
    #[must_use]
    pub fn new_catalog_internal(request_id: Uuid) -> Self {
        Self {
            request_id,
            project_id: None,
            authentication: None,
            base_url: "http://localhost:8181".to_string(),
            actor: InternalActor::CatalogInternal,
            matched_path: None,
    ...
    #[must_use]
    #[inline]
    pub fn is_catalog_internal(&self) -> bool {
        matches!(self.actor, InternalActor::CatalogInternal)
    }

    /// Source of the authz decision for this request, for audit logging.
    #[must_use]
    pub fn privilege_source(&self) -> PrivilegeSource {
        if self.is_catalog_internal() {
    ...
    ///   (`CatalogInternal`) or a configured instance admin, **and**
    ...
    /// `is_catalog_internal()` for the in-process variant (which
    ...
    pub fn actor(&self) -> &Actor {
        match &self.actor {
            ...
            InternalActor::CatalogInternal => ANONYMOUS_ACTOR,
        }
    }
...
pub(crate) async fn create_request_metadata_with_trace_and_project_fn(
    ...
    let Some(base_uri) = determine_base_uri(&headers) else {
        return verglas_iceberg_ext::catalog::rest::IcebergErrorResponse::from(ErrorModel::bad_request(
...
fn determine_base_uri_with(
    ...
    if use_x_forwarded_headers {
        ...
        // as catalog does not terminate TLS. Any external entity that terminates TLS should set the x-forwarded headers.
...
mod test {
    ...
    fn test_determine_host_with_prefix_clean() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("/catalog"),
        );

        let result = determine_base_uri(&headers);
        assert_eq!(result, Some("https://example.com/catalog".to_string()));
    }
    ...
    fn test_determine_host_with_prefix_trailing_slash() {
        ...
        headers.insert(
            X_FORWARDED_PREFIX_HEADER,
            HeaderValue::from_static("api/catalog/"),
        );

        let result = determine_base_uri(&headers);
        assert_eq!(result, Some("https://example.com/api/catalog".to_string()));
    }
    ...
    fn test_determine_forwarded_prefix() {
        ...
        assert_eq!(determine_forwarded_prefix(&headers), Some("catalog"));
        ...
        assert_eq!(determine_forwarded_prefix(&headers), Some("api/catalog"));
