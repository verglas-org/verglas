...
impl Valuable for InternalActor {
    ...
    fn visit(&self, visit: &mut dyn Visit) {
        match self {
            InternalActor::CatalogInternal => {
                visit.visit_entry(
                    Value::String("actor_type"),
                    Value::String("catalog-internal"),
                );
            }
...
impl Mappable for InternalActor {
    fn size_hint(&self) -> (usize, Option<usize>) {
        match self {
            InternalActor::CatalogInternal => (1, Some(1)),
...
/// # Examples
/// ```rust,ignore
/// use verglas_catalog_core::audit_operation;
/// use verglas_catalog_core::service::events::backends::audit::AuditPrincipal;
///
/// // Without context
/// audit_operation!(
