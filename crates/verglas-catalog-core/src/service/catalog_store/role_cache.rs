...
mod tests {
    ...
    async fn test_role_cache_insert_and_get_by_id() {
        ...
        let role = test_role(role_id, project_id, "catalog", "test-source", 0);
    ...
    async fn test_role_cache_get_by_ident() {
        let role_id = RoleId::new_random();
        let project_id = Arc::new(ProjectId::new_random());
        let role = test_role(role_id, project_id, "catalog", "test-ident-source", 0);
    ...
    async fn test_role_cache_get_by_ident_wrong_project() {
        ...
        let role = test_role(role_id, project_id, "catalog", "wrong-project-source", 0);
    ...
    async fn test_role_cache_invalidate() {
        let role_id = RoleId::new_random();
        let project_id = Arc::new(ProjectId::new_random());
        let role = test_role(role_id, project_id, "catalog", "invalidate-source", 0);
        let project_id = role.project_id_arc();
        let ident = role.ident_arc();
    ...
    async fn test_role_cache_insert_older_version_ignored() {
        ...
        let newer = test_role(role_id, project_id.clone(), "catalog", "ver-source", 5);
        role_cache_insert(newer.clone()).await;

        let older = test_role(role_id, project_id, "catalog", "ver-source", 3);
    ...
    async fn test_role_cache_ident_updated_on_ident_change() {
        ...
        let v1 = test_role(role_id, project_id.clone(), "catalog", "old-source", 0);
        let old_ident = v1.ident_arc();
        role_cache_insert(v1).await;

        // Update with a new ident (simulates set_role_source_system)
        let v2 = test_role(role_id, project_id.clone(), "catalog", "new-source", 1);
    ...
    async fn role_get_or_load_coalesces_concurrent_misses() {
        ...
        let role = test_role(role_id, project_id, "catalog", "coalesce-source", 0);
    ...
    async fn role_get_or_load_version_gate_keeps_newer_concurrent_insert() {
        ...
        let newer = test_role(role_id, project_id.clone(), "catalog", "vg-source", 5);
        let older = test_role(role_id, project_id, "catalog", "vg-source", 3);
    ...
    async fn role_ident_to_id_get_or_load_coalesces_concurrent_misses() {
        ...
        let loads = Arc::new(AtomicUsize::new(0));
        let role = test_role(role_id, project_id.clone(), "catalog", "ident-coalesce", 0);
        let ident = role.ident_arc();
