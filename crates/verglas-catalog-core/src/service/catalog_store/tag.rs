...
pub const RESERVED_TAG_PREFIXES: &[&str] = &["system.", "catalog."];
...
mod tests {
    ...
    fn reserved_names_detected_case_insensitively() {
        ...
        assert!(is_reserved_tag_name("catalog"));
        assert!(is_reserved_tag_name("Catalog.Internal"));
        assert!(!is_reserved_tag_name("pii.classification"));
        // "system." is the prefix — "systematic"/"catalogs" are normal names
        assert!(!is_reserved_tag_name("systematic"));
        assert!(!is_reserved_tag_name("catalogs"));
    }
