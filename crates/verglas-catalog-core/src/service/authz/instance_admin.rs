...
//! capabilities granted by server configuration (`VERGLAS_CATALOG__INSTANCE_ADMINS`)
...
impl InstanceAdminAuthorizer {
    ...
    /// admin (`Actor::Principal` in `VERGLAS_CATALOG__INSTANCE_ADMINS`) or an
    /// in-process (`CatalogInternal`) caller.
    ///
    ...
    /// must additionally check [`RequestMetadata::is_catalog_internal`].
    #[must_use]
    pub fn has_bypass(metadata: &RequestMetadata) -> bool {
        metadata.is_catalog_internal() || metadata.is_instance_admin()
    }
...
/// `VERGLAS_CATALOG__INSTANCE_ADMINS` configuration, preserving zero-config bootstrap.
...
/// normally sourced from the static `VERGLAS_CATALOG__INSTANCE_ADMINS` configuration
...
impl ConfiguredInstanceAdmins {
    ...
    /// Snapshot the configured admin set (`VERGLAS_CATALOG__INSTANCE_ADMINS`). Config is
...
mod tests {
    ...
    fn internal_is_allowed() {
        let md = RequestMetadata::new_catalog_internal(uuid::Uuid::now_v7());
        assert!(InstanceAdminAuthorizer::has_bypass(&md));
        assert!(InstanceAdminAuthorizer::require(&md, ACTION).is_ok());
    }
