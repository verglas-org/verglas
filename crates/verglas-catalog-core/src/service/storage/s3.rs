...
pub struct S3Profile {
    ...
    #[serde(default)]
    /// Optional ARN to assume when accessing the bucket from Catalog.
    #[builder(default, setter(strip_option))]
    ...
    /// Default: `auto`. When set to `auto`, Catalog will first try to parse the URL as
    ...
    ///
    /// For more details, refer to Catalog's
    /// [Soft-Deletion documentation](https://docs.catalog.io/docs/nightly/concepts/#soft-deletion).
    ...
    /// When enabled, Catalog will use the legacy MD5 checksum for operations like `DeleteObjects`.
    #[serde(default)]
    #[builder(default, setter(strip_option))]
...
pub enum S3Credential {
    ...
    ///  that runs catalog. The AWS SDK is used to load the credentials.
...
pub struct S3CloudflareR2Credential {
    /// Access key ID used for IO operations of Catalog
    #[serde(alias = "aws-access-key-id")]
    pub access_key_id: String,
    #[redact(partial)]
...
impl S3Profile {
    ...
    pub async fn verglas_catalog_io(
        &self,
    ...
    pub async fn generate_table_config(
        ...
        // independent of any S3 bucket-default-encryption configuration. Catalog's own writes
        // already set this header via verglas-catalog-io.
        if let Some(kms_key_arn) = self.aws_kms_key_arn.as_ref() {
        ...
        let policy = json!({
            ...
            "Statement": [
                ...
                },
                {
                    ...
                    // would effectively deny it. An S3-SDK client (Catalog vends OSS through the
    ...
    async fn assume_role_with_sts(
            ...
            && !CONFIG.s3_enable_direct_system_credentials
        {
            ...
            "This deployment of Catalog requires an `assume-role-arn` to be set for system identity credentials."
                .to_string(),
        ));
        }
    ...
    pub async fn get_aws_sdk_config(
        ...
        {
            return Err(CredentialsError::Misconfiguration(
                "System identity credentials are disabled in this Catalog deployment.".to_string(),
            ));
        }
...
fn storage_profile_to_s3_settings(profile: &S3Profile) -> S3Settings {
    S3Settings {
        ...
        // Currently set per-credential by `verglas_catalog_io` for Alibaba OSS; see `S3Settings`.
...
fn aliyun_assume_role_params<'a>(
    ...
    params.insert("RoleSessionName", "catalog-sts".to_string());
...
pub(super) async fn catalog_io_from_vended_table_config(
...
fn validate_region(region: &str) -> Result<(), InvalidProfileError> {
    verglas_catalog_io::s3::validate_region(region).map_err(|e| InvalidProfileError {
        source: None,
        reason: e,
...
pub(crate) mod test {
    ...
    fn test_deserialize_aliyun_assume_role_response() {
        ...
        let response = serde_json::json!({
            ...
            "AssumedRoleUser": {
                "AssumedRoleId": "34158XXXXX2653686:catalog-sts",
                "Arn": "acs:ram::123456789012:role/oss-role/catalog-sts"
            },
            "Credentials": {
    ...
    pub(crate) mod minio_integration_tests {
        ...
        static TEST_BUCKET: LazyLock<String> =
            LazyLock::new(|| std::env::var("VERGLAS_CATALOG_TEST__S3_BUCKET").unwrap());
        static TEST_REGION: LazyLock<String> = LazyLock::new(|| {
            std::env::var("VERGLAS_CATALOG_TEST__S3_REGION").unwrap_or("local".into())
        });
        static TEST_ACCESS_KEY: LazyLock<String> =
            LazyLock::new(|| std::env::var("VERGLAS_CATALOG_TEST__S3_ACCESS_KEY").unwrap());
        static TEST_SECRET_KEY: LazyLock<String> =
            LazyLock::new(|| std::env::var("VERGLAS_CATALOG_TEST__S3_SECRET_KEY").unwrap());
        static TEST_ENDPOINT: LazyLock<String> =
            LazyLock::new(|| std::env::var("VERGLAS_CATALOG_TEST__S3_ENDPOINT").unwrap());
    ...
    pub(crate) mod aws_integration_tests {
        ...
        pub(crate) fn get_storage_profile() -> (S3Profile, S3Credential) {
            let profile = S3Profile::builder()
                .bucket(std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_BUCKET").unwrap())
                .key_prefix(uuid::Uuid::now_v7().to_string())
                .region(std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_REGION").unwrap())
                .path_style_access(true)
                .sts_role_arn(std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_STS_ROLE_ARN").unwrap())
                .flavor(S3Flavor::Aws)
                .sts_enabled(true)
                .remote_signing_enabled(true)
                .allow_alternative_protocols(false)
                ...
                .build();
            let cred = S3Credential::AccessKey(S3AccessKeyCredential {
                access_key_id: std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_ACCESS_KEY_ID").unwrap(),
                secret_access_key: std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_SECRET_ACCESS_KEY")
                    .unwrap(),
                external_id: None,
            });
        ...
        fn test_multipart_upload_with_vended_credentials() {
            test_block_on(
                async {
                    ...
                    let table_location: verglas_catalog_io::Location = format!(
                        "s3://{}/{}",
                        profile.bucket,
                    ...
                    // Get the downscoped STS policy that Catalog generates
                    let policy = profile
    ...
    pub(crate) mod aws_kms_integration_tests {
        ...
        pub(crate) fn get_storage_profile() -> (S3Profile, S3Credential) {
            let profile = S3Profile::builder()
                .bucket(std::env::var("VERGLAS_CATALOG_TEST__AWS_KMS_S3_BUCKET").unwrap())
                .key_prefix(uuid::Uuid::now_v7().to_string())
                .assume_role_arn(
                    std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_STS_ROLE_ARN").unwrap(),
                )
                .region(std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_REGION").unwrap())
                .path_style_access(true)
                .flavor(S3Flavor::Aws)
                ...
                .remote_signing_enabled(true)
                .allow_alternative_protocols(false)
                .aws_kms_key_arn(std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_KMS_ARN").unwrap())
                .legacy_md5_behavior(false)
                .push_s3_delete_disabled(false)
                .build();
            let cred = S3Credential::AccessKey(S3AccessKeyCredential {
    ...
    pub(crate) mod r2_integration_tests {
        ...
        pub(crate) fn get_storage_profile() -> (S3Profile, S3Credential) {
            let profile = S3Profile::builder()
                .bucket(std::env::var("VERGLAS_CATALOG_TEST__R2_BUCKET").unwrap())
                .key_prefix(uuid::Uuid::now_v7().to_string())
                .endpoint(
                    std::env::var("VERGLAS_CATALOG_TEST__R2_ENDPOINT")
                        .unwrap()
                        .parse()
                        .unwrap(),
                )
            ...
            let cred = S3Credential::CloudflareR2(S3CloudflareR2Credential {
                access_key_id: std::env::var("VERGLAS_CATALOG_TEST__R2_ACCESS_KEY_ID").unwrap(),
                secret_access_key: std::env::var("VERGLAS_CATALOG_TEST__R2_SECRET_ACCESS_KEY")
                    .unwrap(),
                token: std::env::var("VERGLAS_CATALOG_TEST__R2_TOKEN").unwrap(),
                account_id: std::env::var("VERGLAS_CATALOG_TEST__R2_ACCOUNT_ID").unwrap(),
            });
    ...
    pub(crate) mod oss_integration_tests {
        ...
        pub(crate) fn get_storage_profile() -> (S3Profile, S3Credential) {
            let profile = S3Profile::builder()
                .bucket(std::env::var("VERGLAS_CATALOG_TEST__OSS_BUCKET").unwrap())
                .key_prefix(uuid::Uuid::now_v7().to_string())
                .endpoint(
                    std::env::var("VERGLAS_CATALOG_TEST__OSS_ENDPOINT")
                        .unwrap()
                        .parse()
                        .unwrap(),
                )
                .region(std::env::var("VERGLAS_CATALOG_TEST__OSS_REGION").unwrap())
                .sts_role_arn(std::env::var("VERGLAS_CATALOG_TEST__OSS_STS_ROLE_ARN").unwrap())
                ...
                .build();
            let cred = S3Credential::AliyunOss(S3AccessKeyCredential {
                access_key_id: std::env::var("VERGLAS_CATALOG_TEST__OSS_ACCESS_KEY_ID").unwrap(),
                secret_access_key: std::env::var("VERGLAS_CATALOG_TEST__OSS_SECRET_ACCESS_KEY")
                    .unwrap(),
                external_id: std::env::var("VERGLAS_CATALOG_TEST__OSS_EXTERNAL_ID").ok(),
            });
    ...
    fn test_vended_credential_carries_qualifying_properties() {
        ...
        let profile = S3Profile::builder()
            ...
            .endpoint("http://s3-compatible:8333".parse().unwrap())
            .sts_enabled(true)
            .sts_role_arn("arn:aws:iam::123456789012:role/catalog".to_string())
    ...
    fn policy_string_stays_within_aws_plaintext_policy_limit() {
        ...
        let table_location = format!(
            "s3://{bucket}/\
             catalog-integration-tests/0123456789abcdef0123456789abcdef/\
        ...
        let profile = S3Profile::builder()
            ...
            .flavor(S3Flavor::Aws)
            .sts_enabled(true)
            .sts_role_arn("arn:aws:iam::123456789012:role/catalog-sts".to_string())
            .build();
        let policy = profile
            .get_sts_policy_string(
                &table_location.parse().unwrap(),
