...
pub enum ValidationCheckName {
    ...
    /// Catalog itself can write, read back and delete a test file, using the
...
pub(crate) const SKIPPED_BY_CONFIG: &str =
    "Storage validation is disabled on this server (VERGLAS_CATALOG__SKIP_STORAGE_VALIDATION).";
...
mod tests {
    ...
    fn report_is_invalid_when_any_check_failed() {
        let report = ValidationReport::new(vec![
            ValidationCheck::passed(ValidationCheckName::ProfileWellFormed, 1),
            ValidationCheck::failed(ValidationCheckName::CatalogReadWrite, 5, err("no write")),
        ]);
        assert!(!report.valid);
    }
    ...
    fn into_result_reports_first_failure_and_stacks_the_rest() {
        ...
        assert!(error.message.contains("catalog-read-write"), "{error:?}");
