...
use verglas_iceberg_ext::configs::table::{TableProperties, creds, gcs};
            ...
            GcsCredential::GcpSystemIdentity {} => {
                if !CONFIG.enable_gcp_system_credentials {
                    return Err(CredentialsError::Misconfiguration(
                        "GCP System identity credentials are disabled in this Catalog deployment."
                            .to_string(),
                    ));
                }
...
pub(super) async fn catalog_io_from_vended_table_config(
    ...
    let auth = GcsAuth::BearerToken(verglas_catalog_io::gcs::GcsBearerTokenAuth { access_token });
    GCSSettings {}
...
pub(crate) mod test {
    pub(crate) mod gcs_integration_tests {
        ...
        pub(crate) fn get_storage_profile() -> (GcsProfile, GcsCredential) {
            let bucket =
                std::env::var("VERGLAS_CATALOG_TEST__GCS_BUCKET").expect("Missing GCS_BUCKET");
            let key = std::env::var("VERGLAS_CATALOG_TEST__GCS_CREDENTIAL")
                .expect("Missing GCS_CREDENTIAL");
    ...
    pub(crate) mod gcs_hns_integration_tests {
        ...
        pub(crate) fn get_storage_profile() -> (GcsProfile, GcsCredential) {
            let bucket = std::env::var("VERGLAS_CATALOG_TEST__GCS_HNS_BUCKET")
                .expect("Missing GCS_HNS_BUCKET");
            let key = std::env::var("VERGLAS_CATALOG_TEST__GCS_CREDENTIAL")
                .expect("Missing GCS_CREDENTIAL");
            let key: GcsServiceKey = serde_json::from_str(&key).unwrap();
            let cred = GcsCredential::ServiceAccountKey { key };
