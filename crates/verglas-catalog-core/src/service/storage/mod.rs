...
use verglas_catalog_io::{
    CatalogStorage, InvalidLocationError, Location, LocationParseError, StorageBackend,
    s3::S3Location,
};
...
impl StorageProfile {
    ...
    pub async fn file_io(
        ...
        match self {
            ...
            StorageProfile::Memory(_) => Ok(StorageBackend::Memory(
                verglas_catalog_io::memory::MemoryStorage::new(),
            )),
        }
    }
    ...
    pub async fn validate_access_report(
        ...
        if CONFIG.skip_storage_validation {
            ...
            report.skip(ValidationCheckName::CatalogReadWrite, SKIPPED_BY_CONFIG);
        ...
        let io = match self.file_io(credential).await {
            ...
            Err(e) => {
                ...
                report.skip(ValidationCheckName::CatalogReadWrite, SKIPPED_PREREQUISITE);
                for name in vended_checks {
        ...
        let ns_location = match self.default_namespace_location(&namespace_path) {
            ...
            Err(e) => {
                report.push(ValidationCheck::failed(
                    ValidationCheckName::CatalogReadWrite,
                    0,
                    e,
                ));
                for name in vended_checks {
        ...
        let direct_validation = async {
            let started = Instant::now();
            let result = self.validate_read_write_catalog(&io, &test_location).await;
    ...
    async fn validate_vended_credentials_access(
        ...
        let read_write_validation = async {
            let started = Instant::now();
            let result = self
                .validate_read_write_catalog(&sts_storage, &sub_location)
                .await;
            (elapsed_ms(started), result)
        };
        let no_write_validation = async {
            let started = Instant::now();
            let result = self
                .validate_no_write_access_catalog(&sts_storage, test_location)
                .await;
            (elapsed_ms(started), result)
        };
    ...
    async fn issue_vended_credentials(
        ...
        Ok(match &self {
            StorageProfile::S3(_) => {
                ...
                s3::catalog_io_from_vended_table_config(&tbl_config.config)
                    .await?
                    .into()
            }
            StorageProfile::Adls(profile) => {
                ...
                profile
                    .catalog_io_from_vended_table_config(&tbl_config.config)
                    .await?
                    .into()
            }
            StorageProfile::OneLake(profile) => {
            ...
            StorageProfile::Gcs(_) => {
                ...
                gcs::catalog_io_from_vended_table_config(&tbl_config.config)
                    .await?
                    .into()
            }
            #[cfg(feature = "test-utils")]
    ...
    async fn validate_read_write_catalog(
        &self,
        io: &impl CatalogStorage,
        test_location: &Location,
    ) -> Result<(), ValidationError> {
    ...
    async fn validate_no_write_access_catalog(
        &self,
        io: &impl CatalogStorage,
        test_location: &Location,
    ) -> Result<(), ValidationError> {
...
pub enum StorageCredential {
    ...
    ///
    /// ```
    /// use verglas_catalog_core::service::storage::StorageCredential;
...
mod validate_access_report_tests {
    ...
    async fn unreachable_storage_fails_the_probe_and_skips_vending() {
        ...
        assert_eq!(
            status_of(&report, ValidationCheckName::CatalogReadWrite),
        ...
        assert!(
            error
                .message
                .starts_with("Storage validation failed [catalog-read-write]:"),
            "{}",
            error.message
        );
    }
...
mod tests {
    ...
    mod gcs_integration_tests {
        ...
        async fn test_vended_gcs() {
            ...
            let cred: StorageCredential = std::env::var("VERGLAS_CATALOG_TEST__GCS_CREDENTIAL")
                ...
                .expect("Missing cred")
                .into();
            let bucket = std::env::var("VERGLAS_CATALOG_TEST__GCS_BUCKET").expect("Missing bucket");
        ...
        fn test_vended_aws() {
            test_block_on(
                async {
                    ...
                    let bucket = std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_BUCKET").unwrap();
                    let region = std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_REGION").unwrap();
                    let sts_role_arn =
                    ...
                    let cred: StorageCredential = S3Credential::AccessKey(S3AccessKeyCredential {
                        access_key_id: std::env::var("VERGLAS_CATALOG_TEST__AWS_S3_ACCESS_KEY_ID")
                            .unwrap(),
                        secret_access_key: std::env::var(
                            "VERGLAS_CATALOG_TEST__AWS_S3_SECRET_ACCESS_KEY",
                        )
                        .unwrap(),
                        external_id: None,
                    })
    ...
    async fn test_profile_vended_creds(cred: &StorageCredential, profile: &mut StorageProfile) {
        ...
        let (downscoped1, downscoped2): (StorageBackend, StorageBackend) = match profile {
            StorageProfile::Adls(p) => (
                p.catalog_io_from_vended_table_config(&config1.config)
                    .await
                    .unwrap()
                    .into(),
                p.catalog_io_from_vended_table_config(&config2.config)
                    .await
                    .unwrap()
                    .into(),
            ),
            ...
            StorageProfile::S3(_) => (
                s3::catalog_io_from_vended_table_config(&config1.config)
                    .await
                    .unwrap()
                    .into(),
                s3::catalog_io_from_vended_table_config(&config2.config)
                    .await
                    .unwrap()
                    .into(),
            ),
            StorageProfile::Gcs(_) => (
                gcs::catalog_io_from_vended_table_config(&config1.config)
                    .await
                    .unwrap()
                    .into(),
                gcs::catalog_io_from_vended_table_config(&config2.config)
                    .await
                    .unwrap()
                    .into(),
            ),
