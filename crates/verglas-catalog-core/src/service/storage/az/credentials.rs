...
use veil::Redact;
use verglas_catalog_io::adls::{AzureAuth, AzureClientCredentialsAuth, AzureSharedAccessKeyAuth};
    ...
    fn try_from(cred: AzCredential) -> Result<Self, Self::Error> {
            ...
            && matches!(cred, AzCredential::AzureSystemIdentity {})
        {
            return Err(CredentialsError::Misconfiguration(
                "Azure System identity credentials are disabled in this Catalog deployment."
                    .to_string(),
            ));
        }
