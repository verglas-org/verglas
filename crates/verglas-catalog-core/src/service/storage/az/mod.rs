...
use verglas_iceberg_ext::configs::table::{TableProperties, adls, creds, custom};
...
/// "now" — i.e. the smallest window between the moment Catalog hands the
...
pub(super) async fn adls_catalog_io(
...
/// thin wrapper method.
pub(super) async fn catalog_io_from_vended_adls_table_config(
    settings: AzureSettings,
...
pub(crate) mod test {
    ...
    pub(crate) mod azure_integration_tests {
        ...
        pub(crate) fn azure_profile() -> GenericAdlsProfile {
            let account_name = std::env::var("VERGLAS_CATALOG_TEST__AZURE_STORAGE_ACCOUNT_NAME")
                .expect("VERGLAS_CATALOG_TEST__AZURE_STORAGE_ACCOUNT_NAME to be set");
            let filesystem = std::env::var("VERGLAS_CATALOG_TEST__AZURE_STORAGE_FILESYSTEM")
                .expect("VERGLAS_CATALOG_TEST__AZURE_STORAGE_FILESYSTEM to be set");
        ...
        pub(crate) fn client_creds() -> AzCredential {
            let client_id = std::env::var("VERGLAS_CATALOG_TEST__AZURE_CLIENT_ID")
                .expect("VERGLAS_CATALOG_TEST__AZURE_CLIENT_ID to be set");
            let client_secret = std::env::var("VERGLAS_CATALOG_TEST__AZURE_CLIENT_SECRET")
                .expect("VERGLAS_CATALOG_TEST__AZURE_CLIENT_SECRET to be set");
            let tenant_id = std::env::var("VERGLAS_CATALOG_TEST__AZURE_TENANT_ID")
                .expect("VERGLAS_CATALOG_TEST__AZURE_TENANT_ID to be set");
        ...
        pub(crate) fn shared_key() -> AzCredential {
            let key = std::env::var("VERGLAS_CATALOG_TEST__AZURE_STORAGE_SHARED_KEY")
                .expect("VERGLAS_CATALOG_TEST__AZURE_STORAGE_SHARED_KEY to be set");
            AzCredential::SharedAccessKey { key }
        }
    ...
    ///
    /// # Required env vars
    /// - `VERGLAS_CATALOG_TEST__ONELAKE_WORKSPACE_ID` — Fabric workspace UUID.
    /// - `VERGLAS_CATALOG_TEST__ONELAKE_LAKEHOUSE_ID` — lakehouse UUID inside that workspace.
    /// - `VERGLAS_CATALOG_TEST__ONELAKE_CLIENT_ID` — Entra app client ID with rights on the workspace.
    /// - `VERGLAS_CATALOG_TEST__ONELAKE_CLIENT_SECRET` — client secret for that app.
    /// - `VERGLAS_CATALOG_TEST__ONELAKE_TENANT_ID` — Entra tenant ID.
    ///
    /// # Mode-specific
    /// - `VERGLAS_CATALOG_TEST__ONELAKE_REGION` — Azure region slug
    ...
    pub(crate) mod onelake_integration_tests {
        ...
        pub(crate) fn onelake_profile() -> OneLakeProfile {
            let workspace_id = std::env::var("VERGLAS_CATALOG_TEST__ONELAKE_WORKSPACE_ID")
                .expect("VERGLAS_CATALOG_TEST__ONELAKE_WORKSPACE_ID to be set");
            let lakehouse_id = std::env::var("VERGLAS_CATALOG_TEST__ONELAKE_LAKEHOUSE_ID")
                .expect("VERGLAS_CATALOG_TEST__ONELAKE_LAKEHOUSE_ID to be set");
            let directory_rel_path = Some(format!("test-{}", uuid::Uuid::now_v7()));
            OneLakeProfile {
                workspace_id: Uuid::parse_str(&workspace_id)
                    .expect("VERGLAS_CATALOG_TEST__ONELAKE_WORKSPACE_ID is not a valid UUID"),
                lakehouse_id: Uuid::parse_str(&lakehouse_id)
                    .expect("VERGLAS_CATALOG_TEST__ONELAKE_LAKEHOUSE_ID is not a valid UUID"),
                directory_rel_path,
        ...
        pub(crate) fn client_creds() -> AzCredential {
            let client_id = std::env::var("VERGLAS_CATALOG_TEST__ONELAKE_CLIENT_ID")
                .expect("VERGLAS_CATALOG_TEST__ONELAKE_CLIENT_ID to be set");
            let client_secret = std::env::var("VERGLAS_CATALOG_TEST__ONELAKE_CLIENT_SECRET")
                .expect("VERGLAS_CATALOG_TEST__ONELAKE_CLIENT_SECRET to be set");
            let tenant_id = std::env::var("VERGLAS_CATALOG_TEST__ONELAKE_TENANT_ID")
                .expect("VERGLAS_CATALOG_TEST__ONELAKE_TENANT_ID to be set");
        ...
        #[tokio::test]
        #[ignore = "live OneLake test; opt in with --ignored (see module docs). \
                    Also requires VERGLAS_CATALOG_TEST__ONELAKE_REGION."]
        async fn test_can_validate_onelake_regional() {
            let region = std::env::var("VERGLAS_CATALOG_TEST__ONELAKE_REGION")
                .expect("VERGLAS_CATALOG_TEST__ONELAKE_REGION to be set");
