...
use verglas_iceberg_ext::configs::table::TableProperties;

use super::{
    AdlsTableConfigContext, AzCredential, MAX_ONELAKE_SAS_TOKEN_VALIDITY_SECONDS, SasMintContext,
    adls_catalog_config, adls_catalog_io, catalog_io_from_vended_adls_table_config,
    generate_adls_table_config, iceberg_expiration_property_key, iceberg_sas_property_key,
    key_prefix_overlaps, validate_sas_token_validity_seconds,
};
use crate::{
    WarehouseId,
...
/// each lakehouse item. `Files/` is the default for Catalog-managed Iceberg
...
/// How Catalog connects to the `OneLake` DFS endpoint.
///
...
///   points the global FQDN at a tenant-PE NIC. From Catalog's
...
///   workspace-scoped PE. Catalog needs to build that FQDN — use
...
pub enum EndpointMode {
    ...
    /// only changes DNS resolution, not the URL Catalog constructs.
    #[default]
    Default,
...
pub struct OneLakeProfile {
    ...
    /// directory under which Catalog writes all warehouse data.
...
impl OneLakeProfile {
    ...
    /// Get the Catalog IO for this storage profile.
    ///
    /// # Errors
    ...
    pub async fn verglas_catalog_io(
        ...
        adls_catalog_io(self.azure_settings(), credential).await
    }

    /// Build an `AdlsStorage` client from previously-vended credentials.
    pub(in crate::service::storage) async fn catalog_io_from_vended_table_config(
        &self,
        config: &TableProperties,
    ) -> Result<AdlsStorage, CredentialsError> {
        catalog_io_from_vended_adls_table_config(
            self.azure_settings(),
