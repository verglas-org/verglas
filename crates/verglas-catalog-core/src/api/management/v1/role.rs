...
/// the membership sites deliberately permit it — `system` (and `catalog`)
...
pub struct CreateRoleRequest {
    ...
    /// Provider that owns this role (e.g. `"catalog"`, `"oidc"`).
    /// Must be provided together with `source-id`. Omit both to let the server
    /// assign `provider-id = "catalog"` and a fresh UUIDv7 `source-id`.
    #[serde(default)]
    #[builder(default)]
    #[cfg_attr(feature = "open-api", schema(value_type=Option::<String>))]
...
async fn authorize_create_role<A: Authorizer, C: CatalogStore>(
    ...
    // record for this role (i.e. the `catalog` provider). Not the
    ...
    let provider_id = request.provider_id.unwrap_or_else(RoleProviderId::catalog);
...
mod tests {
    ...
    fn reject_managed_provider_denies_system_and_configured_providers() {
        ...
        // The native `catalog` namespace and any unconfigured namespace stay
        // writable (deny-list: only `system` + currently-configured providers).
        assert!(reject_managed_provider(&RoleProviderId::catalog(), &managed).is_ok());
