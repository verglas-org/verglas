...
use verglas_catalog_io::{
    CatalogStorage, DeleteError, IOError, InvalidLocationError, Location, ReadError, WriteError,
};
...
impl From<IOErrorExt> for ErrorModel {
    fn from(value: IOErrorExt) -> Self {
        ...
        match value {
            ...
            IOErrorExt::IOError(e) => ErrorModel::from_io_error(e, "Catalog IO operation failed."),
        }
    }
}
...
mod tests {
    ...
    async fn test_list_and_remove_all(cred: StorageCredential, profile: StorageProfile) {
        async fn list_simple(io: &impl CatalogStorage, location: &Location) -> Vec<Location> {
