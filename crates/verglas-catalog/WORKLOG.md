...
- #84: Added a fail-closed, database-scoped catalog registry. Multiple lakehouse databases can share one tenant Catalog deployment while retaining distinct warehouses and storage bindings; external catalogs remain independently bound by stable resource IDs.
...
- #135: Added the native managed-catalog client used by the Catalog REST/domain
...
- #135: Added typed record reads and collection listings to the managed catalog
  client. Catalog domain objects now have a CRaft transport path alongside
