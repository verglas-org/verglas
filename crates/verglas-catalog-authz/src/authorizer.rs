...
use crate::{
    CredentialAuthzConfig, DecisionClient, DecisionClientError, VerglasAction,
    resource::ResourceMapper,
};

/// Catalog authorizer that verifies every catalog decision locally.
#[derive(Clone, Debug)]
...
impl VerglasAuthorizer {
    ...
    pub fn try_new(server_id: ServerId, config: CredentialAuthzConfig) -> anyhow::Result<Self> {
        if config.tenant_id.trim().is_empty() {
            anyhow::bail!("Verglas authz tenant_id cannot be empty");
        }
        if config.issuer.trim().is_empty() {
            anyhow::bail!("control-plane credential issuer cannot be empty");
        }
        Ok(Self {
            server_id,
