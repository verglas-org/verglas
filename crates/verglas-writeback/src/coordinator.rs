...
//!   in the background.
//! - If the committed voter set cannot durably stage the required coded
//!   certificate, the write is refused. Origin writes are not a second
//!   acknowledgement authority for write-back objects.
...
pub struct WriteCoordinator<W: ObjectWrite> {
    ...
    /// Direct-to-origin writer used only after a consensus-committed object propagates.
    origin: Arc<W>,
    /// Universal consensus authority that commits the staged object header.
    committer: Arc<dyn ConsensusCommitter>,
    /// This node's id.
    self_id: NodeId,
    /// Deadline to gather `w` durable fragments before refusing the write.
    ack_deadline: Duration,
    /// Last acknowledgement mode, retained for consensus-write telemetry.
    last_mode: AtomicU8,
...
impl<W: ObjectWrite> WriteCoordinator<W> {
    ...
    /// the acknowledgement only after coded staging and consensus commit.
    ///
    /// The write-back size limit is NVMe headroom, not DRAM: a candidate node is
    /// excluded when its fragment store cannot hold the fragment. A shortfall
    /// refuses the write before consuming the body. `k`/`m`/`w` are resolved
    ...
    pub async fn put_stream(
        ...
        // moves into propagation only after the consensus commit.
        ...
        // degraded to one live member refuses admission.
        ...
        // refuses admission, never a silent drop.
        ...
        let (object_len, payload_hash) =
            match feed_encoder(geometry, body, total, &mut senders).await {
                Ok(len) => len,
                Err(error) => {
        ...
        if acked.len() >= w {
            ...
            Err(quorum_shortfall(w, acked.len()))
        }
    }
    ...
    async fn finish_stream_ack(
        ...
        self.committer
            .commit(StagedObject {
                ...
                payload_hash,
                geometry,
                placements: journal.placements.clone(),
            })
            .await
    ...
    fn record_mode(&self, mode: u8) {
        ...
        if previous != MODE_UNSET && previous != mode {
            ...
            eprintln!("writeback: mode transition to consensus write-back");
        }
    }
...
async fn feed_encoder(
    ...
    let object_len = encoder
        ...
        .map_err(|e| WriteError::Backend(format!("write-back encode: {e}")))?;
    flush_pending(&mut pending, senders).await;
    Ok((object_len, hash.finalize().into()))
}
