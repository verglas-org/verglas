...
//!
//! The CLI names tables as a dotted string. The last component is the table
...
/// Parses `namespace.name` into an Iceberg [`TableIdent`]. Requires at least one
...
pub fn parse_table_ident(dotted: &str) -> Result<TableIdent> {
    ...
    if parts.len() < 2 || parts.len() != dotted.split('.').count() {
        return Err(AgentError::BadIdent(dotted.to_owned()));
    }
    let (name, namespace) = parts
        .split_last()
        .ok_or_else(|| AgentError::BadIdent(dotted.to_owned()))?;
    let namespace = NamespaceIdent::from_vec(namespace.to_vec())
        .map_err(|_| AgentError::BadIdent(dotted.to_owned()))?;
