...
//! config, environment, or daemon probe into one of these and hand it to the
//! engine. The engine reads and writes wherever the endpoint points
//! (WHITEPAPER §7.4): the daemon points it at its own S3 surface for cache
...
pub struct Connection {
    ...
    /// routed through it so a daemon in the path gives cache residency.
