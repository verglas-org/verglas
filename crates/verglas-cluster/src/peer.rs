...
impl PeerServer {
    ...
    /// Binds cache and fragment routes plus another internal router on one peer listener.
    pub async fn bind_with_store_fragments_and_router(
        addr: SocketAddr,
        secret: Option<String>,
        source: LocalBlockFn,
        store: LocalBlockStoreFn,
    ...
    async fn bind_inner(
        ...
        Self::bind_inner_with_router(addr, secret, source, store, fragments, None).await
    }

    /// Shared bind implementation with an optional disjoint internal router.
    async fn bind_inner_with_router(
        addr: SocketAddr,
        secret: Option<String>,
        source: LocalBlockFn,
        store: Option<LocalBlockStoreFn>,
        ...
        let mut app = app.with_state(ServerState {
            source,
            store,
            secret,
            fragments,
