...
//! (always one node, so write-back uses complete-mode consensus commit).
...
pub trait LiveMembership: Send + Sync + 'static {
    ...
    /// pod refuses admission so it never silently drops the
...
/// complete-mode consensus commit it fast-acks from local durability (#286):
...
impl LiveMembership for SingleNodeMembership {
    ...
    /// through its complete-mode consensus group (#286).
    fn is_single_node(&self) -> bool {
        true
    }
}
