...
//!
//! This module translates the standard Catalog Iceberg DTOs into durable
...
use crate::{
    MetadataStoreError, VerglasCatalog, VerglasCatalogError,
    domain::{NamespaceDocument, TableDocument, ViewDocument},
    idempotency::HostedIdempotency,
};

#[async_trait]
impl verglas_catalog_core::api::iceberg::v1::config::Service<VerglasCatalog> for VerglasCatalog {
    /// Advertises the hosted CRaft route set without persisting or vending secrets.
    async fn get_config(
        _query: verglas_catalog_core::api::iceberg::v1::config::GetConfigQueryParams,
        state: ApiContext<VerglasCatalog>,
        _request_metadata: RequestMetadata,
    ) -> Result<verglas_catalog_core::api::CatalogConfig> {
        Ok(verglas_catalog_core::api::CatalogConfig {
...
/// Returns the exact route capabilities exported by the mounted Catalog router.
fn hosted_endpoints() -> &'static [String] {
    verglas_catalog_core::api::iceberg::supported_endpoints()
}
...
/// Converts an immutable-metadata publication failure into the standard
/// Iceberg error model, preserving a commit conflict as HTTP 409.
///
/// `verglas_catalog_core::server::commit_tables::apply_commit` (called from
/// `FileIoMetadataStore::commit_table`) already classifies a failed
/// `TableRequirement` as `ErrorKind::CatalogCommitConflicts`, but
/// `commit_table`'s error path flattens that into a plain
/// `MetadataStoreError { message }` string. Left as a blanket 424
/// `MetadataPublicationFailed`, the standard Iceberg REST client never
/// recognizes this as retryable — `StatusCode::CONFLICT` (409) alone maps to
/// `ErrorKind::CatalogCommitConflicts` on the client
/// (crates/catalog/rest/src/catalog.rs), which is the one status that
/// triggers its built-in reload-and-retry loop
/// (verglas-iceberg's `commit_data_files`, #306). Without this, two
/// concurrent commits against the same table (the async ingest queue's
/// background commit racing an immediate `wait=true` append) permanently
/// fail the second writer instead of retrying it past the conflict.
fn commit_publication_error(error: MetadataStoreError) -> IcebergErrorResponse {
    if error.message.contains("CatalogCommitConflicts") {
        return ErrorModel::conflict(error.message, "CommitFailedException", None).into();
    }
...
impl NamespaceService<VerglasCatalog> for VerglasCatalog {
    ...
    async fn list_namespaces(
        ...
        let mut namespaces = state
            ...
            .into_iter()
            .filter(|(_, document)| {
                // Namespace documents store the DURABLE warehouse id (see
                // create_namespace); comparing against the raw prefix string
                // matched nothing, so every root listing came back empty and
                // iceberg-datafusion (which discovers tables ONLY through
                // list_namespaces) saw an empty catalog.
                document.warehouse_id == durable_id("warehouse", prefix.as_str())
            })
...
impl TablesService<VerglasCatalog> for VerglasCatalog {
    ...
    async fn create_table(
        ...
        let namespace = namespace_document(&state.v1_state, &prefix, &parameters.namespace)
            .await?
            .1;
        let location = match request.location.clone() {
            Some(location) => {
                state
                    .v1_state
                    .metadata()
                    .validate_location(prefix.as_str(), &location)
                    .map_err(|error| {
                        ErrorModel::failed_dependency(
                            error.message,
                            "WarehouseProfileUnavailable",
                            None,
                        )
                    })?;
                location
    ...
    async fn register_table(
        ...
        state
            .v1_state
            .metadata()
            .validate_location(prefix.as_str(), &request.metadata_location)
            .map_err(|error| {
                ErrorModel::failed_dependency(error.message, "WarehouseProfileUnavailable", None)
            })?;
    ...
    async fn load_table_credentials(
        ...
        _metadata: RequestMetadata,
    ) -> Result<verglas_catalog_core::api::LoadCredentialsResponse> {
        Ok(verglas_catalog_core::api::LoadCredentialsResponse {
    ...
    async fn commit_table(
        ...
        let (metadata_location, table_metadata) = state
            ...
            .await
            .map_err(commit_publication_error)?;
        ...
        crate::table_commit_queue::enqueue_table_commit(&parameters.table, &metadata_location);
    ...
    async fn drop_table(
        parameters: TableParameters,
        _drop: verglas_catalog_core::api::iceberg::types::DropParams,
        state: ApiContext<VerglasCatalog>,
        metadata: RequestMetadata,
    ) -> Result<()> {
        let prefix = required_prefix(parameters.prefix)?;
    ...
    async fn commit_transaction(
        ...
        let queued_commits: Vec<(TableIdent, String)> = metadata_changes
            .iter()
            .zip(published.iter())
            .filter_map(|((_, change), (metadata_location, _))| {
                change
                    .identifier
                    .clone()
                    .map(|ident| (ident, metadata_location.clone()))
            })
            .collect();
        let idempotency = metadata
            .idempotency_key()
        ...
        match transaction.commit().await {
            Ok(()) => {
                for (ident, metadata_location) in &queued_commits {
                    crate::table_commit_queue::enqueue_table_commit(ident, metadata_location);
                }
                Ok(())
            }
            Err(error) if error.is_conflict() => {
...
mod tests {
    ...
    use verglas_catalog_core::api::iceberg::v1::{NamespaceIdent, Prefix};

    use super::{commit_publication_error, namespace_key, storage_error};
    use crate::{MetadataStoreError, VerglasCatalogError};
    ...
    /// `apply_commit`'s requirement-check failure reaches this layer flattened
    /// into a `MetadataStoreError` string (see `FileIoMetadataStore::commit_table`),
    /// but its Display still embeds `ErrorKind::CatalogCommitConflicts`
    /// verbatim. That must surface as the standard Iceberg REST commit-conflict
    /// response (409 `CommitFailedException`) — the one status the standard
    /// Iceberg REST client's `update_table` recognizes as retryable
    /// (`StatusCode::CONFLICT` -> `ErrorKind::CatalogCommitConflicts`,
    /// `.with_retryable(true)`) — or a concurrent committer (the async ingest
    /// queue's background commit racing an immediate `wait=true` append) fails
    /// permanently instead of retrying past the conflict.
    #[test]
    fn commit_conflict_maps_to_commit_failed_response_not_failed_dependency() {
        let response = commit_publication_error(MetadataStoreError {
            message: "table commit validation failed: CatalogCommitConflicts (409): \
    ...
    /// A metadata-store failure unrelated to a commit conflict (for example,
    /// the backing object store rejecting a write) is not retryable and stays
    /// a 424 — only genuine commit conflicts get the 409 treatment.
    #[test]
    fn non_conflict_metadata_failure_stays_failed_dependency() {
        let response = commit_publication_error(MetadataStoreError {
            message: "metadata publication failed: connection reset".to_owned(),
        });
        assert_eq!(response.error.code, 424);
        assert_eq!(response.error.r#type, "MetadataPublicationFailed");
    }
}

#[cfg(test)]
mod warehouse_filter_tests {
    use super::*;

    /// The regression the empty-catalog bug shipped: stored namespace
    /// documents carry the durable warehouse id, so listing must compare
    /// durable id to durable id — never the raw prefix string.
    #[test]
