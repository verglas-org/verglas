...
//!
//! Route implementations use this state to make Catalog authorization
...
/// CRaft table document projected to Catalog's table authorization contract.
struct CraftTableInfo {
    warehouse_id: WarehouseId,
    namespace_id: NamespaceId,
    table_id: TableId,
    ident: verglas_catalog_core::api::iceberg::v1::TableIdent,
    protected: bool,
...
impl AuthZTableInfo for CraftTableInfo {
    ...
    fn table_ident(&self) -> &verglas_catalog_core::api::iceberg::v1::TableIdent {
        &self.ident
    }
...
/// CRaft view document projected to Catalog's view authorization contract.
...
impl AuthZViewInfo for CraftViewInfo {
    ...
    fn view_ident(&self) -> &verglas_catalog_core::api::iceberg::v1::TableIdent {
        &self.ident
    }
...
/// Converts one durable namespace document to Catalog authorization context.
fn namespace_info(
    document: NamespaceDocument,
) -> std::result::Result<NamespaceWithParent, verglas_catalog_core::api::IcebergErrorResponse> {
    let namespace_id = NamespaceId::from_str_or_internal(&document.id)
        .map_err(verglas_catalog_core::api::IcebergErrorResponse::from)?;
    let warehouse_id = WarehouseId::from_str_or_internal(&document.warehouse_id)
        .map_err(verglas_catalog_core::api::IcebergErrorResponse::from)?;
    let ident = verglas_catalog_core::api::iceberg::v1::NamespaceIdent::from_vec(document.name)
        .map_err(|error| {
            verglas_catalog_core::api::ErrorModel::internal(
...
/// Converts one durable table document and route identity to a Catalog authz resource.
fn table_info(
    document: TableDocument,
    ident: verglas_catalog_core::api::iceberg::v1::TableIdent,
    properties: HashMap<String, String>,
) -> std::result::Result<CraftTableInfo, verglas_catalog_core::api::IcebergErrorResponse> {
    Ok(CraftTableInfo {
...
/// Converts one durable view document and route identity to a Catalog authz resource.
fn view_info(
    document: ViewDocument,
    ident: verglas_catalog_core::api::iceberg::v1::TableIdent,
    properties: HashMap<String, String>,
) -> std::result::Result<CraftViewInfo, verglas_catalog_core::api::IcebergErrorResponse> {
    Ok(CraftViewInfo {
        warehouse_id: WarehouseId::from_str_or_internal(&document.warehouse_id)
            .map_err(verglas_catalog_core::api::IcebergErrorResponse::from)?,
        namespace_id: NamespaceId::from_str_or_internal(&document.namespace_id)
...
impl<A: Authorizer + Clone> AuthorizedVerglasCatalog<A> {
    ...
    /// Returns the explicit Catalog warehouse context used for auth decisions.
    pub fn warehouse(&self) -> &ResolvedWarehouse {
        &self.warehouse
    }

    /// Resolves the registered durable database identity for one Iceberg prefix.
    ///
    /// `warehouse_database_id` returns the CRaft *database* identity bound at
    /// startup (the node binds the tenant name as that database id), not a
    /// UUID: it exists so a hosted request can be routed to the right durable
    /// database, not to mint an authz warehouse id.
    /// `self.warehouse.warehouse_id` already holds the correct value,
    /// deterministically derived from the warehouse name at construction with
    /// the same `Uuid::new_v5` scheme the node uses. Reparsing
    /// the database id as a UUID here always failed for any non-UUID tenant
    /// name (`WarehouseIdIsNotUUID`) the moment `ensure_warehouse` started
    /// succeeding.
    fn warehouse_for(&self, prefix: &Prefix) -> Result<ResolvedWarehouse> {
        self.catalog
            .metadata()
            .ensure_warehouse(prefix.as_str())
            .map_err(|error| {
                ErrorModel::failed_dependency(error.message, "WarehouseProfileUnavailable", None)
            })?;
        let mut warehouse = (*self.warehouse).clone();
        warehouse.name = prefix.as_str().to_owned();
        Ok(warehouse)
    }
}
...
impl<A: Authorizer + Clone> verglas_catalog_core::api::ThreadSafe for AuthorizedVerglasCatalog<A> {}

#[async_trait::async_trait]
impl<A: Authorizer + Clone>
    verglas_catalog_core::api::iceberg::v1::config::Service<AuthorizedVerglasCatalog<A>>
    for AuthorizedVerglasCatalog<A>
{
    /// Returns the same CRaft-hosted catalog configuration as the storage service.
    async fn get_config(
        query: verglas_catalog_core::api::iceberg::v1::config::GetConfigQueryParams,
        state: ApiContext<AuthorizedVerglasCatalog<A>>,
        _metadata: RequestMetadata,
    ) -> Result<verglas_catalog_core::api::CatalogConfig> {
        let prefix = query.warehouse.ok_or_else(|| {
            verglas_catalog_core::api::IcebergErrorResponse::from(ErrorModel::bad_request(
        ...
        state
            ...
            .catalog()
            .metadata()
            .ensure_warehouse(&prefix)
            .map_err(|error| {
                ErrorModel::failed_dependency(error.message, "WarehouseProfileUnavailable", None)
            })?;
        // The warehouse profile's endpoint is this machine's loopback cache
        // listener (the commit write path). Catalog CLIENTS cannot reach
        // loopback, so vend the deployment's public S3 endpoint as an
        // s3.endpoint override — the Iceberg REST spec ranks config
        // overrides above table properties, so every client's FileIO uses
        // it while catalog's own IO keeps the local path.
        let mut overrides = HashMap::new();
        if let Ok(public) = std::env::var("VERGLAS_PUBLIC_S3_ENDPOINT")
            && !public.is_empty()
        {
            overrides.insert("s3.endpoint".to_owned(), public);
        }
        Ok(verglas_catalog_core::api::CatalogConfig {
            defaults: HashMap::from([("prefix".to_owned(), prefix)]),
            overrides,
            endpoints: verglas_catalog_core::api::iceberg::supported_endpoints().to_vec(),
        })
    }
}

/// Converts an authorizer denial into the standard hosted Iceberg forbidden response.
fn denied() -> verglas_catalog_core::api::IcebergErrorResponse {
...
/// Requires the wrapper's Catalog warehouse action before delegating to CRaft storage.
async fn warehouse_action<A: Authorizer + Clone>(
    ...
    let warehouse = state.warehouse_for(prefix)?;
    let allowed = state
        .authorizer()
...
async fn namespace_hierarchy<A: Authorizer + Clone>(
    state: &AuthorizedVerglasCatalog<A>,
    prefix: &Prefix,
    ident: &verglas_catalog_core::api::iceberg::v1::NamespaceIdent,
) -> Result<NamespaceHierarchy> {
    let (_, leaf) = namespace_document(state.catalog(), prefix, ident).await?;
    let all = state
        .catalog()
...
async fn namespace_action<A: Authorizer + Clone>(
    ...
    let warehouse = state.warehouse_for(prefix)?;
    let hierarchy = namespace_hierarchy(state, prefix, ident).await?;
    let allowed = state
        .authorizer()
...
async fn table_action<A: Authorizer + Clone>(
    ...
    let warehouse = state.warehouse_for(prefix)?;
    let hierarchy = namespace_hierarchy(state, prefix, &ident.namespace).await?;
    ...
    let allowed = state
        .authorizer()
        .is_allowed_table_action(metadata, None, &warehouse, &hierarchy, &resource, action)
        .await
        .map_err(|error| {
            ErrorModel::service_unavailable(
                format!("authorization backend unavailable: {error:?}"),
...
async fn view_action<A: Authorizer + Clone>(
    ...
    let allowed = state
        .authorizer()
        .is_allowed_view_action(metadata, None, &warehouse, &hierarchy, &resource, action)
        .await
        .map_err(|error| {
            ErrorModel::service_unavailable(
                format!("authorization backend unavailable: {error:?}"),
    ...
    for AuthorizedVerglasCatalog<A>
{
    ...
    async fn create_namespace(
        ...
        warehouse_action(&state.v1_state, &metadata, &prefix, action).await?;
    ...
    for AuthorizedVerglasCatalog<A>
{
    ...
    async fn commit_table(
        ...
        let (updated_properties, removed_properties) =
            verglas_catalog_core::server::tables::parse_table_property_updates(&request.updates);
        table_action(
            &state.v1_state,
            &metadata,
            &prefix,
            ...
            CatalogTableAction::Commit {
                ...
                target_refs: Arc::new(
                    verglas_catalog_core::server::commit_tables::refs_from_updates(
                        &request.updates,
                    ),
                ),
                update_kinds: Arc::new(verglas_catalog_core::server::commit_tables::update_kinds(
                    &request.updates,
                )),
            },
        )
    ...
    async fn drop_table(
        parameters: TableParameters,
        drop_params: verglas_catalog_core::api::iceberg::types::DropParams,
        state: ApiContext<Self>,
        metadata: RequestMetadata,
    ) -> Result<()> {
        let prefix = required_prefix(parameters.prefix.clone())?;
    ...
    async fn commit_transaction(
        ...
        for change in &request.table_changes {
            ...
            let (updated_properties, removed_properties) =
                verglas_catalog_core::server::tables::parse_table_property_updates(&change.updates);
            table_action(
                &state.v1_state,
                &metadata,
                &prefix_value,
                ...
                CatalogTableAction::Commit {
                    ...
                    update_kinds: Arc::new(
                        verglas_catalog_core::server::commit_tables::update_kinds(&change.updates),
                    ),
                },
            )
            .await?;
    ...
    for AuthorizedVerglasCatalog<A>
{
    ...
    async fn commit_view(
        ...
        let prefix = required_prefix(parameters.prefix.clone())?;
        let (updated_properties, removed_properties) =
            verglas_catalog_core::server::views::commit::parse_view_property_updates(
                &request.updates,
            );
        view_action(
            &state.v1_state,
...
mod tests {
    ...
    fn malformed_durable_resource_identity_fails_closed() {
        let error = table_info(
            ...
            TableIdent {
                namespace: verglas_catalog_core::api::iceberg::v1::NamespaceIdent::new(
                    "analytics".to_owned(),
                ),
                name: "events".to_owned(),
            },
