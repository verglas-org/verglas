...
- #39: Added an explicit dashboard refresh route that reloads the source table through the catalog and rewrites only its owned Rill model with the latest metadata location. Refresh refuses missing or inconsistent ownership instead of touching unrelated resources.
