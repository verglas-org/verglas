...
use verglas_catalog_io::{
    CatalogStorage, ErrorKind, ReadError, StorageBackend, execute_with_parallelism,
};
...
async fn create_memory_storage() -> anyhow::Result<(StorageBackend, TestConfig)> {
    let storage = StorageBackend::Memory(verglas_catalog_io::memory::MemoryStorage::new());
...
async fn create_s3_storage() -> anyhow::Result<(StorageBackend, TestConfig)> {
    let bucket = std::env::var("VERGLAS_CATALOG_TEST__S3_BUCKET")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__S3_BUCKET not set"))?;
    let region = std::env::var("VERGLAS_CATALOG_TEST__S3_REGION")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__S3_REGION not set"))?;
    let access_key = std::env::var("VERGLAS_CATALOG_TEST__S3_ACCESS_KEY")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__S3_ACCESS_KEY not set"))?;
    let secret_key = std::env::var("VERGLAS_CATALOG_TEST__S3_SECRET_KEY")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__S3_SECRET_KEY not set"))?;
    let endpoint = std::env::var("VERGLAS_CATALOG_TEST__S3_ENDPOINT").ok();

    let s3_settings = verglas_catalog_io::s3::S3Settings::builder()
        .endpoint(
            endpoint
                .map(|e| e.parse())
                .transpose()
        ...
        .build();
    let s3_auth =
        verglas_catalog_io::s3::S3Auth::AccessKey(verglas_catalog_io::s3::S3AccessKeyAuth {
    ...
    let base_path = format!(
        "s3://{}/verglas-catalog-io-integration-tests/{}",
        bucket,
        uuid::Uuid::new_v4()
    );
...
async fn create_oss_storage() -> anyhow::Result<(StorageBackend, TestConfig)> {
    let bucket = std::env::var("VERGLAS_CATALOG_TEST__OSS_BUCKET")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__OSS_BUCKET not set"))?;
    let region = std::env::var("VERGLAS_CATALOG_TEST__OSS_REGION")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__OSS_REGION not set"))?;
    let endpoint = std::env::var("VERGLAS_CATALOG_TEST__OSS_ENDPOINT")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__OSS_ENDPOINT not set"))?;
    let access_key = std::env::var("VERGLAS_CATALOG_TEST__OSS_ACCESS_KEY_ID")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__OSS_ACCESS_KEY_ID not set"))?;
    let secret_key = std::env::var("VERGLAS_CATALOG_TEST__OSS_SECRET_ACCESS_KEY")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__OSS_SECRET_ACCESS_KEY not set"))?;
...
async fn create_adls_storage() -> anyhow::Result<(StorageBackend, TestConfig)> {
    let client_id = std::env::var("VERGLAS_CATALOG_TEST__AZURE_CLIENT_ID")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__AZURE_CLIENT_ID not set"))?;
    let tenant_id = std::env::var("VERGLAS_CATALOG_TEST__AZURE_TENANT_ID")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__AZURE_TENANT_ID not set"))?;
    let client_secret = std::env::var("VERGLAS_CATALOG_TEST__AZURE_CLIENT_SECRET")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__AZURE_CLIENT_SECRET not set"))?;

    let account = std::env::var("VERGLAS_CATALOG_TEST__AZURE_STORAGE_ACCOUNT_NAME")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__AZURE_STORAGE_ACCOUNT_NAME not set"))?;
    let filesystem = std::env::var("VERGLAS_CATALOG_TEST__AZURE_STORAGE_FILESYSTEM")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__AZURE_STORAGE_FILESYSTEM not set"))?;

    let settings = verglas_catalog_io::adls::AzureSettings {
        authority_host: None,
        cloud_location: verglas_catalog_io::adls::CloudLocation::Public {
            account: account.clone(),
        },
    };
    let auth = verglas_catalog_io::adls::AzureAuth::ClientCredentials(
        verglas_catalog_io::adls::AzureClientCredentialsAuth {
            client_id,
            client_secret,
            tenant_id,
        },
    ...
    let base_path = format!(
        "abfss://{filesystem}@{account}.dfs.core.windows.net/verglas-catalog-io-integration-tests/{}",
        uuid::Uuid::new_v4()
    );
    let config = TestConfig { base_path };
...
async fn create_gcs_storage(bucket_env_var: &str) -> anyhow::Result<(StorageBackend, TestConfig)> {
    let credential = std::env::var("VERGLAS_CATALOG_TEST__GCS_CREDENTIAL")
        .map_err(|_| anyhow::anyhow!("VERGLAS_CATALOG_TEST__GCS_CREDENTIAL not set"))?;
    let bucket =
        std::env::var(bucket_env_var).map_err(|_| anyhow::anyhow!("{bucket_env_var} not set"))?;

    let credential_file: verglas_catalog_io::gcs::CredentialsFile =
        serde_json::from_str(&credential)
            .map_err(|e| anyhow::anyhow!("Failed to parse GCS credential file: {e}"))?;

    let settings = verglas_catalog_io::gcs::GCSSettings {};
    let auth = verglas_catalog_io::gcs::GcsAuth::CredentialsFile {
    ...
    let base_path = format!(
        "gs://{bucket}/verglas-catalog-io-integration-tests/{}",
        uuid::Uuid::new_v4()
    );
    let config = TestConfig { base_path };
    ...
    ($test_name:ident, $test_fn:ident) => {
        pastey::paste! {
            ...
            fn [<$test_name _gcs_regular>]() -> anyhow::Result<()> {
                execute_in_common_runtime(async {
                    let (storage, config) = create_gcs_storage("VERGLAS_CATALOG_TEST__GCS_BUCKET").await?;
                    $test_fn(&storage, &config).await
                })
            }
            ...
            fn [<$test_name _gcs_hns>]() -> anyhow::Result<()> {
                execute_in_common_runtime(async {
                    let (storage, config) = create_gcs_storage("VERGLAS_CATALOG_TEST__GCS_HNS_BUCKET").await?;
                    $test_fn(&storage, &config).await
                })
            }
        }
...
//             Box::pin(create_gcs_storage("VERGLAS_CATALOG_TEST__GCS_BUCKET"))
//         })
//         .await
//     })
// }
...
//     execute_in_common_runtime(async {
//         test_initialization_performance_impl(|| {
//             Box::pin(create_gcs_storage("VERGLAS_CATALOG_TEST__GCS_HNS_BUCKET"))
//         })
//         .await
//     })
// }
...
/// Catalog REST receives when a client provides a URL-style location.
