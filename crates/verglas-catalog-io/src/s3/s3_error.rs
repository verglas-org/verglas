...
pub(crate) fn parse_delete_error(err: SdkError<DeleteObjectError>, location: &str) -> IOError {
    ...
    let catalog_kind = e
        ...
        .map_or(ErrorKind::Unexpected, |kind| kind.as_catalog_kind());
    ...
    IOError::new(catalog_kind, msg, location.to_string()).set_source(e)
}
...
pub(crate) fn parse_aws_sdk_error(err: &aws_sdk_s3::types::Error, location: &str) -> IOError {
    ...
    IOError::new(catalog_kind, msg, location.to_string())
}
...
impl S3ErrorCode {
    fn as_catalog_kind(&self) -> ErrorKind {
        match self {
