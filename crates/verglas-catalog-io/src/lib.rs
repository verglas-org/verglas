...
pub trait CatalogFileWrite: std::fmt::Debug + Send + Sync + 'static {
    ...
    Self: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    /// Return a `CatalogFileWrite` that can be used to `write` a file chunk by chunk until
    /// it is `closed`.
    ///
    /// `CatalogFileWrite` needs an inner state that depends on the backend, so it is
    ...
    async fn writer(&self, path: &str) -> Result<Box<dyn crate::CatalogFileWrite>, WriteError>;
    ...
    ///
    /// Default implementation calls [`CatalogStorage::metadata`] and maps
...
#[async_trait::async_trait]
impl CatalogStorage for StorageBackend {
    async fn delete(&self, path: &str) -> Result<(), DeleteError> {
        match self {
            #[cfg(feature = "storage-s3")]
...
// Delegating `CatalogStorage` impls for smart pointers.
//
...
//   * `Arc<T>` / `Box<T>` with `T: CatalogStorage` → `&T`.
//   * `Arc<dyn CatalogStorage>` / `Box<dyn CatalogStorage>` →
//     `&dyn CatalogStorage`, dispatching through the vtable. Trait
...
//
// Adding a new method on `CatalogStorage` therefore requires updating
// only this one place.
macro_rules! impl_catalog_storage_delegating {
    ( $( ( $($impl_header:tt)* ) ; )+ ) => {
        $(
            #[async_trait::async_trait]
            $($impl_header)* {
                ...
                async fn writer(
                    &self,
                    path: &str,
                ) -> Result<Box<dyn $crate::CatalogFileWrite>, WriteError> {
...
impl_catalog_storage_delegating! {
    (impl<T> CatalogStorage for Arc<T> where T: CatalogStorage) ;
    (impl<T> CatalogStorage for Box<T> where T: CatalogStorage) ;
    (impl CatalogStorage for Arc<dyn CatalogStorage>) ;
