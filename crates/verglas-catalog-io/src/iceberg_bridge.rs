...
use crate::{
    BoxStream, CatalogFileWrite, CatalogStorage, DeleteBatchError, DeleteError, ReadError,
    WriteError,
};
...
pub struct IcebergStorageBridge {
    verglas_catalog_io: Arc<dyn CatalogStorage>,
}

impl IcebergStorageBridge {
    #[must_use]
    pub fn new(verglas_catalog_io: Arc<dyn CatalogStorage>) -> Self {
        Self { verglas_catalog_io }
    }
}

/// Intentional hard fail for Ser/Deser because `verglas_catalog_io` cannot be ser/deser,
...
impl Storage for IcebergStorageBridge {
    ...
    async fn metadata(&self, path: &str) -> iceberg::Result<FileMetadata> {
        let info = self.verglas_catalog_io.metadata(path).await?;
    ...
    async fn read(&self, path: &str) -> iceberg::Result<bytes::Bytes> {
        self.verglas_catalog_io.read(path).await.map_err(Into::into)
    }

    async fn reader(&self, path: &str) -> iceberg::Result<Box<dyn iceberg::io::FileRead>> {
        Ok(Box::new(IcebergFileRead {
            verglas_catalog_io: self.verglas_catalog_io.clone(),
            path: path.to_string(),
        }))
    }
    ...
    async fn writer(&self, path: &str) -> iceberg::Result<Box<dyn iceberg::io::FileWrite>> {
        let inner = self.verglas_catalog_io.writer(path).await?;
    ...
    // possible to hold `Weak<Self>` in `CatalogStorageBridge`, but would imply `new() -> Arc<Self>`
...
pub(crate) struct IcebergFileWrite {
    inner: Box<dyn CatalogFileWrite>,
}
