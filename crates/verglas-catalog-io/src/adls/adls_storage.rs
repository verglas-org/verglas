...
use crate::{
    CatalogFileWrite, CatalogStorage, DeleteBatchError, DeleteError, ErrorKind, FileInfo, IOError,
    InvalidLocationError, Location, ReadError, WriteError,
...
impl CatalogStorage for AdlsStorage {
    ...
    async fn writer(&self, path: &str) -> Result<Box<dyn CatalogFileWrite>, WriteError> {
        let adls_location = AdlsLocation::try_from_str(path, true)?;
        require_key(&adls_location)?;
        let client = self.get_file_client(&adls_location)?;
        create_file(&client, path).await?;
...
///   2. verglas-catalog-io's contract is that callers only ever observe finished
...
#[async_trait::async_trait]
impl CatalogFileWrite for AdlsFileWrite {
