...
impl<W: CatalogWatcher + 'static> WarmingCoordinator<W> {
    ...
    pub fn new(warmer: Arc<Warmer>, watcher: Arc<W>) -> WarmingCoordinator<W> {
        WarmingCoordinator { warmer, watcher }
    }
    ...
    pub async fn warm_all(&self) {
        warm_all(Arc::clone(&self.warmer), Arc::clone(&self.watcher)).await;
    }
    ...
    pub fn spawn(self) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            warm_all(Arc::clone(&warmer), Arc::clone(&watcher)).await;
            loop {
                match events.recv().await {
                    Ok(change) if change.new_snapshot.is_some() => {
                        warm_table(Arc::clone(&warmer), Arc::clone(&watcher), change.table).await;
                    }
                    Ok(_) => {}
...
fn target_for<W: CatalogWatcher>(watcher: &W, table: &TableIdent) -> Option<WarmTarget> {
...
async fn warm_all<W: CatalogWatcher>(warmer: Arc<Warmer>, watcher: Arc<W>) {
    for table in watcher.watched_tables() {
        warm_table(Arc::clone(&warmer), Arc::clone(&watcher), table).await;
    }
}

/// Warms one table's current pointer if it resolves. Errors are logged and
/// skipped — warming is best-effort background work.
async fn warm_table<W: CatalogWatcher>(warmer: Arc<Warmer>, watcher: Arc<W>, table: TableIdent) {
    let Some(target) = target_for(watcher.as_ref(), &table) else {
        return;
    };
