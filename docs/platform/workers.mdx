...
The SDK contract defines these trigger producers and subscriptions. Every run receives one CloudEvents 1.0 envelope:

| Trigger | Dispatch payload | Typical use |
| --- | --- | --- |
| `cron` | `org.verglas.schedule.tick` with logical interval data | Scheduled ingestion and backfill |
| `webhook` | `org.verglas.http.request` with the complete request in `data` | HTTP ingestion |
| `event` | Any matching CloudEvent type, source, and subject | Event-driven transformations |

The portable CLI manifest registers `manual`, `cron`, `webhook`, and `event` workers. It also registers local-only `follow` workers. Iceberg snapshot commits use the CloudEvent type `org.apache.iceberg.snapshot.committed` and the dotted table name as `subject`.
...
Use a stable input identifier as the commit idempotency key. An Iceberg-event worker can use the input snapshot ID:

```ts
const result = await ctx.client.table(ctx.output).append(rows, {
  idempotencyKey: `analytics.orders@${(ctx.trigger.data as { snapshotId: string }).snapshotId}`,
});
```
