...
The portable CLI manifest registers `manual`, `cron`, `webhook`, and `data_change` workers. It also registers local-only `follow` workers. HTTP and data-update events use the same bounded worker-run contract.
