...
The dirty journal is replicated with those fragments. Any cache node can
discover and reconstruct the object; reads do not depend on the coordinator
...
origin. It retries indefinitely with capped backoff, so an extended outage does
not strand a journal until process restart. Cache-node startup recovers and
serves ring state before probing the origin. Scrubbing verifies fragment checksums and repairs damaged fragments
