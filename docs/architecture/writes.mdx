...
If the cluster cannot meet its staging quorum, Verglas writes through to the
origin before acknowledging. This increases latency but preserves correctness.
It never acknowledges an object that exists only in volatile memory.
