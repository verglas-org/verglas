...
guessing whether bytes are still current. In the six-product platform, Catalog
owns REST metadata and commits, while Sink owns Parquet delivery and calls
Catalog. Property-graph and vector-index builds remain independent semantic
operations over Iceberg tables; a Durable Object does not mutate them.
...
Operators who already receive commit events from their catalog can skip the
shallow REST cache and drive warming from those events instead.
