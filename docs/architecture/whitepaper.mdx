...
A cache node is one `verglas-cache-node` process. Its S3 front end validates
SigV4 requests and maps supported operations onto separate read and write
interfaces. Its private admin listener exposes health, metrics, cache-local
catalog access, and storage ingress. External Iceberg query and write processes
remain independently deployable. The cache backend applies concurrency limits,
retry policy, and circuit breaking before issuing origin requests. Worker/DO
execution and the Stream-to-Iceberg products use their own bindings; the cache
engine, Iceberg layer, and semantic services remain independent interfaces.
...
The cache fleet hosts independent Raft groups for managed warehouse catalog and
Neon timeline state instead of one global log. Durable Objects are outside that
substrate: each DO has one isolated event executor and one remote Turso database,
with pushes admitted only through the external lease-fenced sync ingress.
Catalog and timeline groups elect leaders and commit concurrently, so one busy
timeline or warehouse does not serialize unrelated tenants. Requests for those
groups may arrive at any cache node and are forwarded to the known leader. A
client retry carries the same request identity and never becomes a second
command.
...
Figure 4 shows the cache's handling of one illustrative range read. An external
query engine plans through the walk of Figure 2 — manifest pruning selects the
file, the Parquet footer selects the column chunk's byte range — and issues an
ordinary range GET. That S3 serving path never parses SQL. DO SQL executes in
its Turso database, while Pipeline SQL is a separate stateless transform; neither
turns the cache into a DO table provider. Two things then happen to a range
request independently: the serving path resolves it to version-keyed blocks,
and the classification path maps it to its logical table, partition, and column
and records the access in the heat ledger, off the serving path.
...
This section specifies the target hosted platform contract. Hosted access,
placement, lifecycle, integrations, and agent services live in `verglas-cloud`;
the public repository supplies the Worker/DO component contract and the
independent cache, Iceberg, catalog, and semantic infrastructure that products
use. A target description is not a claim that its acceptance tests have passed.

Verglas has exactly six products, built from two primitives:

1. **Worker** — stateless, pool-executed public ingress. It has no identity or
   storage; bindings are its authority.
2. **Durable Object (DO)** — a named stateful Worker class with strictly
   serialized events and one embedded Turso `0.7.2` database. The local
   database is a working copy. One remote Turso database belongs to each DO;
   an external lease-fenced sync ingress validates placement before forwarding
   `push()`.
3. **Stream** — a prebuilt DO that durably appends ordered JSON. It does not
   offload, execute SQL, own consumer offsets, or call Iceberg/Catalog.
4. **Pipeline** — a prebuilt Worker/DO that owns a Stream cursor, executes
   stateless SQL transforms, batches records, and retries Sink delivery.
5. **Sink** — a prebuilt Worker/DO with idempotent delivery. The first adapter
   writes Parquet and commits Iceberg through a Catalog binding.
6. **Catalog** — a prebuilt Worker/DO exposing Iceberg REST through the
   existing Verglas catalog and object-storage libraries.

The platform follows Cloudflare's Worker/DO authoring surface
([Cloudflare compatibility](/docs/architecture/cloudflare-compat)). The
normative definitions and immutable PP1–PP10 criteria are in the [Worker
primitives platform](/docs/architecture/primitives-platform).

The DO boundary keeps user code close to its state without making it a
lakehouse or semantic authority. Its Turso transaction owns only object state,
KV, alarms, attachments, and optional Stream outbox rows. A Worker holds no
capabilities beyond its bindings. Cache, Iceberg, Catalog, and semantic
graph/vector infrastructure remain independently deployable; Sink and Catalog
own the Iceberg data and metadata boundaries.

Three doctrines run through the platform:

- **Product separation.** Worker and DO are the primitives. Stream, Pipeline,
  Sink, and Catalog are explicit prebuilt Worker/DO deployments, not hidden
  runtimes, queues, databases, or direct offload paths.
- **Durability before effects.** A DO event commits to Turso WAL and confirms
  the remote `push()` boundary before releasing gated effects. Selected changes
  use a transactional outbox; Stream deduplication makes a resend after a
  crash harmless.
- **Connections do not pin compute.** HTTP and WebSocket connections terminate
  at the gateway. The gateway can hold a WebSocket while a DO hibernates and
  deliver each later message after wake. Catalog and Sink operations use their
  own bindings rather than DO-to-Iceberg calls.
...
flowchart LR
    ...
    S --> P["Pipeline\nStream cursor + SQL + batch"]
    P --> K["Sink\nidempotent delivery"]
    K --> C["Catalog\nIceberg REST"]
    C --> I["Iceberg libraries"]
    I --> O[("object storage")]
    X["independent cache / semantic services"] --> O
```

*Figure 7: the target product topology. Worker code reaches stateful and
prebuilt products through bindings. DO state is durable in Turso; Pipeline,
Sink, and Catalog own the stream-to-Iceberg path, while cache and semantic
services remain independent infrastructure.*

### 7.1 Deployment records and grants

A target deployment record names a Worker or prebuilt product, its immutable
component digest, configuration and secret references, bindings, and
placement. A DO record additionally names the one remote Turso database and
its sync credentials. Records are the unit for deployment, audit, pause,
resume, and placement; they do not turn DO state into a separate run log or
Iceberg catalog.

A grant lists the resources a component may reach. A Worker has only the
bindings in its environment. A DO grant covers its object state and named
product bindings; it does not silently grant access to another DO, an Iceberg
table, or a graph/vector index. Pipeline, Sink, and Catalog records carry the
bindings needed for their own product boundaries.

### 7.2 DO component and event contract

Tenant code compiles to a WASM component targeting the `verglas:do-worker` WIT
world. The handler exports `init` before `fetch`, `alarm`,
`websocket-message`, and `websocket-close`. The host binds `get`, `put`,
`delete`, `list`, `sql`, and alarm operations to the DO's embedded Turso
`0.7.2` database. Socket operations bind to gateway connection state. These
imports are capabilities, not network services.

The input gate admits one HTTP request, alarm, WebSocket message, or close event
at a time. Turso serializes the event transaction locally; the host confirms
`push()` through the external lease-fenced sync ingress before releasing the
event result and gated socket effects. No event observes another event's
uncommitted state. A handler error cannot undo state that already reached the
Turso durability boundary.

Selected committed SQL changes may write deterministic outbox rows in the same
Turso transaction. After push, the host sends them to a bound Stream. Stream
deduplicates `(object-id, event-sequence, record-index)` identities; marking an
outbox row delivered happens only after Stream confirmation. This is publication
coordination, not direct Iceberg offload or a dispatch queue.

The component artifact is immutable and content-addressed. A target host fetches
it by digest and verifies the digest before instantiation. Compiled artifacts
may be cached outside the isolation boundary; caching code never changes DO
state authority. JavaScript and TypeScript use ComponentizeJS/StarlingMonkey,
while Rust and other component-toolchain languages target the WIT world
directly. These build and lifecycle details remain Target until their
corresponding acceptance evidence exists.

### 7.3 Durable state and adjacent infrastructure

A DO's KV, SQL tables, alarm, attachments, and optional outbox are its durable
cross-event state. `set-alarm` replaces the one alarm row, `get-alarm` reads its
deadline, and `delete-alarm` clears it in the event transaction. The platform
derives wake scheduling from that committed row; alarm delivery uses the same
serialized event path and does not create a separate queue.

The Turso local database uses ordinary `0.7.2` WAL. A remote database dedicated
to the DO is the machine-loss recovery source. The external sync ingress checks
the current placement lease before forwarding `push()`. A takeover revokes the
prior owner before the new owner opens or pushes the database. The Turso fence
is the only DO durability boundary; the cache ring's independent consensus and
object-storage offload remain cache infrastructure, not DO commit machinery.

The other data layers keep their existing responsibilities. The cache serves
S3 ranges, routes ownership, warms metadata, stages managed writes, and drains
its own objects. Existing Iceberg and catalog libraries provide REST metadata
and table commits to the Catalog product. Semantic graph/vector services keep
their independent APIs and explicit index-build rules over Iceberg data. A DO
cannot publish a table snapshot, Parquet file, graph mutation, or vector index
as a side effect of its SQL transaction.

### 7.4 Placement, isolation, and scale-to-zero

The target platform runs each active DO in one isolated execution environment.
The environment owns one event executor, one local Turso working copy and sync
sidecars, and one private event endpoint. It does not run Raft, elect a DO
leader, or host a custom pager. The lifecycle supervisor passes the placement
lease to the external Turso sync ingress and fences routing after a crash or
restore; a stale owner cannot open or push the DO's remote database.

A microVM can provide the isolation substrate, and suspend/resume can reduce
cold-start cost. Before suspension, the host drains required Stream outbox rows
and confirms the final Turso push. On wake, it opens the DO's one remote
database and restores a local working copy. A retained local copy is an
acceleration only; remote Turso state remains the recovery source. Mutable state
and execution are never shared with another tenant payload.

The isolation boundary is a WASM sandbox inside the selected execution
environment inside the placement fence. Tenant code reaches host state only
through WIT imports. Cache checkpoints, Iceberg objects, semantic indexes, and
cache offload are independent lifecycle concerns and never substitute for the
DO's Turso durability boundary.

### 7.5 Gateway, bindings, and hibernation

HTTP and WebSocket connections terminate at the platform gateway, never inside
the guest. Public routes execute Worker code first. The gateway resolves a
binding name to a DO and wakes it on demand; DOs are reachable only through
Worker or DO bindings, not direct tenant routes. It can hold WebSockets across
hibernation and deliver each message as a serialized DO event. A connection
attachment is a small blob, at most 16 KiB, persisted in the DO's Turso state.

The same binding model composes the prebuilt products. A Worker can call
`env.STREAM.send(records)`. A Pipeline reads its own Stream cursor and calls a
Sink. A Sink calls the Catalog binding for Iceberg metadata operations. None of
these calls makes Stream a queue engine or gives a DO a direct Iceberg path.
Catalog notifications and Sink receipts belong to those products, not to the
DO event protocol.

### 7.6 Cache-pathed and Iceberg I/O

The cache remains an independent S3 and object-storage layer. Query engines and
product adapters may route their S3 requests through it, so object-versioned
cache identity, ring ownership, budgets, staging, and background offload keep
the guarantees described in §§3–6. This wiring is not a DO storage dependency:
DO SQL state remains in Turso, and no component assumes that a key is locally
owned.

Iceberg ingest follows the existing client path. A Sink writes deterministic
Parquet data files and asks the Catalog binding to publish an Iceberg snapshot.
The Catalog uses the existing Iceberg and object-storage libraries. A customer
binding remains authoritative and usable while Verglas is stopped; a managed
binding keeps its Verglas-owned object layout and read path until explicit
detach. Neither binding turns a DO transaction into an Iceberg commit or permits
autonomous graph/vector publication.

### 7.7 Layering

The hosted control plane resolves product resources and DO names to deployment
records. A Worker is the public ingress and reaches a DO, Stream, Pipeline,
Sink, or Catalog through an environment binding. A Pipeline consumes ordered
Stream ranges and advances its cursor only after every targeted Sink confirms a
deterministic batch. A Sink owns delivery idempotency; the Catalog owns Iceberg
REST and metadata publication.

The cache, Iceberg, catalog, and semantic graph/vector layers remain available
to external clients through their existing APIs. Their consistency and
lifecycle contracts are not silently replaced by DO state. Managed bindings
retain the cache and detach guarantees of §§3–6; customer bindings retain the
catalog and object-store semantics of their owners. No product falls back to a
hidden database, queue engine, direct DO offloader, or autonomous customer-table
mutation.
...
The platform layer applies the same discipline to stateful execution. Each
Durable Object is a named Worker class backed by Turso `0.7.2`, with one remote
Turso database and an external lease-fenced sync ingress. Serialized events
commit KV and SQL state in Turso, and optional Stream publication uses a
transactional outbox with producer-identity deduplication. Worker, Stream,
Pipeline, Sink, and Catalog remain separate bindings; none hides a custom pager,
Arrow commit envelope, direct DO offloader, or semantic mutation domain.
