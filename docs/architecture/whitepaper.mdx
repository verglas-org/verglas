...
service. Rill owns its project, metrics views, and dashboards, but uses Verglas
as a live OLAP query integration. Each interactive request sends parameterized
SQL to the Verglas query role and receives an Arrow stream. Verglas resolves
the catalog's current Iceberg state for that execution, so no copied model,
pinned metadata snapshot, refresh lifecycle, or dashboard registry exists.
This preserves the boundary: Rill presents and defines analytics resources;
Verglas is the catalog-aware cache and execution engine.
