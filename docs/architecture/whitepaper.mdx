...
We propose Verglas, an Iceberg-aware local tier that colocates an S3-compatible storage endpoint with compute and serves frequently accessed bytes from DRAM and NVMe. In a self-hosted deployment, one REST composition exposes the cache S3 endpoint, query/write API, and database-scoped shallow Iceberg REST proxies; each proxy caches successful metadata reads, observes changes, and writes mutations through to that database's bound catalog. Verglas never becomes the catalog system of record. Verglas keys cached blocks by storage binding and object version, yielding correctness properties that format-blind systems cannot achieve at any configuration. An erasure-coded, quorum-acknowledged write-back path stages writes durably in front of object storage, which remains the system of record. Above the cache, Verglas runs a pipeline platform (§7). A deployment is one record — kind, triggers, and placement — carrying user code written against one SDK; pipeline I/O is routed through the shared tenant cache, so the working set of all databases and workloads competes in one heat budget.
...
talk to them — `/v1/databases/{database}/query`, graph traversal, vector
search, and table scans. SQL workers receive only the selected database's
catalog configuration; there is no process-global query catalog.
...
The design separates a data path from a table-control path. The data path implements the S3 operations used to read and write objects. The control path has two consistency contracts. Eventual mode polls any Iceberg REST catalog. Strong mode accepts the transactional catalog's ordered pointer mutation, persists that compact record through the cache ring's erasure-coded quorum, and fences each catalog read until the local prepared view reaches the committed ring sequence. Both paths parse table metadata and schedule metadata warming and snapshot-driven prefetch. Clients still resolve namespaces, schemas, snapshots, and commits against the catalog system of record. The self-hosted REST composition may expose a shallow cached Iceberg REST proxy, but it implements no catalog mutation semantics and never becomes the catalog system of record. Logical SQL and table-write requests are dispatched to isolated `verglas-query` and `verglas-write` roles, whose object I/O returns through the S3 cache endpoint. This lets table-level information guide a byte-addressed cache without embedding query or write engines in the storage server.
...
Host-managed scale-to-zero uses an admission fence, not a ring drain. The host authenticates to each workload service's private quiescence API, atomically closes foreground admission across S3 reads, attached block, fragment storage, object writer, and WAL-keeper planes, then waits for already-accepted work to reach zero before stopping the processes. A generation lease prevents a stale failed-stop handler from reopening a newer fence. Retained NVMe is reattached on restart: fragment nodes rebuild their disk indexes without eagerly copying cached bytes into DRAM, while the writer and WAL keeper recover acknowledged state from the ring's durable records and resume background propagation. Catalog polling, recovery, warming, and propagation do not pin the process awake; their durable inputs remain on NVMe or in the origin.
...
**Commit-driven warming.** A demand cache faults metadata in one miss at a time, on the first query after every change. In eventual mode the table-control path polls the Iceberg REST catalog. In strong mode the catalog sends an ordered mutation naming the new immutable metadata location; Verglas EC-persists and applies that pointer before acknowledgment, invalidates prepared catalog responses, and makes lagging query nodes replay through the ring watermark before serving. The next fenced read loads the catalog's current response rather than attempting to re-fetch historical versions that an Iceberg REST catalog no longer exposes. On startup and after each observed commit, the control path walks the new metadata — table metadata file, current manifest list, manifests, and Parquet footers — through the normal cache interface into the metadata store. Catalog availability and mutation semantics remain the catalog service's responsibility. Footer warming uses a bounded suffix request, with a second request only when the footer exceeds the initial window. Warming concurrency and byte rate are bounded independently, and if a table's metadata footprint exceeds the configured budget the warmer stops admitting rather than thrashing the store.
...
A write transaction against the buffer is therefore: enter through the workload's writer, encode, append self-describing fragment records on distinct nodes, and acknowledge after `w` distinct fragment logs report a durable watermark covering the transaction. The durable records are the recovery journal; there is no additional coordinator-local journal in the acknowledgment path. The remainder of this section describes that path and its failure handling.

### 6.1 Quorum staging

Encoding and placement stream one stripe at a time, bounding memory use independently of object size. Fragment capacity is reserved from each node's configured NVMe ceiling. Cache nodes expose a private fragment-log protocol, not an application write endpoint. Each node keeps preallocated, append-only segment files. A fragment record contains the stream or object identity, transaction sequence or LSN range, coding geometry, fragment index, length, and checksum, so a quorum of records is sufficient to discover and reconstruct acknowledged data after loss of the writer process.

The writer orders candidate nodes by rendezvous hash and opens one long-lived connection to every member of its assigned ring. It sends all `k + m` fragments concurrently and places at most one fragment on each node. The node appends self-describing records (identity, fragment index, checksum, length, and payload) to its current segment and group-commits compatible requests with one `fdatasync`; it advances its durable watermark only after that call succeeds. A receipt or a RAM copy never counts. The shared quorum collector used by both Iceberg and WAL acknowledges as soon as `w` distinct durable watermarks cover the transaction and lets the remaining fragment writes finish in the background. It does not wait for the slowest of `k + m` nodes. Configuration requires `k ≤ w ≤ k + m`; tolerating all `m` fragment losses requires `w = k + m`.

This append protocol replaces one-file-per-fragment placement. It performs no file creation, rename, parent-directory synchronization, coordinator-journal synchronization, or replicated-head round trip per client commit. Segment creation and directory synchronization happen when a segment is allocated, outside the per-commit barrier. A sealed segment carries a checksummed footer and is rolled atomically; recovery scans records only through its persisted durable watermark and intersects the highest contiguous transaction prefix present on `w` distinct nodes. Group commit may delay a request for a short bounded batching window, but the durability rule does not change: an acknowledgment is emitted only after the required `fdatasync` results.

Figure 5 shows how the cluster builds the acknowledgment. The workload-local writer owns protocol sequencing; fragment nodes only append and durably flush records. This split is shared by the object writer and WAL keeper, so neither Iceberg nor Postgres can bypass the same durability barrier.

```mermaid
sequenceDiagram
    participant Client
    participant W as Writer / WAL keeper
    participant NA as Node a
    participant NB as Node b
    participant NC as Node c
    participant S3

    Client->>W: WAL append or object write
    Note over W: Encode stripe by stripe
    par Concurrent fragment appends
        W->>NA: Append self-describing fragment 0
        W->>NB: Append self-describing fragment 1
        W->>NC: Append self-describing fragment 2
    end
    Note over NA,NC: Group commit to open segment
    NA-->>W: Durable watermark after fdatasync
    NB-->>W: Durable watermark after fdatasync
    NC-->>W: Durable watermark after fdatasync
    Note over W: w distinct durable watermarks reached
    W-->>Client: ACK
    Note over W,S3: Background drain after ACK
    W->>S3: PUT reconstructed object or WAL segment
    S3-->>W: 200
    Note over W: Commit clean marker and release fragments
```

*Figure 5: The common durable-EC acknowledgment protocol with `k = 2, m = 1, w = 3`. Each fragment lands on a distinct node's append-only log and counts only when that node's `fdatasync` advances beyond the record. Self-describing fragment records eliminate a second coordinator-journal barrier. Propagation to object storage runs after acknowledgment; fragments are released only once the origin write succeeds.*

Concurrent object writers need no consensus. Every PUT is an independent, immutable erasure-coded transaction under a pod-unique identifier: two writes to the same key share no fragments, so the quorum counts the durability of one write rather than agreement among writers. Ordering between them is the contract the origin itself offers for concurrent PUTs — the last acknowledged write wins. A ring-recoverable dirty index points at the most recently acknowledged transaction, reads of the key serve that object, version-committed reads (§5.1) never interleave two versions, and propagation delivers to an origin whose own semantics for concurrent PUTs are also last-writer-wins. The quorum of this section is a durability quorum, not an agreement protocol; the coordination problem concurrent writers usually pose is dissolved by immutability, exactly as on the read path.

### 6.2 Degraded operation

Before consuming a request body, the writer checks live membership and fragment-store headroom. A standalone deployment with no configured EC ring streams the request directly to the origin and acknowledges only after that write succeeds. A configured cluster requires `w` eligible nodes and returns an explicit write failure when it cannot meet that EC quorum; it never changes durability modes mid-request or bypasses the ring through an origin fallback.

An acknowledged but unpropagated object is readable through the same public S3 endpoint. GET and HEAD consult the ring-recovered dirty index and reconstruct the requested object from its fragments. Background propagation uploads the object to the origin with capped exponential backoff and keeps retrying until it succeeds or the process shuts down. It appends a clean record and releases fragments only after the origin write succeeds. Dirty transactions are recovered from fragment logs after restart.

The repair loop observes membership changes. If an acknowledged object loses a fragment node but retains at least `k` placements, the writer reconstructs the object, re-encodes the missing fragments, and places them on unused live nodes. Contract tests cover acknowledgment threshold, a delayed fourth node, insufficient-node failure without bypass, mid-stream shortfall, read-after-write, propagation, node-loss repair, process loss, and recovery of the highest quorum-durable prefix.
...
A background scrubber periodically walks fragments associated with dirty transactions. It verifies each checksum and re-encodes missing or corrupt fragments while at least `k` healthy fragments remain. The scrubber yields between objects and reports the number of fragments examined, corrupt fragments found, and fragments repaired. Integration tests inject fragment corruption, verify byte-identical reconstruction, require failure when corruption exceeds the parity budget, and exercise repair by the scrubber.

The fragment logs are the authority. Writer-local indexes are disposable accelerators rebuilt by scanning quorum-durable records, so an acknowledged object or WAL range remains discoverable after permanent loss of the writer or WAL-keeper machine.
...
Workers are ordinary code packaged as immutable OCI images. Python projects carry `pyproject.toml` and `uv.lock`; JavaScript and TypeScript projects carry `package.json` and `bun.lock`; other languages supply an auditable Dockerfile. Registration builds a content-addressed image, and the scheduler asks `verglas-container-runtime` to execute one fresh container per dispatch. Secrets enter only at run time. Docker enforces CPU, memory, process, and wall-clock ceilings, so a misbehaving worker is killed without touching the scheduler or server serve paths. A conformance check asserts the two properties that a generated program can violate without a type error: schema stability across runs (a worker must not silently drift its output shape) and kill-and-resume equivalence (a run killed at any point and re-dispatched produces the same table state — the observable form of the idempotent-commit discipline). Watermark monotonicity, a property of the retired job model, is no longer a conformance concern: there is no worker-visible watermark to move backward.
...
Databases are explicit tenant resources. `verglas db create NAME --type postgres` declares an independent managed Verglas Neon database. `verglas db create NAME --type lakehouse` creates managed storage plus a warehouse in the tenant's managed Lakekeeper deployment. Supplying `--data-path s3://…` binds a scoped S3 secret while retaining managed Lakekeeper; supplying `--catalog https://… --warehouse NAME` additionally binds a scoped Iceberg REST secret and uses the external catalog. The stable secret resource IDs are stored on the database record, so later secrets with overlapping scopes cannot silently rebind an existing database; rotating a secret replaces its value under the same ID.

There is one managed Lakekeeper deployment per tenant, not one process per database. Each managed lakehouse receives its own Lakekeeper warehouse and access resource while sharing the tenant's Lakekeeper, Neon dependency, private network, and scale-to-zero lifecycle. A physically dedicated catalog is an explicit isolation/pinning tier rather than the default consequence of creating a logical database. Postgres databases remain independent managed Neon resources and do not acquire catalog bindings.

Secret material is entered through stdin or a hidden prompt with `verglas secret create`; values never appear in command arguments or database records. Encrypted, versioned secret values live beside the tenant authorization state in the `verglas_permissions` Postgres database, while OpenFGA controls `use_secret`. Database creation resolves the longest authorized matching scope once, stores the stable resource ID, and fails closed on missing, ambiguous, or unauthorized bindings.

Each managed Postgres database runs one workload-local **Verglas durable EC keeper** beside compute. It replaces the hacked single-cache-node Neon safekeeper deployment. Neon speaks its existing WAL proposer protocol to the keeper over the VM-local network; the keeper preserves PostgreSQL LSNs and terms, owns long-lived fragment-log connections to all `N` members of the database's assigned ring, and acknowledges only after `w` distinct nodes report a durable watermark for the appended WAL. Cache nodes do not speak the Neon protocol and no raw TCP proxy selects a cache ingress. The keeper is replaceable: after process or machine loss it reconstructs the timeline epoch, contiguous durable tail, and object-store drain position from quorum fragment records before accepting another proposer. Exact or overlapping WAL retries are byte-validated before only a new suffix advances the tail.

The keeper batches adjacent proposer messages by timeline and flush generation, matching Neon's useful group-commit behavior without retaining Neon's single-node safekeeper storage implementation. A bounded batching deadline collects small commits; an explicit proposer flush or a full batch closes it. All fragments in the batch are appended concurrently, and an `fdatasync` on each node advances every covered request at once. Committed WAL accumulates per database timeline to a 16 MiB PostgreSQL WAL segment before background object-store drain; a one-second deadline bounds low-volume tails. The ten-second idle lease therefore gives the timed drain several opportunities before suspension, while retained NVMe remains authoritative if object storage is temporarily unavailable. Pageserver reads both the EC tail and the object-resident prefix through the keeper.

Iceberg enters through the workload-local `verglas-write` service and uses the same durable EC fragment-log client, record format, quorum counter, recovery scanner, and background-drain lifecycle as the WAL keeper. Only its batching boundary differs: a table writer rolls data files at `write.target-file-size-bytes` (512 MiB by default), while the S3 surface emits exact 8 MiB multipart parts and flushes the final short part when the object closes or the client explicitly flushes. Buffers are scoped by warehouse, table, and object; unrelated tables are never coalesced. PUT, COPY, DELETE, multipart create, part upload, complete, and abort all enter through `verglas-write`. No mutating S3 operation is implemented by the read cache or delegated directly to object storage in a configured cluster.

### 7.6 Placement: local server, vessels, and workers

The standalone scheduler owns durable triggers, jobs, and leases but never executes tenant code. It builds locked worker projects through `verglas-container-runtime`, stores the resulting immutable image identity, resolves secret references immediately before a run, and submits each claimed job to the runtime. The runtime creates, limits, observes, and removes the ephemeral Docker container. The engine's storage endpoint points at the server's own cache surface. The commit and read contract between user code and the platform is one HTTP surface — commit, snapshot, rows, delta — served by the local server from the engine library. Because idempotency lives in snapshot summaries (§7.2), a worker or container observes identical semantics whether invoked by cron, HTTP, or a CloudEvent producer.

### 7.7 Cache-pathed I/O

Local I/O uses one S3-compatible endpoint, but the endpoint is deliberately asymmetric. GET, HEAD, and LIST enter the read cache; every mutating operation enters `verglas-write`. The router is not itself a storage implementation and there is no second public endpoint that can write directly to a cache node. The writer owns long-lived connections to all fragment nodes in the workload's dedicated ring and chooses placement below Iceberg `FileIO`, so data and delete files, manifests, manifest lists, metadata JSON, Puffin files, and multipart writes all use the §6 durable EC protocol. The catalog pointer commit remains separate and runs only after every referenced object write is durably acknowledged. Two guarantees follow. First, residency: every read and write a workload makes populates the cache as a side effect, so the tables an operator's workloads touch are precisely the tables the cache keeps warm — the working set is defined by use rather than by prediction. Second, ownership: no user, engine, or harness path chooses a cache ingress or assumes a key is locally owned; placement is exclusively the writer or keeper's responsibility and workloads behave identically on one node or a pod. The turn-off invariant of §3 is preserved: output drains to ordinary committed Iceberg, and with Verglas removed the tables remain readable by any engine.

### 7.8 Queues

Some stages need decoupling that a table's commit cadence does not provide: a burst-prone producer should not block on downstream consumers, and several consumers may need independent positions in one stream. Queues are explicit resources: `verglas queue create NAME` provisions a dedicated managed Verglas Neon database and an independently scalable, stateless queue container through the container service. No queue is created by default or as a side effect of enqueue, poll, or scheduler startup.

Messages receive monotonically increasing positions in PostgreSQL and carry an exact topic plus a producer-defined idempotency identity. A subscription claims exclusive, expiring leases for one consumer group and owner; separate groups receive independent copies while owners inside one group compete. Every delivery carries an opaque receipt containing its claim generation. Acknowledgement succeeds only for that exact live generation, so an expired worker cannot acknowledge a message after another worker has reclaimed it. PostgreSQL `LISTEN/NOTIFY` wakes long-lived subscriptions after commit, but durable rows, lease deadlines, and acknowledgements remain authoritative. The client never polls. Expiry re-delivers unacknowledged messages from an exact deadline timer. Delivery remains at-least-once with consumer-side idempotency, deliberately the same discipline as the commit path (§7.2), while database durability and scale-to-zero recovery use the same Neon, Verglas cache-ring, and object-storage path as §7.5.

Acknowledged Iceberg writes publish their database, table, and snapshot identity to an explicitly configured queue before the write endpoint returns success. PostgreSQL commits wake subscribers in the same transaction that inserts the durable message. A failed Iceberg event publication makes the write request fail after the snapshot commit; retrying the write reuses both the snapshot idempotency key and the queue message identity, repairing delivery without duplicating table state or queue state.
...
Every database-scoped Iceberg REST catalog remains authoritative. A local Verglas server resolves `database_id → catalog_binding_id → gateway/watcher` and exposes that database's shallow cached proxy; the proxy implements no catalog semantics of its own. Managed bindings share the tenant Lakekeeper endpoint but use distinct warehouses. External bindings carry their own endpoint and stable secret ID. The engine's Iceberg REST client and metadata-warming trigger (§5) remain transport-agnostic. The hosted Lakekeeper integration calls that trigger directly after a successful mutation; third-party catalogs may continue to discover changes through polling and therefore provide eventual consistency only.
