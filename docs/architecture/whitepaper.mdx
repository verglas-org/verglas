...
Messages receive monotonically increasing positions in PostgreSQL. Poll claims an exclusive, expiring lease for one consumer group and owner; every delivery carries an opaque receipt containing its claim generation. Acknowledgement succeeds only for that exact live generation, so an expired worker cannot acknowledge a message after another worker has reclaimed it. Expiry re-delivers unacknowledged messages. Delivery remains at-least-once with consumer-side idempotency, deliberately the same discipline as the commit path (§7.2), while database durability and scale-to-zero recovery use the same Neon, Verglas cache-ring, and object-storage path as §7.5.
