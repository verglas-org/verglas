...
An acknowledged but unpropagated object is readable through any cache-node S3 endpoint. GET and HEAD discover the replicated dirty journal on the fragment ring and reconstruct the requested object from its fragments. Background propagation retries indefinitely with capped backoff until the origin accepts the object, marks the journal clean, and releases fragments only after the origin write succeeds. Dirty journals are replayed after restart, and origin reachability never gates recovery or serving.
...
Pageserver layer and index PUTs cross the same ring barrier. Dirty fragments are not evictable until origin propagation succeeds, and safekeeper retention advances only from the pageserver's explicit ring-durable LSN. Neon immutable layer ranges and Iceberg ranges share the cache node's admission ledger. The pageserver also keeps exact `GetPage` reconstruction results by tenant, timeline, relation, block, and LSN in its process-global page cache, so repeated page reads avoid repeated reconstruction without aliasing versions.
