...
Host-managed scale-to-zero uses an admission fence, not a ring drain. The host authenticates to the cache node's private quiescence API, atomically closes foreground admission across S3, attached block, fragment, and Neon safekeeper planes, then waits for the already-accepted count to reach zero before stopping the process. A generation lease prevents a stale failed-stop handler from reopening a newer fence. Retained NVMe is reattached on restart: the cache rebuilds its disk indexes without eagerly copying cached bytes into DRAM, and dirty fragment/WAL state resumes its ordinary replay and background propagation. Catalog polling, recovery, warming, and propagation do not pin the process awake; their durable inputs remain on NVMe or in the origin.
...
Each cache node may also embed the Postgres WAL ingress. Neon compute keeps pooled transports to every member of its database's cache ring but sends each LSN range through exactly one active ingress. That ingress preserves PostgreSQL LSNs and terms and acknowledges only after the new WAL is erasure-coded and fsynced across the Verglas ring. The compute is not given the ring members as a Neon safekeeper quorum: endpoint count is transport availability, while one ingress coordinates the single Verglas durability quorum. Ring-replicated timeline state lets another ingress recover the same logical stream after failure, and exact or overlapping WAL retries are byte-validated before only a new suffix advances the tail. The private Neon proposer may aggregate available WAL into frames of at most 8 MiB, while the ingress performs each frame's independent EC and state-quorum writes concurrently. Committed WAL accumulates per database timeline to a 16 MiB PostgreSQL WAL segment before background object-store drain; a one-second deadline bounds low-volume tails. The ten-second idle lease therefore gives the timed drain several opportunities before suspension, while retained NVMe remains the authority if object storage is temporarily unavailable. Pageserver reads both the EC tail and the object-resident prefix through the same safekeeper protocol. Running the listener inside `verglas-cache-node` shares its NVMe fragment store, peer plane, failure detection, and scheduler lifecycle without introducing another deployable instance.

Iceberg uses a different batching boundary because its commits publish immutable files rather than WAL. A table writer rolls data files at the table's `write.target-file-size-bytes` property (512 MiB by default) and the S3 transport emits exact 8 MiB multipart parts, flushing only the final short part when the file closes. These buffers are scoped to one table writer and multipart operations for one object remain pinned to one cache-ring ingress. This avoids a snapshot per multipart part without pooling unrelated tables or weakening an Iceberg catalog commit's idempotency boundary.
...
Local I/O routes through the server's own S3 surface — the cache. This is a wiring property, not a code dependency: each database or lakehouse receives every S3 endpoint in its dedicated ring, and a workload-local pool assigns objects by rendezvous hash. The pool sits below Iceberg `FileIO`, so data and delete files, manifests, manifest lists, metadata JSON, Puffin files, reads, and multipart writes share the policy. Query parameters do not affect placement, keeping every multipart operation for one object on one ingress; the catalog pointer commit remains separate and runs only after its referenced object writes succeed. Two guarantees follow. First, residency: every read and write a workload makes populates the cache as a side effect, so the tables an operator's workloads touch are precisely the tables the cache keeps warm — the working set is defined by use rather than by prediction. Second, ownership: the §4 ring routes below the engine, so no user or harness code path can assume a key is locally owned, and workloads behave identically on one node or a pod. The turn-off invariant of §3 is preserved: output is ordinary committed Iceberg, and with Verglas removed the tables remain readable by any engine.
