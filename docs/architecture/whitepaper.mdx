...
query and write roles. Cache nodes implement the coded Multi-Raft substrate and
...
The cache fleet hosts many independent Raft groups instead of one global log. A tenant-root group owns warehouse discovery and tenant-scoped allocation; each managed warehouse owns a catalog group; each Neon timeline owns a WAL group. Independent groups elect leaders and commit concurrently, so one busy timeline or catalog does not serialize unrelated tenants. Requests may arrive at any node and are forwarded to the known leader. A client retry carries the same request identity and never becomes a second command.
...
### 6.2 Degraded operation, repair, and archive
...
Committed immutable object bodies, complete WAL segments, and catalog checkpoints drain to object storage in the background. An uploader first makes the object visible and verifies its identity, then proposes an `ArchiveCheckpoint` command. Only application of that committed command permits log truncation or release of the last reconstructable local representation. Origin failure therefore consumes the configured dirty-state budget and eventually applies backpressure; it never permits unsafe eviction or an acknowledgment from RAM.
...
- **Sockets never pin compute.** Container edge sockets and the SDK's `follow` terminate on a hibernating edge object. Managed catalog events are derived from committed warehouse-group indexes rather than delivered by a separate database-backed service. Worker scheduling accepts bounded events, including complete HTTP callbacks; it never owns a client connection.
...
The scheduler is a running queue consumer. Pushed worker events are persisted before the event API acknowledges them and are eligible to run immediately. Cron, retry, and lease deadlines use exact timers derived from durable queue state; there is no periodic queue poll and no separate wake endpoint. Managed catalog data-update events are consumed from the warehouse group's applied event stream; external catalogs enter through their configured gateway. Cache invalidation for managed tables occurs in the same state-machine apply that publishes the new pointer.
...
Local I/O routes through the server's own S3 surface — the cache. This is a wiring property, not a code dependency: the engine reads and writes through whatever storage endpoint it is given, and the server hands it its own. Two guarantees follow. First, residency: every read and write a workload makes populates the cache as a side effect, so the tables an operator's workloads touch are precisely the tables the cache keeps warm. Second, ownership: the §4 ring routes below the engine, so no user or harness code path can assume a key is locally owned. A clean detach preserves portability by draining objects and exporting the catalog as described in §3.2; destroying the authoritative quorum before that fence is not equivalent to disabling a disposable read cache.
...
A managed Iceberg commit creates its database, table, snapshot, and warehouse-index event as part of applying the catalog command. An explicitly configured queue consumes that applied event idempotently and may wake PostgreSQL-backed subscribers, but queue delivery is not part of catalog atomicity. Consumers resume from the warehouse index, so retrying delivery cannot duplicate table state and does not require a cross-database transaction.

### 7.9 Sockets on hibernating edge objects

A long-lived inbound socket must not pin a backend that is supposed to scale to zero. Verglas terminates client-facing sockets on a hibernating edge object. Managed catalog mutation delivery is an applied-log subscription identified by warehouse index, not a client socket or a Lakekeeper outbox callback. Worker scheduling accepts bounded HTTP callback events and does not retain the client connection.

### 7.11 Layering

The hosted control plane resolves `database_id → catalog_binding`. A managed binding routes REST operations through an engine ingress to `tenant root → warehouse group`; the ingress exposes the private state-machine contract, not the hosted Iceberg REST or identity surface. An external binding routes to a remote catalog gateway and carries its own endpoint and stable secret ID. The same query/write engine clients and metadata-warming code operate above both bindings, but their consistency contracts remain explicit: managed reads and commits are linearizable through Raft fences, while third-party catalogs retain their own polling, notification, and transaction behavior. No managed request falls back to an external catalog or PostgreSQL persistence.
