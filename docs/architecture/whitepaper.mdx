...
The scheduler is a running queue consumer. Pushed worker events are persisted before the event API acknowledges them and are eligible to run immediately. Cron, retry, and lease deadlines use exact timers derived from durable queue state; there is no periodic queue poll and no separate wake endpoint. The catalog change feed pushes matching data-update events to the scheduler, which claims work and runs the local worker harness.
