...
principle applies to tenant code. A Durable Object (DO) is the worker: a named
stateful object with durable KV and SQL state and one tenant WASM component.
HTTP, WebSocket, and alarm events enter through the platform gateway. The DO
wakes on demand and runs against its own state. Its event and storage contract
is fixed by the component ABI rather than by a per-run protocol.

The DO boundary keeps user code close to its state without making it a storage
authority. The component can use only the capabilities in its WIT world.
