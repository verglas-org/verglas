...
Verglas, and no dashboard registry is stored in customer tables. Because the
Rill model is materialized against one immutable Iceberg metadata file, refresh
is explicit: Verglas reloads the table through the catalog and rewrites the
owned model with the latest metadata location after a new snapshot commits.
