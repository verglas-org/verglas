...
We propose Verglas, an Iceberg-aware local tier that colocates an S3-compatible storage endpoint with compute and serves frequently accessed bytes from DRAM and NVMe. Independently deployed cache, query, and write roles communicate with an authoritative Iceberg REST catalog and object store; Verglas never becomes either system of record. Verglas keys cached blocks by storage binding and object version, yielding correctness properties that format-blind systems cannot achieve at any configuration. An erasure-coded, quorum-acknowledged write-back path stages writes durably in front of object storage, which remains the system of record. A hosted control plane may schedule these engine roles and the pipeline platform described in §7, but that orchestration is not part of the public storage engine.
...
node (including WAL and virtual block devices) plus independently deployed
query and write roles. The engine is a catalog **client**, not a catalog
implementation: it connects to an Iceberg REST catalog and never stores
authoritative catalog state.

The public CLI and SDKs live in `verglas-client`. Hosted access, scheduling,
workers, databases, integrations, applications, and agent runtime services live
in the private `verglas-cloud` repository. The private `verglas-app` repository
contains only the cloud console and workspace client. `rime` is a separate
public plugin. CI rejects copies of those products in this repository.
...
The hosted platform may attach Rill Developer as an optional analytics
service. Its control plane resolves a selected table through the configured Iceberg
...
A cache node is one `verglas-cache-node` process. Its S3 front end validates SigV4 requests and maps supported operations onto separate read and write interfaces. Its private admin listener exposes health, metrics, and cache-local catalog access for engine roles. Query and write execute in their own processes. The backend layer applies concurrency limits, retry policy, and circuit breaking before issuing origin requests. The cache engine and Iceberg layer sit between these interfaces.
...
does not place scheduler, access, worker, database, application, integration,
or agent-runtime source in this repository.
...
- **Engine/control separation.** Cache, query, and write roles are public and independently runnable. Hosted access, placement, worker scheduling, and container lifecycle live in `verglas-cloud`; they consume engine contracts without becoming storage-engine dependencies.
...
Scheduling is a separate stateless hosted service. Each deployment has one logical worker queue. Manual REST calls, complete HTTP callbacks, cron reconciliation, and catalog data-update subscriptions all materialize the same durable job shape in that queue. A deterministic trigger identity makes enqueue idempotent; lease generations fence competing or recovered consumers; a completion is accepted only for the live generation. The scheduler can therefore restart or migrate without a filesystem guard or process-local cursor. Postgres owns queue state. Public engine roles remain independently runnable and do not mount worker-trigger routes.
...
Each cache node may also embed the Postgres WAL ingress. Neon compute speaks its ordinary safekeeper protocol to one selected reachable cache node; that ingress preserves PostgreSQL LSNs and terms and acknowledges only after the new WAL is erasure-coded and fsynced across the configured Verglas ring. The four-node managed geometry is `k=2, m=2, w=3`; placements run concurrently and the fourth does not delay commit after three durable reports. The compute is not given the ring members as a Neon safekeeper quorum: one ingress coordinates the Verglas durability quorum, while service discovery may select any healthy ingress after a failure. Complete 16 MiB WAL batches drain to object storage in the background; partial tails remain on persistent cache volumes until completion or an explicit lifecycle checkpoint. Pageserver reads both the EC tail and the object-resident prefix through the same safekeeper protocol. Running the listener inside `verglas-cache-node` shares its NVMe fragment store, peer plane, failure detection, and fleet lifecycle without introducing another deployable instance.

### 7.6 Hosted placement, containers, and workers

The hosted scheduler owns durable triggers, jobs, and leases but never executes tenant code. It builds locked worker projects through the cloud container runtime, stores the resulting immutable image identity, resolves secret references immediately before a run, and submits each claimed job to the runtime. The runtime creates, limits, observes, and removes the ephemeral container. Workload storage points at the tenant's cache surface. Because idempotency lives in snapshot summaries (§7.2), a worker or container observes identical semantics whether invoked by cron, HTTP, or a CloudEvent producer.
