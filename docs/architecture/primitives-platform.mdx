...
Machine-loss durability comes from Turso for Durable Objects and from each
product's named external destination. Origin writes complete at the origin
before success; local cache contents are disposable.
...
- **PP8 — six-product and runtime closure.** Deployment and architecture
  ...
  deployment on `verglas-runtime`. Retired execution and storage engines are
