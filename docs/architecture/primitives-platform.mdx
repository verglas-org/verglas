...
object owns one embedded Turso database. The local database file is a cache;
the configured Turso sync authority is the durable state service. An event is
not successful, and its gated WebSocket effects are not released, until its
Turso commit reaches the configured durability boundary.
