...
Object components through Wasmtime under celld/Firecracker isolation, owns
Turso adapters, and uses Foyer memory and disk tiers to cache origin-backed
bytes and compiled/runtime data. Foyer is a performance cache, never a commit
authority.

The runtime is not a storage cluster. It has no Raft, consensus groups, rings,
quorums, peer replication, membership protocol, safekeeper, distributed WAL,
erasure-coded write-back, block-device service, or NBD endpoint. Machine-loss
durability comes from Turso for Durable Objects and from each product's named
external destination. Origin writes complete at the origin before success.
...
identity. These are in-process host calls, not hidden services or a seventh
product. The retired `verglas-cache-node` package, binary, configuration, and
operational surface do not remain as aliases.
...
reaches the Turso durability boundary, `verglas-runtime` sends pending rows to the
...
- **PP8 — six-product and runtime closure.** Deployment and architecture
  inventories expose exactly Worker, Durable Object, Stream, Pipeline, Sink,
  and Catalog. Every system resource is implemented as a prebuilt Worker/DO
  deployment on `verglas-runtime`. No job Worker, direct DO public route,
  standalone queue engine, custom DO database, clustered cache-node, Raft/ring/
  peer plane, distributed write-back, block/NBD service, or direct
  DO-to-Iceberg path remains.
