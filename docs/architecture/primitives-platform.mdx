...
primitives. There is no separate job runtime, queue service, database runtime,
...
not a second SQL commit or storage authority. The complete Turso sidecar set is
...
The following alternate mechanisms are removed rather than retained as
fallback paths:

- a custom SQLite pager or secondary state store;
- an alternate SQL provider or table layer;
- alternate transaction envelopes;
- DO-owned vector/graph mutation domains;
- direct DO-to-Iceberg offload and archive batching;
- an object-store write path as the DO SQL commit authority.
...
- **PP2 — Turso Durable Object.** A literal Cloudflare-style Durable Object
  ...
  alternate SQL provider, alternate mutation format, or direct lake offloader
  remains in the runtime dependency graph.
