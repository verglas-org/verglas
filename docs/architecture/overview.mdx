...
| Analytics | Manage Rill resources when the optional Analytics profile is running. |

Self-hosted installs typically run the REST composition in one container.
Operators may also run the same crates as separate roles so cache, query,
write, catalog, and scheduling scale independently.
