...
const batch = await queue.poll("enrichment-workers", {
  owner: "worker-17",
  max: 100,
  leaseSeconds: 30,
});
for (const delivery of batch.deliveries) {
  // Process delivery.payload idempotently before acknowledging its live lease.
  await queue.ack("enrichment-workers", delivery.receipt);
}
```

Queues deliver at least once. Leases are exclusive until expiry, stale receipts
are fenced, and consumers must make processing idempotent.
