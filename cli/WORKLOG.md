...
- #1: Extracted the unified Verglas CLI into the independent public client repository while preserving its complete command surface and history. Former monorepo dependencies now resolve through explicit pinned engine contracts and the local Rust SDK.
- #1: Rebased CLI documentation fixtures on the new client repository root and removed the engine Compose packaging assertion. The CLI continues to verify that runnable worker examples exactly match their published documentation without claiming ownership of server deployment topology.
...
- Repository consolidation: Moved the CLI back beside the engine and SDKs so one source tree owns the public distribution surface. The CLI remains a thin client of public contracts.
- #67: Removed retired `query`, `graph`, and `index` CLI command groups and their legacy server-route tests. The remaining table commands retain catalog and ingestion responsibilities without advertising removed service products.
- #135: Added a documentation boundary test for standalone and managed-edge
  self-hosting. It requires the S3 origin, optional catalog choices, and managed
